...
$(document).ready(function(){	
	...
	//初始化menu
	menu=new dhtmlXMenuObject("orgTree");
...
//	}
	...
	if(menuid=="addElement"){
		...
		if(type == "PAGE"){
			$.ajax({
				url:"/MenuAction.do?method=createPageElement",
				type:"post",
				async: false,
				dataType:"json",
				data:{itemId:itemId,level:level},
...
//					        		"","tombs.gif","tombs.gif","tombs.gif");
	...
	if(menuid=="addPage"){
		...
		if(type == "MENU"){
			$.ajax({
				url:"/MenuAction.do?method=createPage",
				type:"post",
				async: false,
				dataType:"json",
				data:{itemId:itemId,level:level},
	...
	if(menuid=="deleteMenu"){
		...
		confirm(ITEM_DELETE_CONFIRM,function(){
			$.ajax({
				url:"/MenuAction.do?method=deleteNode",
				type:"post",
				async: false,
				dataType:"json",
				data:{itemId:itemId},
...
function moveUp(itemId,menuid){
	if(checkMoveNode(itemId,menuid)){
		...
		$.ajax({
			url:"/MenuAction.do?method=MoveUpNode",
			type:"post",
			dataType:"json",
...
function moveDown(itemId,menuid){
	if(checkMoveNode(itemId,menuid))
	{
		...
		$.ajax({
			url:"/MenuAction.do?method=MoveDownNode",
			type:"post",
			dataType:"json",
