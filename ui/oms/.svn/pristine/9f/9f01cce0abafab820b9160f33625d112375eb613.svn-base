...
public class DataBaseFactoryImpl implements IDataBaseFactory {
	...
	@Override
	public Map findMapList(String statement, Object paramObj,String parm) throws BusinessException {
		SqlSession sqlSession = null;
		String sql = null;
		try {
			sqlSession = getSession(statement);
			return sqlSession.selectMap(statement, paramObj, parm);
		} catch (Exception e) {
			throw new BusinessException(e, "-20", ",index:" + statement + ",params:" + paramObj);
		} finally {
			closeSession(sqlSession);
		}
	}
