...
function unitSelect(){
	window.top.window.returnCustomModalDialog();
	$("#UnitFrame").attr("src","../views/system/organization/user/UnitSelect.html?check=true")
...
function addNewUser(){
	...
	dom.children().remove();
	dom.html("<input id='txtUserPassword' name='txtUserPassword' type='password' placeholder='' required  maxlength='32' class='form-control' />"+
...
function lockUser(queryFormName,listFormName){
	...
	window.top.window.showModalConfirm( "确定(锁定/解锁)该用户？", locked);
}
function locked(){
...
//	unitSelectModal = window.top.window.borrowCustomModalDialog($("#FrameUnitSelect"));
	$("#UnitFrame").attr("src","../views/system/organization/user/UnitSelect.html?check=false")
	var editDialog = window.top.window.borrowCustomModalDialog($("#FrameUnitSelect"));
	editDialog.modal({show:true, backdrop:'static'});
