...
//			List unitTree = unitService.getRootUnit(currentUser.getUserId());
	...
	public void getUnitTreeManageable(HttpServletRequest request,HttpServletResponse response) {
		try {
			CurrentUser currentUser = WebContextFactory.getWebContext().getCurrentUser();
			List unitTree = unitService.getRootUnit("adminUser");
			if (unitTree.size() > 0) {
...
//			List subItem = unitService.getSubUnit(currentUser.getUserId(), itemId, needStation, false);
			List subItem = unitService.getSubUnit("adminUser", itemId, needStation, false);
			String strXml = null;
			if (subItem.size() != 0) {
	...
	public void getUnitType(HttpServletRequest request,
			HttpServletResponse response) {
...
//		}
	...
	public void saveUnit(HttpServletRequest request,
			HttpServletResponse response) {
...
//			String itemDetail = JSONObject.fromObject(unitVO).toString();
			jsonObject.put("itemDetail", unitVO);
			// response.setContentType("text/json");
			// response.setCharacterEncoding("UTF-8");
		} catch (IOException e) {
	...
	public Map<String, Object> getUnitDetail( HttpServletRequest request,
			HttpServletResponse response) {
