 ...
 *          </p>
 */
public class MenuDAO_Sybase125Impl extends SqlMapClientDaoSupport implements IMenuDAO {

	public MenuDAO_Sybase125Impl() {
	...
	public List getMenuItemsByCondition(WF_ORG_MENU menuItemVO, String expandAll) throws DataAccessException {
		...
		try {
			if (expandAll.equals("1") || expandAll.equalsIgnoreCase("true")) {// 全部查询
				...
				returnList = getSqlMapClient().queryForList("WF_ORG_MENU.selectMenuPart2", menuItemVO);
			} else if (expandAll.equals("0") || expandAll.equalsIgnoreCase("false")) {// 部分查询
				// List userIdList = new ArrayList();
				// List returnlist1 = getSqlMapClient().queryForList(
				// "WF_ORG_MENU.selectRootorSubMenuPart1", menuItemVO);
				// for (int i = 0; i < returnlist1.size(); i++) {
				// Map map = (Map) returnlist1.get(i);
				...
				menuItemVO.setUserIdTemp(roleIDList);
				menuItemVO.setUserID(oldUserID);
				returnList = getSqlMapClient().queryForList("WF_ORG_MENU.selectRootorSubMenuPart2", menuItemVO);
			}
	...
	public List getUserRolesIncludeDelegates(WF_ORG_MENU menuItemVO) throws SQLException, Exception {
		...
		// 1、取自己的角色（带权限排除）
		String tempRoleType = menuItemVO.getRoleType();
		menuItemVO.setRoleType(null);
		list1 = getSqlMapClient().queryForList("WF_ORG_MENU.selectMenuPart1", menuItemVO);
		menuItemVO.setRoleType(tempRoleType);
	...
	public void insertMenuItem(WF_ORG_MENU menuItemVO) throws DataAccessException {
		try {
			List returnList1 = this.getSqlMapClient().queryForList("WF_ORG_MENU.insertPart1", menuItemVO);
			Map map = (Map) returnList1.get(0);
			String str = map.get("menu_order") == null ? "0" : (String) map.get("menu_order").toString();
			// if(returnList1.size() == 0)
			// {
			// str = "0";
			// }
			int num = Integer.parseInt(str);
			menuItemVO.setMaxValue(num);
			this.getSqlMapClient().insert("WF_ORG_MENU.insertPart2", menuItemVO);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new DataAccessException("数据库操作异常");
		} catch (Exception ex) {
