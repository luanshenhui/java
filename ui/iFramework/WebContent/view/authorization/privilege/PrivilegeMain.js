...
$(document).ready(function(){
	...
	tree.setXMLAutoLoading(webpath+"/MenuAction.do?method=getElementsByMenuItemID&showAllMenu=assignable");
	tree.loadXML(webpath+"/MenuAction.do?method=getMenuTree&showAllMenu=assignable");
	...
	//画列表框
	//获取所有角色
	var sURL1 = webpath + "/RoleAction.do?method=getAllRolesJSON";
	$.ajax( {
		url : sURL1,
		type : "post",
		dataType : "json",
		...
		success : function(data) {
			if(data.errorMessage == undefined){
				//循环所有role画下拉列表框
				//下面的方法ie、firefox可用
				var myOption=document.getElementsByTagName('select')[0];
				for(var i=0; i<data.roleList.length;i++){
					var role=data.roleList[i];
					myOption.options[myOption.options.length]=new Option(role.roleName,role.roleId);
					myOption.options[myOption.options.length-1].setAttribute("title", webpath + "/view/base/theme/css/redmond/msdropdown/image/icon_email.gif");
					//myOption.options[myOption.options.length-1].setAttribute("id",role.roleId);
					//myOption.options[myOption.options.length-1].attachEvent('onchange', roleSelect);
				}
...
function drawOrgStatTree(){
	...
	orgTree1.setImagePath(webpath+"/view/base/theme/css/redmond/dhtmlxtree/image/DhtxTree/csh_books/");
	...
	orgTree1.setXMLAutoLoading(webpath+"/UnitAction.do?method=getSubUnitTree&needStation=true"); 
	orgTree1.loadXML(webpath+"/UnitAction.do?method=getUnitTree&needStation=true");
...
function roleSelect(targetCombo){
	...
	var itemText = window.event ? element.innerText : element.textContent;
	
	var selectIndex = $("#"+targetCombo+" option:selected");
	if(selectIndex[0] == undefined || selectIndex[0] == null)
		return null;
	var myOption=document.getElementsByTagName('select')[0];
	var roleID = myOption.options[selectIndex[0].index].value;
		...
		success : function(data) {
			if(data.errorMessage == undefined){
			    ...
			    if (expandAll == "true"){
			    	...
			    	tree.loadXML(webpath+"/MenuAction.do?method=getMenuTree&expandAll=true&showAllMenu=assignable");
			    	//把树全展开
					tree.openAllItems(null);
			    }else {
			    	tree.setXMLAutoLoading(webpath+"/MenuAction.do?method=getElementsByMenuItemID&showAllMenu=assignable");
...
function getSubUnitTreeManageable(id){
	$.ajax({
		url:webpath + "/UnitAction.do?method=getSubUnitTreeManageable&needStation=true",
		type:"post",
		async:false,
		dataType:"json",
		data:{id:id},
		success:function(data){
			if(data.errorMessage==null || data.errorMessage==undefined){
				for(var i=0;i<data.length;i++){
					if(data[i].MANAGEABLE != 1){
						orgTree1.showItemCheckbox(data[i].UNIT_ID,false); //隐藏复选框
						orgTree1.setItemColor(data[i].UNIT_ID,"#aaaaaa","#aaaaaa");//节点字体颜色置灰
					}
				}
			} else {
				if (data.errorMessage == "session timeout")
