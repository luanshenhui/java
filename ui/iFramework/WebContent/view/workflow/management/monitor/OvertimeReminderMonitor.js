...
function doDelete(){
	...
	if(confirm("确认删除？")){
		var instID = ECSideUtil.getPropertyValue(crow,"instID",name);
		var url = webpath + "/OvertimeReminderMonitor.do?method=doDetele";
		jQuery.ajax( {
			url : url,
			type : "post",
			dataType : "json",
...
function doResume(){
	...
	if(confirm("确认恢复？")){
		var instID = ECSideUtil.getPropertyValue(crow,"instID",name);
		var url = webpath + "/OvertimeReminderMonitor.do?method=doResume";
		jQuery.ajax( {
			url : url,
			type : "post",
			dataType : "json",
