...
<title><bean:message key="role.title.admin_role_select"/></title>
...
<script type="text/javascript">
	...
	function confirmRoleSelect(){
		...
		if (crow == null || crow.cells[0] == undefined){
			...
			var selectedRoleName = ECSideUtil.getPropertyValue(crow,"roleName","ec");
			returnValue.itemIds = selectedRoleID;
			returnValue.itemTexts = selectedRoleName;
		}
		return returnValue;
		/*
...
<div style="width:98%">
	<fieldset class="title1"><legend><bean:message key="role.title.admin_role_list"/></legend> 
		<ec:table
			items="roleList" var="record" retrieveRowsCallback="limit" toolbarContent="navigation|pagejump|pagesize|status"
			useAjax="true" action="${pageContext.request.contextPath}/RoleAction.do?method=getRole&forward=adminRoleSelect&isAdminRole=true"
