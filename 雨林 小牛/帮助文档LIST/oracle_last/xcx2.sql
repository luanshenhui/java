...
select s.name,(case when (c.id = s.companyid)then c.name else '无公司' end)
from company c,sales s,
...
select t.name,(case when (c.id = t.companyid)then c.name else '无公司' end) 公司
from company c,
...
where m1.id=m2.movied and m2.tager=t1.id and m1.name='七剑下天山'
...
where m1.id=m2.movied and m1.name='七剑下天山') t2
...
where t1.id=m2.tager and t1.name='动作') t2
...
--1)not null 不能为空
--2)unique 唯一约束
--3)prinary key 主键约束  (not null+unique),一张表设计
--4)foreign key 外键约束     多张表设计
--5)check 检查约束
--6)default 默认值约束

--(二)表之间的关联关系
--  一对一关联；比如用户信息和私密信息。用的少
--  一对多关联：比如部门表和员工表。最常用。多的一方添加公共列。
--  多对多关联：比如学生表盒科目表。添加第三张中间表。

--(三)数据库的三范式
--  第一范式(1NF)：表中的列是原子级的不可再分。
--  第二范式(2NF)：首先满足第一范式，根据领域分组设计表，每张表要有主键
姓名，年龄，公司，公司地址，公司电话
数据沉余
添加异常
删除异常
修改异常
--  第三范式(3NF)：首先满足第二范式，消除传递依赖
...
insert into d values('1','市场部');
insert into d values('2','开发部');
--外键
--添加受限
--insert into e values('1','张三','3');
insert into e values('1','张三','2');
--修改受限
...
select-- (case when m1.nname='杀破狼' then m1.name else '无' end )
from movie m1,
(select *
from tag t1,movie2tag m2
where t1.id=m2.tager and t1.name='动作') t2
