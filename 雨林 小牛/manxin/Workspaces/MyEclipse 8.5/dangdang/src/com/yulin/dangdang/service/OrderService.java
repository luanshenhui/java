...
public class OrderService {
	...
	/* 保存Item */
	public boolean insertItem(List<Item> itemList, int orderId){
		boolean bo = false;
		for(int i = 0; i < itemList.size(); i++){
			itemList.get(i).setOrder_id(orderId);
			bo = new OrderDaoImpl().insertItem(itemList.get(i));
		}
		return bo;
	}
	...
	public boolean insertAddress(Address address){
		return new OrderDaoImpl().insertAddress(address);
	}
	
	/* 改变订单地址 */
	public void updateAddress(Order order){
		new OrderDaoImpl().updateAddress(order);
	}
	
	/* 购物车的假删除 */
	public List<Item> delItem (int id){
		return new OrderDaoImpl().delItemById(id);
	}
