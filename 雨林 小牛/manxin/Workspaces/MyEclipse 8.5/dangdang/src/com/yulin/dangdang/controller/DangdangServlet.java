...
//			System.out.println(order);
			...
			session.setAttribute("delItem", delItem);
			response.sendRedirect("../cart/cart_list.jsp");
		}else if("/huifu".equals(path)){
			int itemId = Integer.parseInt(request.getParameter("itemId"));
			List<Item> delItem = (List<Item>)session.getAttribute("delItem");
			List<Item> itemList = is.turn(delItem, itemId);
			
			session.setAttribute("item", itemList);
			response.sendRedirect("../cart/cart_list.jsp");
		}else if("/deleted".equals(path)){
			int itemId = Integer.parseInt(request.getParameter("itemId"));
			List<Item> delList = (List<Item>)session.getAttribute("delItem");
			System.out.println("delList"+delList);
			
			List<Item> itemList = is.deletedItem(delList, itemId);
			System.out.println("deltel"+itemList);
			if(itemList == null){
				session.setAttribute("delList", null);
			}else{
				session.setAttribute("delList", itemList);
			}
