...
public abstract class HibernateGenericDao<T> extends HibernateDaoSupport {
	...
	public Criteria createCriteria(Session session, String orderBy,
	...
	public List<T> findBy(String propertyName, Object value)
		...
		try {
			session = getSession();
			ll = createCriteria(session, Restrictions.eq(propertyName, value))
					.list();
		} catch (RuntimeException e) {
			throw e;
		} finally {
	...
	public List<T> findBy(String propertyName, Object value, String orderBy,
		...
		try {
			session = getSession();
			ll = createCriteria(session, orderBy, isAsc,
	...
	public List<T> findByValues(String propertyName, Collection values) {
		...
		try {
			session = getSession();
			ll = createCriteria(session, Restrictions.in(propertyName, values))
					.list();
		} catch (RuntimeException e) {
			throw e;
		} finally {
	...
	public T findUniqueBy(String propertyName, Object value) {
		...
		try {
			session = getSession();
			obj = createCriteria(session, Restrictions.eq(propertyName, value))
					.uniqueResult();
		} catch (RuntimeException e) {
			throw e;
		} finally {
	...
	public boolean isUnique(Object entity, String uniquePropertyNames) {
		...
		session = getSession();
		Criteria criteria = createCriteria(session).setProjection(
				...
				for (Map map : listMap) {
					...
					while (it.hasNext()) {
						...
						if (value instanceof Timestamp) {
							Date dateValue = DateUtils.parseTimestamp(value
									.toString());
		...
		try {
			...
			if (listMap != null && listMap.size() > 0) {
				...
				for (Map map : listMap) {
					...
					while (it.hasNext()) {
						Map.Entry<String, Object> me = (Entry) it.next();
						String key = StringUtils.getNameFromDB(me.getKey(),
								false);
						Object value = me.getValue();
						if (value == null) {
							continue;
						...
						if (value instanceof Clob) {
							if (value != null) {
								...
								String dateValue = clob.getSubString(1,
										(int) clob.length());
								BeanUtils.setProperty(t, key, dateValue);
								continue;
							}
