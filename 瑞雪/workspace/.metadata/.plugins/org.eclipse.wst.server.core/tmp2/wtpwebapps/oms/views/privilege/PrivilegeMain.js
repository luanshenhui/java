...
//				$("#listbox").hide();
				 ...
				 window.top.window.showModalAlert("操作失败,"+data.errorMessage);
			}
		},
			...
			if(data.errorMessage==null || data.errorMessage==undefined){
				for(var i=0;i<data.obj.length;i++){
					if(data.obj[i].MANAGEABLE != 1){
						orgTree1.showItemCheckbox(data.obj[i].UNIT_ID,false); //闅愯棌澶嶉�夋
						orgTree1.setItemColor(data.obj[i].UNIT_ID,"#aaaaaa","#aaaaaa");//鑺傜偣瀛椾綋棰滆壊缃伆
					}
				}
			} else {
				 window.top.window.showModalAlert("操作失败,"+data.errorMessage);
