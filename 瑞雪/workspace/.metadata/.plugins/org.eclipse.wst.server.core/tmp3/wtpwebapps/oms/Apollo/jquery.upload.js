	...
	ajaxFileUpload : function(s) {
		...
		var uploadCallback = function(isTimeout) {
			...
			if (xml || isTimeout == "timeout") {
				...
				var status ;
				try {
					status = (isTimeout != "timeout"||xml) ? "success" : "error";
		...
		try {
			...
			//=== add the parameter that POST XXX
			if(typeof s.param!='undefined'){
				for(var e in s.param){
					var v = s.param[e];
					var inputs = '<input id="'+e+'" name="'+e+'" value="'+v+'"/>';
...
/*
$.ajaxFileUpload({
                 url:'<%=path%>/msisdn_range/updataMsisdn.action?group_id='+group_id+'&index=MSISDN!INSERT',
                 secureuri:false,
