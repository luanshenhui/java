...
dhtmlXTreeObject.prototype.enableIEImageFix=function(mode){
	...
	this._getImg=function(){ var z=document.createElement("DIV"); z.innerHTML="&nbsp;"; z.style.width="18px"; z.style.height="18px"; return z; }
...
*/
   dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode,afterNode){
      ...
      if (!this.XMLloadingWarning)
      {
             ...
             //2010/12/02  Thu 以下 注释掉的2行代码用于解决新加结点，删除结点时，树分支显示错乱的问题
    ...
    dhtmlObject.skipLock=true;
      ...
      for(var i=start||0; i<node.childNodes.length; i++)
      {
           ...
           if ((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
         {
            ...
            var locked = nodx.getAttribute("locked");
                  ...
                  var zcall="";
                        if (topoffset) dhtmlObject.setItemTopOffset(newNode,topoffset);
      ...
      if (!level) {
         ...
         for (var n=0; n<chArr.length; n++)
            if (chArr[n]) dhtmlObject.setCheck(chArr[n],1);
         		//----->wxy start Time:Thu May 27 14:09:04 2010 cause:带checkbox的树，点到没有子结点时会报错
         		var blnTemp = false;
         		if(temp != null)
         			blnTemp = (temp.id!=dhtmlObject.rootId);
         		//------>wxy over
               if ((dhtmlObject.XMLsource)&&(dhtmlObject.tscheck)&&(dhtmlObject.smcheck)&&(blnTemp)){// wxy -->使用blnTemp替换：temp.id!=dhtmlObject.rootId)){
         ...
         if (dhtmlObject._branchUpdate) dhtmlObject._branchUpdateNext(node);
	     }
      ...
      if (dhtmlObject.parsCount==1) {
         ...
         if ((!dhtmlObject._edsbps)||(!dhtmlObject._edsbpsA.length)){
               	window.setTimeout( function(){ if (dhtmlObject.onXLE) dhtmlObject.onXLE(dhtmlObject,parentId); dhtmlObject.callEvent("onXLE",[dhtmlObject,parentId]); },1);
...
*/
   dhtmlXTreeObject.prototype._correctPlus=function(itemObject){
		...
		//Wed Sep 15 11:58:55 2010 wxy idx变量用于解决分支结点加减号的问题
		var plusImgIndex = -1;
		var minusImgIndex = -1;
		var lineImgIndex = -1;
		if(imsrc.src){
			plusImgIndex = imsrc.src.indexOf("plus",0);
			minusImgIndex = imsrc.src.indexOf("minus",0);
			lineImgIndex = imsrc.src.indexOf("line",0);
		}
		//over
      ...
      if (!itemObject.treeNod.treeLinesOn) this._setSrc(imsrc,this.imPath+workArray[3]);
          ...
          //Wed Sep 15 11:56:11 2010 下面用于解决树分支条显示图片

          //当前点的结点是plus结点，而且该结点已经没有子结点
          if((plusImgIndex > 0 || lineImgIndex > 0) && itemObject.childsCount == 0)
        	  workArray = this.lineArray;
          else if((plusImgIndex > 0 || lineImgIndex > 0) && itemObject.childsCount > 0)//当前结点是plus结点，且有子结点,点后变成minus结点
        	  workArray = this.minusArray;
          else if(minusImgIndex > 0 && this._getOpenState(itemObject) == -1)//当前结点是minus结点，点后发现里面又有数据了，则点后还是minus结点
        	  workArray = this.plusArray;
          else if(minusImgIndex > 0 && this._getOpenState(itemObject) == 1)//当前结点是minus结点，且它的子结没隐藏，则点后变plus结点
        	  workArray = this.minusArray;
          else
        	  workArray = this.plusArray;
...
*/      
   dhtmlXTreeObject.prototype.onRowClick2=function(){
    ...
    //modify likan wxy
    if(that.getSelectedItemId()&&(this.parentObject.XMLload==1)&&this.parentObject.childsCount<=0)
    //	that._loadDynXML(that.getSelectedItemId());
    	that._loadDynXML(this.parentObject.id);
      if (!this.parentObject.treeNod.callEvent("onDblClick",[this.parentObject.id,this.parentObject.treeNod])) return 0;
...
//#on_open_end_event:11052006{
   if    (that.checkEvent("onOpenEnd"))
           if (!that.xmlstate)
   			...
   			//Thu Sep 16 10:58:02 2010 wxy 注释下面部分，用于解决多次操作树后产生的Stack overflow问题
            //else{
...
*     @eventdesc: Event raised immideatly after selection in tree was changed
...
*/      
   ...
   if (htmlObject.tr.previousSibling)  tempos2=htmlObject.tr.previousSibling.nodem;
      ...
      //2010/12/02  Thu 下面注释的2行，用于修改删除后图标错乱问题，目前仅考虑动态加载树，如果是静态树可能会有问题
      if (tempos) {
...
dhtmlXTreeObject.prototype.enableContextMenu=function(menu){
	...
	_tree.attachEvent("onClick",function(sId){
		...
		_tree.lastFocusItem=tree._idpull[sId].span;
	});
...
dhtmlXTreeObject.prototype.closeAllItems =function(itemId)
{
    ...
    if (!temp) return 0;
    this._xcloseAll(temp);
	//temp.childNodes[0].id;  //li-k 当右侧树没有子节点，点击时报脚本错误
...
{
	if (item.childsCount)return this._lastChild(item.childNodes[item.childsCount-1]);
    else 
    return item
};
dhtmlXTreeObject.prototype._getPrevNode=function(node,mode){
    ...
    return node.tr.previousSibling.nodem; //li-k 上移问题修改
