...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        var dfop =
            {
                ...
                showTitle:true,  //是否显示节点title
		        nodeclickshowtree:true //是否启用点击节点文字展开树
            };
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            //是否显示title 2010-09-26
            if(dfop.showTitle){
            	ht.push("<div id='", id, "_", nid, "' tpath='", path, "' unselectable='on' title='", nd.text, "'");
            }else{
            	ht.push("<div id='", id, "_", nid, "' tpath='", path, "' unselectable='on' ");
            }            
            var cs = [];
        ...
        function nodeclick(e) {
            ...
            if (et.tagName == "IMG") {
                ...
                //2011-03-16针对点击节点文字展开树增加
                if (et.tagName == "SPAN"&&$(et).parent().is("A")&&dfop.nodeclickshowtree) {
                	imgExpand(et,path,item);
                }
                dfop.citem = item;
        ...
        function imgExpand(et,path,item){ 
        	var _div = $(et).parent().parent();
        	var _img = $(_div).children(".bbit-tree-ec-icon");		
		if ($(_img).hasClass("bbit-tree-elbow-plus") || $(_img).hasClass("bbit-tree-elbow-end-plus")) {
        		var ul = $(et).parent().parent().next(); //"bbit-tree-node-ct"
                if (ul.hasClass("bbit-tree-node-ct")) {
                    ul.show();
                }
                else {
                ...
                if ($(_img).hasClass("bbit-tree-elbow-plus")) {
                    $(_img).swapClass("bbit-tree-elbow-plus", "bbit-tree-elbow-minus");
                }
                else {
                    $(_img).swapClass("bbit-tree-elbow-end-plus", "bbit-tree-elbow-end-minus");
                }
                $(_div).swapClass("bbit-tree-node-collapsed", "bbit-tree-node-expanded");
        	}                
		else if ($(_img).hasClass("bbit-tree-elbow-minus") || $(_img).hasClass("bbit-tree-elbow-end-minus")) {  //- 号需要收缩
                    $(_div).next().hide();
                    if ($(_img).hasClass("bbit-tree-elbow-minus")) {
                        $(_img).swapClass("bbit-tree-elbow-minus", "bbit-tree-elbow-plus");
                    }
                    else {
                        $(_img).swapClass("bbit-tree-elbow-end-minus", "bbit-tree-elbow-end-plus");
                    }
                    $(_div).swapClass("bbit-tree-node-expanded", "bbit-tree-node-collapsed");
                }             
        }
        function expandnode() {
            var item = this;
