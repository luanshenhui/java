 ...
 *          </p>
 */
public class RoleDAO_Sybase125Impl extends SqlMapClientDaoSupport implements IRoleDAO {

	/**
	 * 构造函数
	 */
	public RoleDAO_Sybase125Impl() {
	...
	public List getRoleByCondition(WF_ORG_ROLE roleVO) throws DataAccessException {
		...
		try {
			if (roleVO.getStartRow() == 0 && roleVO.getEndRow() == 0) {
				resultList = this.getSqlMapClient().queryForList("WF_ORG_ROLE.select", roleVO);
			} else {
				String querySQL1 = "select ROLE_ID,\n" + "       PARENT_ROLE_ID,\n" + "       ROLE_NAME,\n"
						+ "       ROLE_DESCRIPTION,\n" + "       IS_ADMINROLE,\n" + "       ROLE_TIME_BEGIN,\n"
						+ "       ROLE_TIME_END,\n" + "       USER_NUMBERS,\n" + "       IS_UNIQUE\n"
						+ "  FROM WF_ORG_ROLE R\n"
						+ (roleVO.getUserId() != null || roleVO.getRoleId() != null || roleVO.getParentRoleId() != null
								|| roleVO.getRoleName() != null || roleVO.getIsAdminrole() != null ? " where " : "")
						+ (roleVO.getUserId() == null ? ""
								: "   R.ROLE_ID IN\n" + " (SELECT A.ROLE_ID FROM WF_ORG_USER_ROLE A WHERE A.USER_ID = '"
										+ roleVO.getUserId() + "')\n");
				String querySQL2 = "";
				String querySQL3 = "";
				String querySQL4 = "";
				...
				if (roleVO.getUserId() == null) {
					querySQL2 = (roleVO.getRoleId() == null ? "" : " ROLE_ID = '" + roleVO.getRoleId() + "'\n");
				} else {
					querySQL2 = (roleVO.getRoleId() == null ? "" : "   and ROLE_ID = '" + roleVO.getRoleId() + "'\n");
				}
				if (roleVO.getUserId() == null && roleVO.getRoleId() == null) {
					querySQL3 = (roleVO.getParentRoleId() == null ? ""
							: " PARENT_ROLE_ID = '" + roleVO.getParentRoleId() + "'\n");
				} else {
					querySQL3 = (roleVO.getParentRoleId() == null ? ""
							: "   and PARENT_ROLE_ID = '" + roleVO.getParentRoleId() + "'\n");
				}
				if (roleVO.getParentRoleId() == null && roleVO.getRoleId() == null && roleVO.getUserId() == null) {
					querySQL4 = (roleVO.getRoleName() == null ? ""
							: " ROLE_NAME like '" + roleVO.getRoleName() + "'\n");
				} else {
					querySQL4 = (roleVO.getRoleName() == null ? ""
							: "   and ROLE_NAME like '" + roleVO.getRoleName() + "'\n");
				}
				if (roleVO.getRoleName() == null && roleVO.getRoleId() == null && roleVO.getUserId() == null
						&& roleVO.getParentRoleId() == null) {
					querySQL5 = (roleVO.getIsAdminrole() == null ? ""
							: " IS_ADMINROLE = '" + roleVO.getIsAdminrole() + "'");
				} else {
					querySQL5 = (roleVO.getIsAdminrole() == null ? ""
							: "   and IS_ADMINROLE = '" + roleVO.getIsAdminrole() + "'");
				}
				String querySQL = querySQL1 + querySQL2 + querySQL3 + querySQL4 + querySQL5;
				Map paramMap = new HashMap();
				paramMap.put("sqlStr", querySQL);
				paramMap.put("startRec", roleVO.getStartRow());
				paramMap.put("lastRec", roleVO.getEndRow());
				resultList = this.getSqlMapClient().queryForList("WF_ORG_ROLE.GetPagingData", paramMap);
			}
		} catch (SQLException e) {
			e.printStackTrace();
