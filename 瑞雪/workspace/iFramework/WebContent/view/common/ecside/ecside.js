...
var ECSide=function(formid){
	...
	Me.fillForm=function(originalRequest){
		...
		//Mon Oct 10 16:22:57 2011 wxy 表格刷新结果是会话超时，自动跳转到登录页面
		if (newhtml.indexOf("timeout") != -1){
			confirm("是否重新登录？",sessionTimeOut,"会话超时");
			newhtml = "";
		}
		//over
		if (newhtml==''){
	...
	Me.beforeSubmit=function(formid,crow,shadowRow,eventSrc){};
 	...
 	Me.init=function(){
		...
		Me.ECForm.style.visibility ="visible";
		if(hashTable.get(999999) != "undefined" && hashTable.get(999999) != null){
		ECSideUtil.initPageCheckValue("checkBoxID",1);//初始化本页
		//把所有的checkbox都设置onchange事件
		for(i = 0; i < Me.ECForm.elements["checkBoxID"].length; i++) {
			if (!Me.ECForm.elements["checkBoxID"][i].disabled){
				//给checkbox绑定check事件，check掉的时候，循环初始的，把它删除掉
				Me.ECForm.elements["checkBoxID"][i].onchange = function(){
					if (this.checked==false){
						if(hashTable.get(999999) != "undefined" && hashTable.get(999999) != null){
				            for(var n=0;n<hashTable.get(999999).length;n++){   
				                if(hashTable.get(999999)[n]==this.value){ 
				                	hashTable.get(999999).baoremove(n);
				                	break;
				                }
				            }
					    }
...
//屏蔽，否则导致FF下表格不断变大
	...
	Me.handleResize=function(){
		...
		//以下内容导致弹出窗口中的表格内容太窄
		//ECSideUtil.syncRegionTableWidth(Me.ECListHead.parentNode,Me.ECListBody.parentNode,Me.ECListHead.rows,Me.ECListBody.rows);
...
Me.updateCallBack=function(responseObj){
	...
	//var tableId=rs[0];//注释此行   li-k 解决 有tableId属性时，正在提交 不消失 
...
ECSideUtil.syncRegionTableWidth=function(tableObj,bodyObj,rows1,rows2){
	//表格宽度被撑开，解决上下宽度不统一导致表格对不齐的问题
	var tableWidth=6;
	tableObj.width = tableWidth+"px";
	bodyObj.width = tableWidth+"px";
};
	...
	if (rows1 && rows2 && rows1.length>0 && rows2.length>0){
		...
		if (window.isIE){
			...
			for (var i=0;i<colNum;i++ )	{
				var tt;
				//兼容宽度设置为px 或 % 两种情况 2011-01-27 liyang
				if(/^((-|\+)?\d{1,2}(\.\d+)?|100)%$/.test(bodyTDs[i].width)){
					//如果宽度用% 则不用重设宽度
				}else{
					...
					//tt=bodyTDs[i].offsetWidth;elementObj
					var brw=0;
					...
					if (headTDs[i].getAttribute("resizeColWidth")!="true" && dw!=0){
						...
						headTDs[i].width =(tt+ FF_FIX+ brw ) +dw+"px";
						bodyTDs[i].width =(tt+ FF_FIX+ brw ) +dw+"px";
						bodyTDs[i].style.width =(tt+ FF_FIX+ brw ) +dw+"px";
					}
			}
			}
...
ECSideUtil.gotoPage=function(pageno,formid){
	var allcheckarray =ECSideUtil.getPageCheckValue("checkBoxID")  ;//获得所有的check值
	var ecsideObj=ECSideUtil.getGridObj(formid);
	//把当前页选定的checkbox放到HashTable中去，以便于下次到此页提取
	hashTable.put(ecsideObj.getPageNo(),allcheckarray);	
	...
	//alert(hashTable.get(ecsideObj.getPageNo()));
	if(hashTable.get(999999) != "undefined" && hashTable.get(999999) != null){
	//如果有初始化check，则需要先挑上
	ECSideUtil.initPageCheckValue("checkBoxID",ecsideObj.getPageNo());
	for(i = 0; i < form.elements["checkBoxID"].length; i++) {
		if (!form.elements["checkBoxID"][i].disabled){
			//给checkbox绑定check事件，check掉的时候，循环初始的，把它删除掉
			form.elements["checkBoxID"][i].onchange = function(){
				if (this.checked==false){
					if(hashTable.get(999999) != "undefined" && hashTable.get(999999) != null){
			            for(var n=0;n<hashTable.get(999999).length;n++){   
			                if(hashTable.get(999999)[n]==this.value){ 
		...
		if(hashTable.get(ecsideObj.getPageNo()) != "undefined" && hashTable.get(ecsideObj.getPageNo()) != null)
			ECSideUtil.initPageCheckValue("checkBoxID",hashTable.get(ecsideObj.getPageNo()));//初始化本页
	}
	//跳转页后，重新算表格对齐问题
	listBodyWidthFlag = false;
};
...
ECSideUtil.gotoPageByInput=function(inputNoObj,formid){
	...
	if(totalpages==0){
		alert(ECSideUtil.getMessage("NO_RECODE"));
		return;
	}
...
ECSideUtil.NearPagesBar.showMe=function(fireObj,formid){
	...
	//修改对火狐top值的算法，火狐不需要再加上document.body.scrollTop被卷的部分。2011-5-26 liyang
	var app=navigator.appName;
	  var verStr=navigator.appVersion;
	  if (app.indexOf('Netscape') != -1) {
		  this.coreElement.style.top= (ECSideUtil.getPosTop(fireObj)- this.coreElement.offsetHeight) +"px";
	  }else{
...
ECSideUtil.DragobjBodyCellList=[];
...
/*	if(ECSideUtil.Dragobj==null){
			ECSideUtil.Dragobj = obj.parentElement;
		}*/
...
//修改cellIndex 当有隐藏列时取值错误的bug ie bug 2011-01-27 liyang
	//var cellIndex=ECSideUtil.Dragobj.cellIndex;	
  var cellIndex = ECSideUtil.getCellIndex(ECSideUtil.Dragobj);
  
	//修改ie下如果只修改第一行的列宽，其他行的列居中不对齐的问题 2011-01-26 liyang begin
	try{
		 for (var i=0,j=ECSideList[formid].ECListBody.rows.length;i<j;i++ ){   
            ECSideUtil.DragobjBodyCellList.push(ECSideList[formid].ECListBody.rows[i].cells[cellIndex]); 
        }
  //end
...
//修改过度拖动的bug 2011-01-27 liyang
if(cc<=0){cc = 10;}
ECSideUtil.Dragobj.style.width=cc+"px";
ECSideUtil.DragobjBodyCell.style.width=cc+"px";
ECSideUtil.DragobjBodyCell.width = cc+"px";
for (var i=0,j=ECSideUtil.DragobjBodyCellList.length;i<j;i++ ){   
	//修改ECSideUtil.DragobjBodyCellList[i]=null情况时出现异常
	try{
		 ECSideUtil.DragobjBodyCellList[i].style.width=cc+"px";
         ECSideUtil.DragobjBodyCellList[i].width = cc+"px";
	}catch(Exception){
...
ECSideUtil.getColumnName=function(cellObj,formid){
	...
	var cname='';
	try {
		//Mon Sep 27 17:41:15 2010 wxy 解决ie浏览器td 的display:none后，cellIndex窜的问题，ff浏览器正常
		var idx=ECSideUtil.getCellIndex(cellObj);//cellObj.cellIndex;
...
//Mon Sep 27 17:41:15 2010 wxy 解决ie浏览器td 的display:none后，cellIndex窜的问题，ff浏览器正常
ECSideUtil.getCellIndex= function(td){   
    if (window.isIE){   
...
ECSideUtil.updateCellContent=function(cellObj,elementObj){
		...
		//Tue Jul 30 14:28:53 2013 wangxy 双击CELL进入编辑状态以后，行就变为编辑状态了。
		cellObj.parentNode.setAttribute("edited","true");
		//Tue Jul 30 14:14:38 2013 wangxy CELL可以在状态状态和非编辑状态之间切换
		cellObj.setAttribute("editing","false");
};
...
ECSideUtil.getRowCellsMap=function(rowObj,formid){
	...
	for (var i=0;i<cells.length;i++ ){
		...
		var value=cellObj.getAttribute("cellValue");
		if (!value){
			value='';//2013/09/02  Mon wxy 这样做明显是错误的！window.isIE?cellObj.innerText:cellObj.textContent;
		}
...
ECSideUtil.addToGird = function(buttonObj,templateId,formid){
	...
	// alert(Form.serialize(newTr,true));
	// alert(Form.getElements(newTr));
	...
	//Tue Sep 28 14:44:51 2010 wxy 解决listBody中添加新行后表格对不齐窜位的问题
	//列表宽度只能调整一次
	if(!listBodyWidthFlag){
		var element = ecsideObj.ECListBody.parentNode.parentNode;
		//alert((element.scrollHeight) + "|" + element.clientHeight);
		if (element.scrollHeight>element.clientHeight) {
			var headWidth = ecsideObj.ECListHead.parentNode.parentNode.clientWidth;
			var ListWidth = ecsideObj.ECListBody.parentNode.parentNode.clientWidth;
			ecsideObj.ECListBody.parentNode.parentNode.style.width = ListWidth +32+ "px";
			ecsideObj.ECListHead.parentNode.parentNode.style.width = headWidth + "px";
			//ie浏览器需要设置2遍才能生效，这是ie浏览器的一个bug
			if(window.isIE){
				ListWidth = ecsideObj.ECListBody.parentNode.parentNode.clientWidth;
				ecsideObj.ECListBody.parentNode.parentNode.style.width = ListWidth +20+ "px";
				ecsideObj.ECListHead.parentNode.parentNode.style.width = headWidth + "px";
			}
			listBodyWidthFlag=true;
		}
	}
	// WXY 2014年3月19日15时0分38秒 返回新行
	return newTr;
}
...
//            		if (StringUtils.isBlank(column.getCellName())){
...
ECSideUtil.initPageCheckValue=function(checkboxname,pageNumber){//翻页保存checkbox状态   
	var box = document.getElementsByName(checkboxname);
	var checkedarry = hashTable.get(pageNumber);
	if(checkedarry != "undefined" && checkedarry != null){		
	    for(var i=0;i<box.length;i++){
    ...
    //hashTable中key为999999表示页面初始化时需要传入初始化默认的checked值
    //这个循环可能会很大，大数据量下影响性能。解决方法：调用的页面上，不要hashTable.put(999999,xxx);
    if(hashTable.get(999999) != "undefined" && hashTable.get(999999) != null){
    	for(var m=0;m<box.length;m++){   
            for(var n=0;n<hashTable.get(999999).length;n++){   
                if(hashTable.get(999999)[n]==box[m].value){ 
                    box[m].checked=true;   
                }   
            }   
        }
 ...
 * 获取所有选定的checkbox
 */
ECSideUtil.getCheckedBox=function(checkboxname){//获取所有的check值   
	var carray = new Array();   
    var box = document.getElementsByName(checkboxname);
    for(var i=0;i<box.length;i++){ 
    	//alert(box[i].checked);
...
/**
 * 得到某列的值 currRow 当前行 naem:列名 tableId ：表名
 */
ECSideUtil.getProperty = function(currRow, name, tableId) {
	...
	var theadName = tableId + "_table_head";
	...
	if (thead != null) {
		...
		for ( var i = 0; i < cells.length; i++) {
			if (cells[i].getAttribute("columnname") != null) {
				if (ECSideUtil.trimString(cells[i].getAttribute("columnname")) == ECSideUtil
						.trimString(name)) {
 ...
 * tableId:表名
 */
ECSideUtil.setPropertyValue = function(currRow, name, sValue, tableId) {
		...
		for ( var i = 0; i < cells.length; i++) {
			if (cells[i].getAttribute("columnname") != null) {
				if (ECSideUtil.trimString(cells[i].getAttribute("columnname")) == ECSideUtil
					...
					//Wed Jul 31 10:09:01 2013 wxy 如果不把textContent置空，会影响""值的设置。
					window.isIE ? currRow.cells[cellIndex].innerText=sValue : currRow.cells[cellIndex].textContent=sValue;
 ...
 * tableId:表名
 */
ECSideUtil.getPropertyValue = function(currRow, name, tableId) {
	var tableId;
	var returnValue;
	if (arguments.length > 3 && tableId != undefined && tableId != "") {
		tableId = tableId;
		...
		for ( var i = 0; i < cells.length; i++) {
			if (cells[i].getAttribute("columnname") != null) {
				if (ECSideUtil.trimString(cells[i].getAttribute("columnname")) == ECSideUtil
						.trimString(name)) {
					//cellIndex = cells[i].cellIndex;
					returnValue = currRow.cells[i].innerText;
					flag = true;
					break;
				}
			}
	...
	if (!flag) {
		alert("getPropertyValue设计属性出错，请检查列名是否正确！");
		return;
	}
	return returnValue;
}
...
/**
 * 根据id进行取值（要求必须有checkbox列）
 * id 主键 
 * name 列名 
 * tableId:表名
 */
ECSideUtil.getPropertyValueById = function(rowID, name, tableId) {
	var tableId;
	var returnValue;
	if (arguments.length > 3 && tableId != undefined && tableId != "") {
		tableId = tableId;
...
//~~~~~~~~~~下面是hashtalbe用于存放分页checkbox选中的数据
function Hashtable() {
 ...
 * 根据表格产生json文本
 * tableId 表格id，一般是：ec
 * flag 所要取得数据的标识：insert；update；delete；默认取新增的数据
 */
ECSideUtil.generateJson=function(tableId,flag) {
	if("update" == flag) {
		return "{"+ECSideUtil.getInsertJSON(tableId)+","+ECSideUtil.getDeleteJSON(tableId)+","+ECSideUtil.getUPdateJSON(tableId)+"}";
	} 
	else if("delete" == flag) {
		return "{"+ECSideUtil.getDeleteJSON(tableId)+"}";
	}
	else if("insert,update" == flag) {
		return "{"+ECSideUtil.getInsertJSON(tableId)+","+ECSideUtil.getUPdateJSON(tableId)+"}";
	}
	else if("insert,delete" == flag) {
		return "{"+ECSideUtil.getInsertJSON(tableId)+","+ECSideUtil.getDeleteJSON(tableId)+"}";
	}
	else if("update,delete" == flag) {
		return "{"+ECSideUtil.getDeleteJSON(tableId)+","+ECSideUtil.getUPdateJSON(tableId)+"}";
	}
	else if("insert,update,delete" == flag) {
		return "{"+ECSideUtil.getInsertJSON(tableId)+","+ECSideUtil.getDeleteJSON(tableId)+","+ECSideUtil.getUPdateJSON(tableId)+"}";
	}
	else {
		return "{"+ECSideUtil.getInsertJSON(tableId)+"}";
	}
 };
...
/**
 * 生成删除行的JSON数据格式:"deleteRecords":[{}]
 * @param tableId
 */
ECSideUtil.getDeleteJSON=function(tableId){
	var deleteRows = "\"deleteRecords\":[";
	var ecsideObj= ECSideUtil.getGridObj(tableId);
	...
	for (var i = 0; i < rowsi.length; i++){
		...
		for(sKey in pars){
			if(pars[sKey]== "" || pars[sKey] == 'null' || pars[sKey] == undefined)
				pars[sKey] = "";
			deleteRows += "\""+sKey+"\":\""+ECSideUtil.trimString(pars[sKey])+"\"," 
		}
