...
public class MenuGrantDAOImpl implements IMenuGrantDAO {
     ...
     */
	@Override
    public boolean isResourceAvailable(Map paramMap) throws Exception {
        boolean returnValue = false;
        // Object resultObj =
        // this.getSqlMapClient().queryForObject("WF_ORG_RESOURCE_AUTHORITY.getUserResourcePrivilege",
        // paramMap);
        Object resultObj = dao.findList("WF_ORG_RESOURCE_AUTHORITY_getUserResourcePrivilege", paramMap);
    ...
    public void delPriRole(String roleId) throws Exception {
        dao.delete("ROLE_PRIV_DEL", roleId);
    }

    /**
     * wf_org_role_priv 表的保存方法
     * 保存前先进行删除操作
     * @throws Exception 
     */
    @Override
    public void insertPriRole(WF_ORG_ROLE_PRIV entry) throws Exception {
      dao.insert("ROLE_PRIV_insert", entry);
    }

    @Override
    public List getRolePrivList(String id) throws Exception {
        return dao.findAllList("ROLE_PRIV_getRolePrivList", id);
    }
