...
function addMessagePage(){
	...
	var el=window.top.window.document.getElementById('xiaoxi');
    el.target = '_new'; //指定在新窗口打开
    el.click();//触发打开事件
	window.top.window.$("#messageID").val("");
	window.top.window.$("#kaiguan").hide();
	window.top.window.$("#sendWay").attr(selected,"selected");
} 
	...
	$.ajax( {
		...
		success : function(data) {
			if(data.result=="success"){
				...
				shopList();
				window.top.window.$("xiaoxi").click();
				
				var el=window.top.window.document.getElementById('aaa');
				el.target = '_new'; //指定在新窗口打开
				el.click();//触发打开事件
				if(hasAdmin){
					window.top.window.$("#snock").find("button").attr("disabled",false);
				}else{
					window.top.window.$("#snock").find("button").attr("disabled",true);
				}
			}else{
				window.top.window.showModalAlert(data.msg);
			}
...
function operationShopFree(e){
    ...
    var editDialog = window.top.window.borrowCustomModalDialog($("#FrameTempe"));
    editDialog.modal({show:true, backdrop:'static'});
    window.top.window.$("#templateFree").val(template);
