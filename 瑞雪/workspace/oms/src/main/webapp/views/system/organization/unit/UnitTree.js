...
$(function() {
    ...
    $("#userOrgDetailForm").validate({
        ...
        messages : {
            orgName : "请填写组织单元名称"
        // ,
        // maxPerson : "请填写正确机构人数,组织人数不能低于之前"
        },
    });
    ...
    // if($("#maxPersonValue").val()>$("#maxPerson").val()){
    // return false;
    // }
    // var regx = /^[0-9]\d*$/;
...
jQuery(document).ready(function() {
    ...
    // document.getElementById("dialog").innerHTML = '<iframe id="frame"
    // src=""/>';
            ...
            if (data.errorMessage == null || data.errorMessage == undefined) {
                for (var i = 0; i < data.obj.length; i++) {
                    if (data.obj[i].MANAGEABLE != 1) {
                        tree.showItemCheckbox(data.obj[i].UNIT_ID, false); // 隐藏复选框
                        tree.setItemColor(data.obj[i].UNIT_ID, "#aaaaaa", "#aaaaaa");// 节点字体颜色置灰
                    }
                }
            } else {
...
function addOrg() {
    ...
    var html = "<table id='selectOrgTable' class='table table-hover'>" + "<thead><tr><th width='15%' style='padding: 10px 11px;'>"
            + "<input id='unitchk' name='unitchk' type='checkbox' colValue='unitchk' class='checkall' />"
            + "</th><th width='45%' colValue='userAccount'>用户账号</th>" + "<th width='40%' colValue='userFullname'>用户姓名</th></tr></thead></table>";
    $("#kbdy").html(html)
    ...
    window.top.window.returnCustomModalDialog();
    $("#unitchk").click(function() {
        $(this).parent().parent().parent().next().find("input[type='checkbox']").prop("checked", this.checked);
    })
}

function getEntName() {
    jQuery.ajax({
        url : "/unit/getEntName.action",
        type : "post",
        dataType : "json",
        success : function(data) {
            window.top.window.returnCustomModalDialog();
            for (i = 0; i <= data.entName.length; i++) {
                if (i == 0) {
                    document.getElementById("ent").options[i] = new Option("", "");
                } else {
                    document.getElementById("ent").options[i] = new Option(data.entName[i - 1].ent_name, data.entName[i - 1].ent_id);
                }
            }
            $("#ent").val('51f17f6ddbae11e68c4d001e67c7ec3a');
            var editDialog = window.top.window.borrowCustomModalDialog($("#FrameUnitOrg"));
            editDialog.modal({
                show : true,
                backdrop : 'static'
...
function updateOrg() {
    $("#kbdy").children().remove();
    var html = "<table id='selectOrgTable' class='table table-hover'>" + "<thead><tr><th width='15%' style='padding: 10px 11px;'>"
            + "<input id='unitchk2' name='unitchk' type='checkbox' colValue='unitchk2' class='checkall' />"
            + "</th><th width='45%' colValue='userAccount'>用户账号</th>" + "<th width='40%' colValue='userFullname'>用户姓名</th></tr></thead></table>";
    $("#kbdy").html(html)
    $("#selectOrgTable2 tbody").children().remove();
    if (!tree.getSelectedItemId()) {
    ...
    jQuery.ajax({
        ...
        success : function(data) {
            ...
            if (data.errorMessage == null || data.errorMessage == undefined) {
                ...
                // window.top.window.showModalAlert('删除成功');
                window.top.window.showScoMessage('ok', '删除成功');
            } else {
                window.top.window.showModalAlert(data.errorMessage.message);
            }
        }
    });
}
...
function onClickCheck() {
    ...
    if (itemColor.acolor == "#aaaaaa") {
        ...
        // 可能存在bug，样式会不断变长
        // jQuery("#btnDel").attr("class",classString +"ui-button ui-widget
...
function saveFormElement() {
    ...
    if (flagType == 0 || flagType == "0") {
        ...
        // extInfo =
        // decodeURIComponent(extInfo,true);//对序列后信息进行解码，以解决乱码问题。但是时间中的空格问题无法解决
    ...
    if (flagType == 1 || flagType == "1") {
        ...
        jQuery.ajax({
            ...
            success : function(obj) {
                if (obj.errorMessage) {
                    window.top.window.showModalAlert(obj.errorMessage);
                    var div = window.top.window.borrowCustomModalDialog($("#FrameUnitOrg"));
                    div.modal({
                        show : true,
                        backdrop : 'static'
...
function showDetail(unitId) {
    ...
    jQuery.ajax({
        ...
        success : function(data) {
            if (data) {
                ...
                for (var k = 0; k < data.userDetail.length; k++) {
                    var tr = "<tr role='row' class='odd'><td><input id='chkItem' name='chkItem' type='checkbox' userid="
                            + data.userDetail[k].userId + "></td><td class='sorting_1'>" + data.userDetail[k].userAccount + "</td><td>"
                            + data.userDetail[k].userFullname + "</td></tr>"
                    $("#newtbody").append(tr);
                }
                var editDialog = window.top.window.borrowCustomModalDialog($("#FrameUnitOrg"));
                editDialog.modal({
            ...
            // if(obj[i].name !="id"){
            // //document.getElementById(obj[i].name).value =
            // mapData[obj[i].name];
...
function getUnitType() {
    jQuery.ajax({
        ...
        data : {
            type : "unittype"
        }, // unittype在数据库wf_org_dictionary表中写死的字段值
        success : function(data) {
            window.top.window.returnCustomModalDialog();
            for (i = 0; i <= data.eleType.length; i++) {
                if (i == 0) {
                    ...
                    // document.getElementById("orgTypeSelect").options[i] = new
                    // Option(data.obj[i-1][1],data.obj[i-1][0]);
                    document.getElementById("orgTypeSelect").options[i] = new Option(data.eleType[i - 1].DICT_NAME,
                            data.eleType[i - 1].DICT_CODE);
                }
            }
            var editDialog = window.top.window.borrowCustomModalDialog($("#FrameUnitOrg"));
            editDialog.modal({
...
function ClearInput() {  
    //清空输入域  
   var validator = $("#FrameUnitOrg").validate({  
        submitHandler: function (form) {  
            form.submit();  
        }  
    });  
    validator.resetForm();  
    $('#FrameUnitOrg .state-error').removeClass('state-error');
    $('#FrameUnitOrg .state-success').removeClass('state-success');
}