...
public class DictController extends ApolloController {
	...
	public Map<String, Object> addDict(@RequestBody DictVO dictVO) throws Exception {
		...
		try {
			...
			dictVO2.setDictCode(dictVO.getDictCode());
			List<DictVO> dictVOList = dictService.getDictByCondition(dictVO2);
			if(dictVOList.size()>= 1){
				returnMap.put(JSON_RESULT, FAILED);
	...
	public Map<String, Object> updateDict(@RequestBody DictVO dictVO) throws Exception {
		...
		try {
			...
			String dicUpdateCode = dictVO.getDicUpdateCode();
			if(!code.equals(dicUpdateCode)){
				DictVO dictVO2 = new DictVO();
				dictVO2.setDictCode(code);
				List<DictVO> dictVOList = dictService.getDictByCondition(dictVO2);
				if(dictVOList.size()>= 1){
					returnMap.put(JSON_RESULT, FAILED);
					returnMap.put(JSON_MSG, "该内部编码已存在");
    ...
    public Map<String, Object> getDictTypeList() throws Exception {
    	/// 用于返回值的json对象
    	Map<String, Object> returnMap = new HashMap<String, Object>();
    	try {
    		List list = dictService.getDictTypeList(new OMS_DICT());
    		returnMap.put(JSON_RESULT, SUCCESS);
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/getParamDictByCode", method = RequestMethod.GET)
    public Map<String, Object> getDictValueType(String code) throws Exception {
    	/// 用于返回值的json对象
    	Map<String, Object> returnMap = new HashMap<String, Object>();
