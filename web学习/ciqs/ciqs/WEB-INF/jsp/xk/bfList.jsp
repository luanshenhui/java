...
<script type="text/javascript"> 
		...
		function pageUtil(page) {
			$("#xk_form").attr("action", "<%=request.getContextPath()%>/xk/bfList?page="+page);
			$("#xk_form").submit();
		}
		...
		function doStatus(license_dno,id){
			qdstatus = $("#sp_"+id).val();
			$("#xk_form").attr("action", "<%=request.getContextPath()%>/xk/insertEvent?status="+qdstatus+"&license_dno="+license_dno+"&type=bf");
	   	    $.blockUI({ message: $('#showDel'), css: { width: '275px' } });
	...
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">口岸卫生许可证核发</a> &gt; <a
				href="${cxt}<%=request.getContextPath()%>/xk/bfList">补发审批</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>	
<!-- 		<div class="title">现场评审查询</div> -->
		<div class="search">
			<div class="main">
				<form action="<%=request.getContextPath()%>/xk/bfList"  method="post" id="xk_form">
					<table class="table_search" id="aa">
						...
						<tr>
							...
							<td style="padding-top:8px;width: 250px;" align="left">补发审批结果:</td>
							<td></td>
						</tr>
						<tr>
							<td align="left" >
							...
							<td align="left">
								<select name="status" id="review_result">
									<option value="('9','10','17')">全部</option>
									<option value="('10')" <c:if test="${status == \"('10')\"}" >selected="selected"</c:if>>同意</option>
									<option value="('17')" <c:if test="${status == \"('17')\"}" >selected="selected"</c:if>>不同意</option>
								</select>
							</td>
							<td></td>						
						</tr>
							...
							<c:forEach items="${list}" var="row">								
								<tr>
						            ...
						            <c:if test="${row.status == 9}"> 
						            	<select id="sp_${row.id}" style="width:50px">
					            			<option value="10">同意</option>
											<option value="17">不同意</option>
					            		</select>
					            		<br/>
