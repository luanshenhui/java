...
<script type="text/javascript"> 
		...
		function pageUtil(page) {
			$("#xk_form").attr("action", "<%=request.getContextPath()%>/xk/findLicenseDecs?page="+page);
			$("#xk_form").submit();
		}
		
		function doExamination(id,no){
			$("#xk_form").attr("action", "<%=request.getContextPath()%>/xk/doExamination?id="+id
	...
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">口岸卫生许可证核发</a> &gt; <a
				href="<%=request.getContextPath()%>/xk/findScs">部门负责人审核</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>
<!-- 		<div class="title">卫生许可证查询</div> -->
		<div class="search">
			<div class="main">
				<form action="<%=request.getContextPath()%>/xk/findScs"  method="post" id="xk_form">
									...
									<td>
										<c:if test="${empty row.exam_result}">
										  <select style="width:65px" id="exam_result_${row.id}">
										  	<option value="1" <c:if test="${row.first_exam_result == 1}">selected="selected"</c:if>>合格</option>
										  	<option value="0" <c:if test="${row.first_exam_result == 0}">selected="selected"</c:if>>不合格</option>
										  </select>
