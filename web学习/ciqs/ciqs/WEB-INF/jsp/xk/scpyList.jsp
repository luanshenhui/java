...
<script type="text/javascript"> 
		...
		function userShow(url,id) {
			...
			url+="&peson_num="+$("#peson_num_"+id).val()+"&pid="+id;
			window.open(url);
		}
		...
		function scsShow(no){
		    var url = "<%=request.getContextPath()%>/xk/toPsyDtail?license_dno="+no;
			window.open(url);
		}
</script>
	...
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">口岸卫生许可证核发</a> &gt; <a
				href="${cxt}/ciqs/xk/findyScs">审查派员查询</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>
<!-- 		<div class="title">卫生许可证查询</div> -->
		<div class="search">
			<div class="main">
				<form action="/ciqs/xk/findyScs"  method="post" id="xk_form">
					...
					<table>
						...
						<c:if test="${not empty list }">
							<c:forEach items="${list}" var="row">
									...
									<td>
										<input type="button" style="width:60px" value="点击查看" onclick="scsShow('${row.license_dno}')"/>
									</td>
									<td>
