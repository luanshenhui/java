...
{
    /// <summary>
    /// 成績表CL：心電図判定所見リスト生成クラス
    /// </summary>
    public class ReportECGlistCreator : PdfCreator
    {

        /// <summary>
        /// 帳票コード
        /// </summary>
        public override string ReportCd { get; } = "002310";
        ...
        /// <summary>
        /// 成績表CL：心電図判定所見リストデータを読み込む
        /// </summary>
        /// <returns>成績表CL：心電図判定所見リストデータ</returns>
            ...
            string sql = @"
                select
                    ...
                    , decode( 
                        judrname
                        , 'Ａ'
                        , decode(result, '1', '正常', '確認してください（判定：Ａ、所見有り）')
                        , null
                        , decode( 
                            rslcmtcd2
                            , null
                            , decode(result, null, '▲心電図検査待ち', '●心電図自動判定待ち')
                            , '×心電図キャンセル'
                        ) 
                        , decode(result, '1', '確認してください（判定：Ｂ以下、所見なし）', '正常')
                    ) as judcomment 
                from
                    ( 
                        select
                            ...
                            , v_consult3.avgresult as result 
                        from
                            ( 
                                select distinct
                                ...
                                where
                                    ...
                                    and rsl.itemcd in (:para_itemcd01)
                            ) v_consult1 
                            left join ( 
                                select distinct
                                    csldate
                            ...
                            left join ( 
                                ...
                                on v_consult1.rsvno = v_consult3.rsvno
                    ) ecgview1 
                order by
                    csldate
                    , judcomment
        ...
        /// <param name="data">成績表CL：心電図判定所見リストデータ</param>
