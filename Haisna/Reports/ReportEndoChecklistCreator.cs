...
{
    /// <summary>
    /// 成績表CL：上部消化管（内視鏡）CL生成クラス
    /// </summary>
    public class ReportEndoChecklistCreator : PdfCreator
    {
       
        /// <summary>
        /// 帳票コード
        /// </summary>
        public override string ReportCd { get; } = "002270";
        ...
        /// <summary>
        /// 成績表CL：上部消化管（内視鏡）CLデータを読み込む
        /// </summary>
        /// <returns>成績表CL：上部消化管（内視鏡）CLデータ</returns>
                            ...
                            , decode( 
                                ...
                                , decode( 
                                    ...
                                    , decode( 
                                        ctjudcd
                                        , '1'
                                        , '正常（判定：A、レポート判定：A、所見なし）'
                                        , '確認してください（判定とレポート判定が合わない）'
                                    ) 
                                    , '確認してください（判定：A、所見有り）'
                                ) 
                                , null
                                ...
                                , decode( 
                                    result
                                    , '1'
                                    , '確認してください（判定-レポート判定：B1、所見なし）'
                                    , decode( 
                                        ctjudcd
                                        , '2'
                                        , '正常（判定：B1、レポート判定：B1、所見有り）'
                                        , '確認してください（判定とレポート判定が合いません）'
                                    )
                                ) 
                                , 'B2'
                                , decode( 
                                    result
                                    , '1'
                                    , '確認してください（判定-レポート判定：B2、所見なし）'
                                    , decode( 
                                        ctjudcd
                                        , '3'
                                        , '正常（判定：B2、レポート判定：B2、所見有り）'
                                        , '確認してください（判定とレポート判定が合いません）'
                                    )
                                ) 
                                , 'C1'
                                , decode( 
                                    result
                                    , '1'
                                    , '確認してください（判定-レポート判定：C1、所見なし）'
                                    , decode( 
                                        ctjudcd
                                        , '4'
                                        , '正常（判定：C1、レポート判定：C1、所見有り）'
                                        , '確認してください（判定とレポート判定が合いません）'
                                    )
                                ) 
                                , 'C3'
                                , decode( 
                                    result
                                    , '1'
                                    , '確認してください（判定-レポート判定：C3、所見なし）'
                                    , decode( 
                                        ctjudcd
                                        , '5'
                                        , '正常（判定：C3、レポート判定：C3、所見有り）'
                                        , '確認してください（判定とレポート判定が合いません）'
                                    )
                                ) 
                                , 'C6'
                                , decode( 
                                    result
                                    , '1'
                                    , '確認してください（判定-レポート判定：C6、所見なし）'
                                    , decode( 
                                        ctjudcd
                                        , '6'
                                        , '正常（判定：C6、レポート判定：C6、所見有り）'
                                        , '確認してください（判定とレポート判定が合いません）'
                                    )
                                ) 
                                , 'D1'
                                , decode( 
                                    result
                                    , '1'
                                    , '確認してください（判定-レポート判定：D1、所見なし）'
                                    , decode( 
                                        ctjudcd
                                        , '7'
                                        , '正常（判定：D1、レポート判定：D1、所見有り）'
                                        , '確認してください（判定とレポート判定が合いません）'
                                    )
                                ) 
                                , 'D2'
                                , decode( 
                                    result
                                    , '1'
                                    , '確認してください（判定-レポート判定：D2、所見なし）'
                                    , decode( 
                                        ctjudcd
                                        , '8'
                                        , '正常（判定：D2、レポート判定：D2、所見有り）'
                                        , '確認してください（判定とレポート判定が合いません）'
                                    )
                                ) 
                                , '確認してください'
        ...
        /// <param name="data">成績表CL：上部消化管（内視鏡）CLデータ</param>
