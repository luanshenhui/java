...
namespace Hainsi.Reports
{
    ...
    /// 一括送付案内（英語版）生成クラス
    /// </summary>
    public class Invitation_eCreator : PdfCreator
    {

        /// <summary>
        /// 帳票コード
        /// </summary>
        public override string ReportCd { get; } = "002040";
        ...
        /// 一括送付案内（英語版）データを読み込む
        /// </summary>
        /// <returns>一括送付案内（英語版）データ</returns>
        ...
        protected override void SetFieldValue(CnForms cnForms, List<dynamic> data)
        {
            ...
            var kensa2_50Layer = cnLayers["検査２（５０）"];
            var kensa2_51Layer = cnLayers["検査２（５１）"];
            ...
            var coueseField = (CnDataField)cnObjects["COURSE"];
            ...
            while (dataPos < data.Count)
            {
                ...
                //## 企業健診（午前）（表）
                kensa2_50Layer.VisibleAtPrint = false;
                //## 企業健診（午後）（表）
                kensa2_51Layer.VisibleAtPrint = false;
                //## 企業健診（午前・午後）（裏）
                ...
                if (detail.CSCD == "100" || detail.CSCD == "105")
                {
                    if (ura == "OFF")
                    {
                        ...
                        //終了予定時間
                        tendtimeField.Text = "Ending Time";
                        ...
                        //コース
                        coueseField.Text = Util.ConvertToString(detail.TCSNAME);
                        ...
                        if (ikensaE != null)
                        {
                            ikensaField.Text = Util.ConvertToString(ikensaE.IKENSAE).Trim();
                        }
                        ...
                        if (retTOpt.Count > 0)
                        {
                            ...
                            foreach (var opt in retTOpt)
                            {
                                ...
                                {
                                    optionListField.ListCell(col, currentLine).Text = Util.ConvertToString(opt.FREEFIELD3).Trim();
                                    currentLine++;
                ...
                if (detail.CSCD == "110" || detail.CSCD == "155")
                {
                    if (ura == "OFF")
                    {
                        ...
                        {
                            kensa2_51Layer.VisibleAtPrint = true;
                        }
                        else
                        {
                            kensa2_50Layer.VisibleAtPrint = true;
                        }
