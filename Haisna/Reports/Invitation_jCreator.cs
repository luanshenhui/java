...
namespace Hainsi.Reports
{
    ...
    /// 一括送付案内（日本語版）生成クラス
    /// </summary>
    public class Invitation_jCreator : PdfCreator
    {

        /// <summary>
        /// 帳票コード
        /// </summary>
        public override string ReportCd { get; } = "002030";
        ...
        /// <summary>
        /// 一括送付案内（日本語版）データを読み込む
        /// </summary>
        /// <returns>一括送付案内（日本語版）データ</returns>
        protected override List<dynamic> GetData()
        {
                ...
                select distinct
                    ...
                    , ( 
                        ...
                        where
                            ...
                            and free.freecd like :freecd_oettime 
            ...
            string rsvNo = queryParams["rsvno"];
            string act1 = queryParams["act1"];
            string act2 = queryParams["act2"];
            string addrDiv = queryParams["addrDiv"];
            string engDiv = queryParams["engDiv"];

            //予約番号
            if (! string.IsNullOrEmpty(rsvNo.Trim()))
            {
                sql += @"  and consult.rsvno = :rsvno ";
            }

            //アクション１（保存）
            //予約番号＋アクション１の指定あり
            if ( (!string.IsNullOrEmpty(rsvNo.Trim())) && (!string.IsNullOrEmpty(act1.Trim())) && (string.IsNullOrEmpty(act2.Trim())) )
            {
                sql += @"
                    and 'SAVE_BTN' = :act_1
                    and consult.prtonsave = 2 
                    ";
            }

            //アクション２（印刷）
            if (string.IsNullOrEmpty(act2.Trim()))
            {
                sql += @"  and org.packagesend = 1 ";
            }

            //予約番号＋アクション２の指定あり
            if ((!string.IsNullOrEmpty(rsvNo.Trim())) && (!string.IsNullOrEmpty(act2.Trim())) && (string.IsNullOrEmpty(act1.Trim())))
            {
                sql += @"
                    and 'PRINT_BTN' = :act_2
                    ";
                ...
                //住所編集の指定あり
                if ((!string.IsNullOrEmpty(addrDiv.Trim())))
                {
                    sql += @"  and peraddr3.addrdiv = :addrdiv ";
                }

                //英語編集の指定あり
                if ((!string.IsNullOrEmpty(engDiv.Trim())))
                {
                    sql += @"  and consult.formouteng = :engdiv  ";
                }
            ...
            if ( ( string.IsNullOrEmpty(addrDiv.Trim()) ) )
            {
                sql += @"  and peraddr3.addrdiv = consult.formaddrdiv ";
            }
        ...
        protected override void SetFieldValue(CnForms cnForms, List<dynamic> data)
        {
            ...
            var kensa2Layer = cnLayers["検査２"];
            var ura2Layer = cnLayers["検査２裏"];
            var ura2_50Layer = cnLayers["検査２裏（５０）"];
            var ura2_51Layer = cnLayers["検査２裏（５１）"];
            var ura2_n51Layer = cnLayers["検査２裏（内５１）"];
            var ura2_nLayer = cnLayers["検査２裏（内）"];
            var ura2_nwLayer = cnLayers["検査２裏（内女）"];
            var ura2_wLayer = cnLayers["検査２裏（女）"];
            var ura2_nw50Layer = cnLayers["検査２裏（内女５０）"];
            var ura2_w50Layer = cnLayers["検査２裏（女５０）"];
            var kensa3Layer = cnLayers["検査３"];
            var ura3Layer = cnLayers["検査３裏"];
            var kensa4Layer = cnLayers["検査４"];
            var ura4Layer = cnLayers["検査４裏"];
            var ura4_wLayer = cnLayers["検査４裏（女）"];
            var kensa5Layer = cnLayers["検査５"];
            var kensa6Layer = cnLayers["検査６"];
            var kensa7Layer = cnLayers["検査７"];
            ...
            var tendcome1TextField = (CnTextField)cnObjects["TENDCOME1"];
            var tendcome2TextField = (CnTextField)cnObjects["TENDCOME2"];
            ...
            var t2_comeTextField = (CnTextField)cnObjects["T2-COME"];
            var t2_souhu1Field = (CnDataField)cnObjects["T2-SOUHU1"];
            var t2_souhu2Field = (CnDataField)cnObjects["T2-SOUHU2"];
            var t2_zisan1Field = (CnDataField)cnObjects["T2-ZISAN1"];
            var t2_zisan2Field = (CnDataField)cnObjects["T2-ZISAN2"];
            var t2_zisan3Field = (CnDataField)cnObjects["T2-ZISAN3"];
            var t2_zisan4Field = (CnDataField)cnObjects["T2-ZISAN4"];
            var t2_zisan5Field = (CnDataField)cnObjects["T2-ZISAN5"];
            var t2_zisan6Field = (CnDataField)cnObjects["T2-ZISAN6"];
            var t2_zisan7Field = (CnDataField)cnObjects["T2-ZISAN7"];
            var t2_zisan8Field = (CnDataField)cnObjects["T2-ZISAN8"];
            var t2_zisan9Field = (CnDataField)cnObjects["T2-ZISAN9"];
            var t2_zisan10Field = (CnDataField)cnObjects["T2-ZISAN10"];
            var t2_zisan11Field = (CnDataField)cnObjects["T2-ZISAN11"];
            var t4_benField = (CnDataField)cnObjects["T4-BEN"];
            var t4_etcField = (CnDataField)cnObjects["T4-ETC"];
            ...
            while (dataPos < data.Count)
            {
                ...
                ura2_50Layer.VisibleAtPrint = false;
                ura2_51Layer.VisibleAtPrint = false;
                ura2_n51Layer.VisibleAtPrint = false;
                ura2_nLayer.VisibleAtPrint = false;
                ura2_nwLayer.VisibleAtPrint = false;
                ura2_wLayer.VisibleAtPrint = false;
                ura2_nw50Layer.VisibleAtPrint = false;
                ura2_w50Layer.VisibleAtPrint = false;
                ...
                kensa5Layer.VisibleAtPrint = false;
                kensa6Layer.VisibleAtPrint = false;
                kensa7Layer.VisibleAtPrint = false;
                ...
                if (detail.CSCD == "100" || detail.CSCD == "105")
                {
                    if (ura == "OFF")
                    {
                        ...
                        tendcome1TextField.Text = "（当日の状況により";
                        tendcome2TextField.Text = "　　遅れる可能性があります。）";
                ...
                if (detail.CSCD == "110" || detail.CSCD == "155")
                {
                    if (ura == "OFF")
                    {
                        ...
                        if (retBen != null)
                        {
                            if (retBen.BEN != "")
                            {
                                t2_souhu1Field.Text = "□ 検便容器（緑の袋）";
                                t2_souhu2Field.Text = "□ 検尿容器（茶色の袋）";
                                t2_zisan1Field.Text = "□ 検便容器（緑の袋）：受診日の4日前から2回分の便を";
                                t2_zisan2Field.Text = "　 　　　　　　　　　　採取して下さい。";
                                t2_zisan3Field.Text = "□ 検尿容器（茶色の袋）：受診当日の朝に最初の尿をおとり";
                                t2_zisan4Field.Text = "　 　　　　　　　　　　　下さい。";
                                t2_zisan5Field.Text = "    ※検便・検尿などの検体は、受診日にお預かりします。";
                                t2_zisan6Field.Text = "       後日ご提出いただくことはできません。";
                                t2_zisan7Field.Text = "□ 診察券（IDカード）：当院を受診したことのある方";
                                t2_zisan8Field.Text = "□ 眼鏡・コンタクト（使い捨てレンズをご使用の方は予備も";
                                t2_zisan9Field.Text = "    お持ち下さい）";
                                t2_zisan10Field.Text = "□ 保険証";
                                t2_zisan11Field.Text = "□ 利用券など：ご利用の団体により必要な方";
                            }
                        }
                        else
                        {
                            t2_souhu1Field.Text = "□ 検尿容器（茶色の袋）";
                            t2_zisan1Field.Text = "□ 検尿容器（茶色の袋）：受診当日の朝に最初の尿をおとり";
                            t2_zisan2Field.Text = "　 　　　　　　　　　　　下さい。";
                            t2_zisan3Field.Text = "    ※検尿などの検体は、受診日にお預かりします。";
                            t2_zisan4Field.Text = "       後日ご提出いただくことはできません。";
                            t2_zisan5Field.Text = "□ 診察券（IDカード）：当院を受診したことのある方";
                            t2_zisan6Field.Text = "□ 眼鏡・コンタクト（使い捨てレンズをご使用の方は予備も";
                            t2_zisan7Field.Text = "    お持ち下さい）";
                            t2_zisan8Field.Text = "□ 保険証";
                            t2_zisan9Field.Text = "□ 利用券など：ご利用の団体により必要な方";
                        }

                        //コメント
                        t2_comeTextField.Text = Util.ConvertToString(detail.SENDCOMMENT);
                    ...
                    else
                    {
                        ...
                        {
                            ura2_50Layer.VisibleAtPrint = true;
                            if (detail.CHESTCNT > 0)
                            {
                                ura2_n51Layer.VisibleAtPrint = true;
                                if (Util.ConvertToString(detail.GENDER) == "2")
                                {
                                    ura2_nw50Layer.VisibleAtPrint = true;
                                }
                            }
                            else
                            {
                                if (Util.ConvertToString(detail.GENDER) == "2")
                                {
                                    ura2_w50Layer.VisibleAtPrint = true;
                                }
                            }
                        }
                        ...
                        if (Util.ConvertToString(detail.RSVGRPCD) == "51")
                        {
                            if (detail.CHESTCNT > 0)
                            {
                                ...
                                if (Util.ConvertToString(detail.GENDER) == "2")
                                {
                                    ura2_nwLayer.VisibleAtPrint = true;
                                }
                            }
                            else
                            {
                                ura2_51Layer.VisibleAtPrint = true;
                                if (Util.ConvertToString(detail.GENDER) == "2")
                                {
                                    ura2_wLayer.VisibleAtPrint = true;
                                }
                            }
                        }
                ...
                if (detail.CSCD == "150")
                {
                    if (ura == "OFF")
                    {
                        ...
                        tendcome1TextField.Text = "（当日の状況により";
                        tendcome2TextField.Text = "　　遅れる可能性があります。）";
                        endhField.Text = "15:10";

                        //団体名
                        companyTextField.Text = Util.ConvertToString(detail.TORGNAME);

                        //コース
                        courseField.Text = "肺ドック";
                ...
                if (detail.CSCD == "170")
                {
                    if (ura == "OFF")
                    {
                        ...
                        //送付品
                        ret1 = GetBen2(detail.RSVNO);
                        if ( ret1 > -1 )
                        {
                            if (ret1 != 0 )
                            {
                                t4_benField.Text = "□検便容器セット（検診前日または当日におとり下さい。）";
                                t4_etcField.Text = "□渡航内科問診票（ご記入の上お持ち下さい。）";
                            }
                            else
                            {
                                t4_benField.Text = "□渡航内科問診票（ご記入の上お持ち下さい。）";
                            }
                ...
                if (detail.CSCD == "223")
                {
                    if (ura == "OFF")
                    {
                        //<検査５（表）編集＞
                        //印刷レイヤの設定（ON）
                        kensa5Layer.VisibleAtPrint = true;
                ...
                if (detail.CSCD == "225")
                {
                    if (ura == "OFF")
                    {
                        //<検査７（表）編集＞
                        //印刷レイヤの設定（ON）
                        kensa7Layer.VisibleAtPrint = true;
                ...
                if (detail.CSCD == "133")
                {
                    if (ura == "OFF")
                    {
                        //<検査６（表）編集＞
                        //印刷レイヤの設定（ON）
                        kensa6Layer.VisibleAtPrint = true;
        ...
        private dynamic GetBen2(int rsvNo)
        {
            // SQLステートメント定義
            string sql = @"
                select
                    count(sysdate) as ben2 
                from
            ...
            // 戻り値設定
            return (result == null) ? -1 : (int)result.BEN2;
        }
