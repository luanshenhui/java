...
namespace Hainsi.Reports
{
    ...
    /// 予約確認はがき(日本語)生成クラス
    /// </summary>
    public class ReservePcard_jCreator : PdfCreator
    {

        /// <summary>
        /// 帳票コード
        /// </summary>
        public override string ReportCd { get; } = "002390";
        ...
        /// <summary>
        /// 予約確認はがき(日本語)データを読み込む
        /// </summary>
        /// <returns>予約確認はがき(日本語)データ</returns>
        ...
        protected override void SetFieldValue(CnForms cnForms, List<dynamic> data)
        {
            ...
            var lastknameField = (CnDataField)cnObjects["LASTKNAME"];
            var firstknameField = (CnDataField)cnObjects["FIRSTKNAME"];
            ...
            var jyobiField = (CnDataField)cnObjects["JYOBI"];
            ...
            var freeField2ListField = (CnListField)cnObjects["FREEFIELD2"];
            ...
            foreach ( var detail in data )
            {
                ...
                //カナ姓
                lastknameField.Text = Util.ConvertToString(detail.LASTKNAME).Trim();

                //カナ名
                firstknameField.Text = Util.ConvertToString(detail.FIRSTKNAME).Trim();
                ...
                //曜日
                jyobiField.Text = Util.ConvertToString(detail.JYOBI);
                ...
                //団体名
                orgnameField.Text = Util.ConvertToString(detail.ORGNAME).Trim();

                if (Util.ConvertToString(detail.CSLDIV).Trim() == "")
                {
                    csnameField.Text = Util.ConvertToString(detail.CSNAME).Trim();
                }
                else
                {
                    csnameField.Text = Util.ConvertToString(detail.CSLDIV).Trim();
                }
                ...
                if (retOpt.Count > 0)
                {
                    ...
                    {
                        freeField2ListField.ListCell(col, currentLine).Text = Util.ConvertToString(opt.FREEFIELD2).Trim();
                        currentLine++;

                        if (currentLine > (freeField2ListField.ListRows.Length - 1))
                        {
                            currentLine = 0;
                            col++;
                        }
                ...
                {
                    freeField2ListField.ListCell(0, 0).Text = "";
                    freeField2ListField.ListCell(0, 1).Text = "";
                }
