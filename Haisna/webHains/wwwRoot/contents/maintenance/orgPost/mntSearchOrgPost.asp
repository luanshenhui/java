...
Dim strDispOrgPostCd	'編集用の所属コード
Dim strDispOrgPostName	'編集用の所属名称
Dim strOrgName			'団体名称
...
<FORM NAME="entryForm" ACTION="<%= Request.ServerVariables("SCRIPT_NAME") %>">
	...
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
		<TR>
			<TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="maintenance">■</SPAN><FONT COLOR="#000000">所属の検索</FONT></B></TD>
		</TR>
	</TABLE>
<%
	'団体名称の読み込み
...
<%
	...
	If strOrgRoomCd <> "" Then
		...
		End If
%>
	&nbsp;室部：&nbsp;<FONT COLOR="#ff6600"><B><%= strOrgRoomName %></B></FONT>
<%
	End If
%>
...
<%
			'新規登録時のURLを編集
			strURL = "mntOrgPost.asp"
			strURL = strURL & "?mode="   & "insert"
			strURL = strURL & "&orgCd1=" & strOrgCd1
			strURL = strURL & "&orgCd2=" & strOrgCd2
%>
			<TD><A HREF="<%= strURL %>"><IMG SRC="/webHains/images/newrsv.gif" BORDER="0" WIDTH="77" HEIGHT="24" ALT="新しく所属を登録します"></A></TD>
		</TR>
	</TABLE>
...
<%
	Do
		...
		'検索条件を満たすレコード件数を取得
		lngAllCount = objOrgPost.SelectOrgPostListCount(strOrgCd1, strOrgCd2, strOrgBsdCd, strOrgRoomCd, strArrKey)

		'検索結果が存在しない場合はメッセージを編集
		If lngAllCount = 0 Then
%>
			検索条件を満たす所属情報は存在しません。<BR>
...
<%
		...
		lngCount = objOrgPost.SelectOrgPostList(strOrgCd1,       strOrgCd2,       _
												strOrgBsdCd,     strOrgRoomCd,    _
												strArrKey,       lngStartPos,     _
												lngGetCount,     strArrOrgBsdCd,  _
												strArrOrgRoomCd, strArrOrgPostCd,    _
												strArrOrgPostName,  strArrOrgPostKName, _
												strArrOrgBsdName,   strArrOrgBsdKName,  _
												strArrOrgRoomName,  strArrOrgRoomKName)
...
<%
			For i = 0 To lngCount - 1
				...
				'所属情報の取得
				strDispOrgPostCd   = strArrOrgPostCd(i)
				strDispOrgPostName = strArrOrgPostName(i)

				'検索キーに合致する部分に<B>タグを付加
				If Not IsEmpty(strArrKey) Then
					For j = 0 To UBound(strArrKey)
						strDispOrgPostCd   = Replace(strDispOrgPostCd,   strArrKey(j), "<B>" & strArrKey(j) & "</B>")
						strDispOrgPostName = Replace(strDispOrgPostName, strArrKey(j), "<B>" & strArrKey(j) & "</B>")
					Next
				End If
...
%>
				<TR>
					<TD WIDTH="10"></TD>
					<TD NOWRAP><%= strDispOrgPostCd %></TD>
					<TD WIDTH="10"></TD>
					<TD NOWRAP><A HREF="<%= strURL %>"><%= strDispOrgPostName %></A></TD>
					<TD></TD>
