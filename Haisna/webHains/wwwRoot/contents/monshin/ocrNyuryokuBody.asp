...
' 共通宣言部
'-----------------------------------------------------------------------------
Const GRPCD_OCRNYURYOKU = "X023"	'OCR入力結果確認グループコード
...
Const OCRGRP_START9 = 524	'OCR入力担当者
...
Do
	...
	If strAction = "" Then
		'OCR入力結果を取得する
		lngRslCnt = objRslOcr.SelectOcrNyuryoku( _
							lngRsvNo, _
							GRPCD_OCRNYURYOKU, _
							0, _
							"", _
	...
	Else
		...
		If strAction = "check" Then
			'OCR入力結果の入力チェック
			lngRslCnt = objRslOcr.CheckOcrNyuryoku( _
								lngRsvNo, _
								GRPCD_OCRNYURYOKU, _
								0, _
								"", _
		...
		If strAction = "save" Then
			...
			'検査結果更新
			Ret = objRslOcr.UpdateOcrNyuryoku( _
												lngRsvNo, _
												strIPAddress, _
												strUpdUser, _
												vntItemCd, _
...
// '### 2004/10/22 Updated by FSIT)Gouda 婦人科問診票のNo.4、13、15の「なし」のチェックが他の項目を選択した時に自動的に消える
var lngIndex; 
// '### 2004/10/22 Updated End
...
<% '### 2004/01/19 Start K.Kagawa OCR入力担当者の追加 %>
<!-- 
******************************************************
	OCR入力担当者
******************************************************
...
'### 2004/1/21 Updated by Ishihara@FSIT 文章参照用コードで名称を取得する必要あり(SUFFIXをしばってあまり感じのいいものではないが）
'		objSentence.SelectSentence vntItemCd(lngIndex), 0, vntResult(lngIndex), strOpeNameStc
	...
	End If
%>
<!-- 2004/01/21 Updated by Ishihara@FSIT 更新者用の文章は違う検査項目
			<TD><A HREF="javascript:showUserWindow(<%= lngIndex %>, <%= vntItemCd(lngIndex) %>)"><IMG SRC="../../images/question.gif" ALT="" HEIGHT="21" WIDTH="21" BORDER="0"></A></TD>
-->
