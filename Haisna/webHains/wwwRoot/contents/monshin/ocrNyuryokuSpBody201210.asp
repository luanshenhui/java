...
' 共通宣言部
'-----------------------------------------------------------------------------
Const GRPCD_OCRNYURYOKU2 = "X0342"  'OCR入力結果確認グループコード
Const GRPCD_ALLERGY = "X052"        '薬アレルギーグループコード

'### 取得した検査結果の先頭位置[0～]
Const OCRGRP_START1 =   0   '現病歴既往歴
Const OCRGRP_START2 =  90   '生活習慣問診１
Const OCRGRP_START3 = 137   '生活習慣問診２
Const OCRGRP_START4 = 159   '婦人科問診
Const OCRGRP_START5 = 273   '食習慣問診
Const OCRGRP_START9 = 295   '特定健診
Const OCRGRP_START10 = 297   'OCR入力担当者
Const OCRGRP_START_Z = 298   '前コード用　前回値出力用(一部の項目)
...
Set objRslOcrSp     = Server.CreateObject("HainsRslOcrSp3.OcrNyuryokuSp3")
...
''### 2015.12.12 張 JCI初診時アセスメントコメント追加の為（OCR問診結果登録有無確認） START ####################################
...
''### 2015.12.12 張 JCI初診時アセスメントコメント追加の為（OCR問診結果登録有無確認） END   ####################################
...
<% ''### 2015.12.12 張 JCI初診時アセスメントコメント追加の時タイトル整理（修正） START ##################################### %>
        <!--TR HEIGHT="20">
        ...
        <TR HEIGHT="20">
            <TD NOWRAP WIDTH="20"></TD>
            <TD NOWRAP COLSPAN="2" BGCOLOR="#98fb98" WIDTH="730"><SPAN ID="Anchor-DiseaseHistory" STYLE="position:relative"><B>今回値&nbsp;<%= IIf(vntCslDate(0)="", "", "(" & vntCslDate(0) & ")") %></B></SPAN></TD>
            <TD NOWRAP BGCOLOR="#98fb98" WIDTH="220"><B>前回値&nbsp;<%= IIf(vntLstCslDate(0)="", "", "(" & vntLstCslDate(0) & ")") %></B></TD>
        </TR>
<% ''### 2015.12.12 張 JCI初診時アセスメントコメント追加の時タイトル整理（修正） END   ##################################### %>

        <TR HEIGHT="20">
            <TD NOWRAP></TD>
            <TD NOWRAP WIDTH="330" BGCOLOR="#eeeeee">センター使用欄</TD>
            <TD NOWRAP ROWSPAN="6" WIDTH="400" VALIGN="TOP">

<% ''### 2015.12.12 張 JCI初診時アセスメントコメント追加（OCR問診結果登録後表示） START #################################### %>
                    ...
                    <TR>
                        <TD BGCOLOR="#ffffff" VALIGN="TOP">
                            <TABLE WIDTH="100%" BORDER="0" CELLSPACING="2" CELLPADDING="1" BGCOLOR="#ffffff">
                                ...
                                <TR ALIGN="CENTER">
                                    <TD><FONT STYLE="font-size:11pt;"><B>初診時アセスメント実施済み</B></FONT></TD>
                                </TR>
                                <TR ALIGN="CENTER"><TD HEIGHT="10"></TD></TR>
                                <TR ALIGN="CENTER">
                                    <TD>
                                        <FONT STYLE="font-size:10pt;">（疼痛、食品アレルギー、栄養、価値観・宗教的制約、感情、コミュニケーションの問題、認知機能、１か月以内の転倒経験、歩行障害・歩行器具、杖・車いす使用、めまい・立ちくらみ）</FONT>
                                    </TD>
                                </TR>
                                <TR ALIGN="CENTER"><TD HEIGHT="10"></TD></TR>
                                <TR ALIGN="CENTER">
                                    <TD><FONT STYLE="font-size:11pt;"><B>詳細とプランは注意事項に記載</B></FONT></TD>
                                </TR>
                                <TR ALIGN="CENTER"><TD HEIGHT="10"></TD></TR>
                            </TABLE>
                        </TD>
...
<% End If %>
<% ''### 2015.12.12 張 JCI初診時アセスメントコメント追加（OCR問診結果登録後表示） END   #################################### %>
...
%>
        ...
        <TR HEIGHT="20">
            ...
            <TD NOWRAP BGCOLOR="#eeeeee">ブチルスコポラミン可否</TD>
            <TD NOWRAP BGCOLOR="#eeeeee"></TD>
        </TR>
<%
    strHTML = ""
...
<% ''### 2015.12.12 張 JCI初診時アセスメントコメント追加の時タイトル整理（追加） START ##################################### %>
    </TABLE>

    <TABLE BORDER="0" CELLSPACING="2" CELLPADDING="0">
        <TR HEIGHT="20">
            <TD NOWRAP WIDTH="20"></TD>
            <TD NOWRAP BGCOLOR="#98fb98" WIDTH="730">現病歴・既往歴問診票</TD>
            <TD NOWRAP BGCOLOR="#98fb98" WIDTH="220"></TD>
        </TR>
<% ''### 2015.12.12 張 JCI初診時アセスメントコメント追加の時タイトル整理（追加） START ##################################### %>
...
<%
                    ...
                    For i=0 To JIKAKUSHOUJYOU_COUNT - 1
                        ...
                        For j=0 To UBound(strArrCodeJikaku1)
                            ...
                            End If
                        Next
                        lngIndex = OCRGRP_START2 + 24 + i*4
                        If vntLstResult(lngIndex) <> "" Then
                            strLstRsl =  strLstRsl & IIf(strLstRsl="", "", "　") & vntLstResult(lngIndex)
                        End If
                        lngIndex = OCRGRP_START2 + 25 + i*4
...
%>
        ...
        <TR HEIGHT="20">
            <TD NOWRAP></TD>
            <TD NOWRAP BGCOLOR="#eeeeee">２．最近１ヶ月の状態</TD>
            <TD NOWRAP BGCOLOR="#eeeeee"></TD>
        </TR>
        <TR HEIGHT="20">
<%
            ...
            strHTML = strHTML & EditRsl(lngIndex, "checkbox", "chk3_2_1", , "1") & "<B>本人希望により未回答</B>" & vbLf
            strHTML = strHTML & "</TD>" & vbLf
            '---前回値---
            strLstRsl = IIf(vntLstResult(lngIndex)="", "", "本人希望により未回答")
            strHTML = strHTML & "<TD NOWRAP>" & strLstRsl & "</TD>" & vbLf
            ...
            <TD NOWRAP>
                <TABLE BORDER="0" CELLSPACING="2" CELLPADDING="1">
                    <TR HEIGHT="28">
                        ...
                        <TD NOWRAP ALIGN="center" width="120">ほとんどなかった</TD>
                        <TD NOWRAP ALIGN="center" width="120">ときどきあった</TD>
                        <TD NOWRAP ALIGN="center" width="120">しばしばあった</TD>
                        <TD NOWRAP ALIGN="center" width="120">ほとんどいつもあった</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">1)ひどく疲れた</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_1", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_1", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_1", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_1", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">2)へとへとだ</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_2", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_2", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_2", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_2", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">3)だるい</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_3", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_3", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_3", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_3", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">4)気がはりつめている</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_4", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_4", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_4", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_4", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">5)不安だ</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_5", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_5", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_5", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_5", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">6)落ち着かない</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_6", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_6", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_6", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_6", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">7)ゆううつだ</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_7", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_7", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_7", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_7", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">8)何をするのも面倒だ</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_8", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_8", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_8", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_8", , "4") %></TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP BGCOLOR="#eeeeee">9)気分が晴れない</TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_9", , "1") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_9", , "2") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_9", , "3") %></TD>
                        <TD NOWRAP ALIGN="center"><%= EditRsl(lngIndex, "radio", "opt3_2_9", , "4") %></TD>
                    </TR>
                </TABLE>
            </TD>
            <TD NOWRAP>
...
%>
        ...
        <TR HEIGHT="20">
            <TD NOWRAP></TD>
            <TD NOWRAP BGCOLOR="#eeeeee">１．食べ方について</TD>
            <TD NOWRAP WIDTH="220" BGCOLOR="#eeeeee"></TD>
        </TR>
        <TR>
            <TD NOWRAP>
                <TABLE BORDER="0" CELLSPACING="2" CELLPADDING="0">
...
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP WIDTH="330">1)食事を食べる速さはいかがですか</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_1", , "1") %>速い</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_1", , "2") %>やや速い</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_1", , "3") %>普通</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_1", , "4") %>ゆっくり</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>2)満腹まで食べることがありますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_2", , "1") %>よくある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_2", , "2") %>たまにある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_2", , "3") %>ほとんどない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>3)栄養のバランスを考えていますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_3", , "1") %>考えている</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_3", , "2") %>時々考えている</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_3", , "3") %>それほどでもない</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_3", , "4") %>全く考えない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>4)味付けは濃い方ですか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_4", , "1") %>濃い</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_4", , "2") %>やや濃い</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_4", , "3") %>やや薄い</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_4", , "4") %>薄い</TD>
                    </TR>
                </TABLE>
            </TD>
            <TD>
...
%>
        ...
        <TR HEIGHT="20">
            <TD NOWRAP></TD>
            <TD NOWRAP BGCOLOR="#eeeeee">２．食習慣について</TD>
            <TD NOWRAP WIDTH="220" BGCOLOR="#eeeeee"></TD>
        </TR>
        <TR>
            <TD NOWRAP>
...
%>
                    <TR HEIGHT="28">
                        <% '### 2013/03/01 張 問診票文言変更に伴う画面変更 START ################################################################### %>
                        <% '###変更前：1)１日３食食べていますか。欠食がある日はありますか %>
                        <TD NOWRAP>1)１日三食のうち、欠食がある日は週に何日ありますか</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_5", , "1") %><%'###変更前：毎日ある%>週５日以上ある</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_5", , "2") %><%'###変更前：週３日以上ある%>週３～４日ある</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_5", , "3") %>週１～２日ある</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_5", , "4") %>ほとんどない</TD>
                        <% '### 2013/03/01 張 問診票文言変更に伴う画面変更 END   ################################################################### %>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>2)食事時刻は規則的ですか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_6", , "1") %>規則的</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_6", , "2") %>ほぼ規則的</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_6", , "3") %>やや不規則</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_6", , "4") %>不規則</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>3)間食をとることはありますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_7", , "1") %>ほぼ毎日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_7", , "2") %>時々ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_7", , "3") %>ほとんどない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>4)夕食が外食となる日は、週に何日位ありますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_8", , "1") %>週５日以上ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_8", , "2") %>週３～４日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_8", , "3") %>週１～２日ある</TD>
                        <% '### 2013/03/01 張 問診票文言変更に伴う画面変更 START ################################################################### %>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_8", , "4") %><%'###変更前：月３日以内ある%>ほとんどない</TD>
                        <% '### 2013/03/01 張 問診票文言変更に伴う画面変更 END   ################################################################### %>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>5)夕食をとる時間が午後９時以降になる日は週に何日ありますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_9", , "1") %>週５日以上ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_9", , "2") %>週３～４日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_9", , "3") %>週１～２日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_9", , "4") %>ほとんどない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>6)夕食後に飲食をとることは、週に何日くらいありますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_10", , "1") %>週５日以上ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_10", , "2") %>週３～４日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_10", , "3") %>週１～２日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_10", , "4") %>ほとんどない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP WIDTH="330">7)お菓子を食べたり、糖分の入った飲み物を飲む</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_11", , "1") %>週５日以上ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_11", , "2") %>週３～４日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_11", , "3") %>週１～２日ある</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_11", , "4") %>ほとんどない</TD>
                    </TR>
                </TABLE>
            </TD>
            <TD>
...
%>
        ...
        <TR HEIGHT="20">
            <TD NOWRAP></TD>
            <TD NOWRAP BGCOLOR="#eeeeee">３．食事内容について</TD>
            <TD NOWRAP WIDTH="220" BGCOLOR="#eeeeee"></TD>
        </TR>
        <TR>
            <TD NOWRAP>
                <TABLE BORDER="0" CELLSPACING="2" CELLPADDING="0">
                    <TR HEIGHT="28">
                        <TD NOWRAP WIDTH="20"><%= EditErrInfo %></TD>
                    </TR>
                    <TR HEIGHT="28">
                        <TD NOWRAP WIDTH="20"><%= EditErrInfo %></TD>
                    </TR>
...
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP WIDTH="330">1)脂肪分の多い食事を食べますか</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_12", , "1") %>よく食べる</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_12", , "2") %>時々食べる</TD>
                        <TD NOWRAP WIDTH="150"><%= EditRsl(lngIndex, "radio", "opt5_12", , "3") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>2)主食、芋類などを毎食食べますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_13", , "1") %>毎食食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_13", , "2") %>１日２食食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_13", , "3") %>１日１食食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_13", , "4") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>3)野菜をよく食べますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_14", , "1") %>毎食食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_14", , "2") %>１日２食は食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_14", , "3") %>１日１食は食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_14", , "4") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>4)野菜の量はいかがですか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_15", , "1") %>１日７皿以上</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_15", , "2") %>１日５～６皿</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_15", , "3") %>１日３～４皿</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_15", , "4") %>１日１～２皿</TD>
                    </TR>
                    <TR HEIGHT="28">
                        <TD NOWRAP>&nbsp;</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_15", , "5") %>毎日は食べない</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_15", , "6") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>5)果物をよく食べますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_16", , "1") %>１日３皿以上</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_16", , "2") %>１日２皿以上</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_16", , "3") %>１日１皿</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_16", , "4") %>週に３～４皿</TD>
                    </TR>
                    <TR HEIGHT="28">
                        <TD NOWRAP>&nbsp;</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_16", , "5") %>週に１～２皿</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_16", , "6") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>6)乳製品をよく食べますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_17", , "1") %>１日１．５杯以上</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_17", , "2") %>１日１杯</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_17", , "3") %>週に３～４杯</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_17", , "4") %>週に１～２杯</TD>
                    </TR>
                    <TR HEIGHT="28">
                        <TD NOWRAP>&nbsp;</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_17", , "5") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>7)大豆製品をよく食べますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_18", , "1") %>週５日以上食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_18", , "2") %>週３～４日食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_18", , "3") %>週１～２日食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_18", , "4") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>8)魚介類をよく食べますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_19", , "1") %>週５日以上食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_19", , "2") %>週３～４日食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_19", , "3") %>週１～２日食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_19", , "4") %>ほとんど食べない</TD>
                    </TR>
<%
                    lngIndex = lngIndex + 1
%>
                    <TR HEIGHT="28">
                        <TD NOWRAP>9)肉類、卵などをよく食べますか</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_20", , "1") %>週５日以上食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_20", , "2") %>週３～４日食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_20", , "3") %>週１～２日食べる</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_20", , "4") %>ほとんど食べない</TD>
                    </TR>
                </TABLE>
            </TD>
            <TD>
...
%>
        ...
        <TR HEIGHT="20">
            <TD NOWRAP></TD>
            <TD NOWRAP BGCOLOR="#eeeeee">４．その他の質問</TD>
            <TD NOWRAP WIDTH="220" BGCOLOR="#eeeeee"></TD>
        </TR>
        <TR>
            <TD NOWRAP>
...
%>
                    <TR HEIGHT="28">
                        ...
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_21", , "1") %>はい</TD>
                        <TD NOWRAP><%= EditRsl(lngIndex, "radio", "opt5_21", , "2") %>いいえ</TD>
                    </TR>
                </TABLE>
            </TD>
            <TD>
...
window.onload = function()
{
    ...
    if( document.getElementById ) {
        document.getElementById('LoadindMessage').style.display = 'none';
    }
