...
Dim lngRecogLevel		'認識レベル(0:面接未実施、1:レベル１、2:レベル２、3:レベル３、4:レベル４、5:レベル５)
'### 2004/11/12 Add End
...
'### 2004/11/12 Add by Gouda@FSIT 生活指導コメントの表示分類
lngRecogLevel = Request("recogLevel")
'### 2004/11/12 Add End
...
// ### 2004/11/16 Add by Gouda@FSIT 生活指導コメントの表示分類 
	// judCmtCdが配列かどうか？
	// 配列の場合
	if ( document.kensakulist.judCmtCd.length > 0 ) {
// ### 2004/11/16 Add End 
...
// ### 2004/11/16 Add by Gouda@FSIT 生活指導コメントの表示分類
		...
		for( kcnt = 0; kcnt < document.entryForm.selCmtCnt.value; kcnt++ ) {
			...
			if ( ( kcnt + 1 ) < document.entryForm.selCmtCnt.value ){
				continue;
			}
			if ( document.kensakulist.judCmtCd.checked ) {
				opener.cmtGuide_varSelCmtCd[jcnt] = document.kensakulist.judCmtCd.value;
				opener.cmtGuide_varSelCmtStc[jcnt] = document.kensakulist.judCmtStc.value;
				opener.cmtGuide_varSelClassCd[jcnt] = document.kensakulist.judClassCd.value;
			}
		}
	}
// ### 2004/11/16 Add End
...
%>
	...
	<TABLE BORDER="0" CELLPADDING="2" CELLPADDING="2" WIDTH="100%">
		<TR>
<!-- ### 2004/11/12 Add by Gouda@FSIT 生活指導コメントの表示分類 -->
<!--		<TD HEIGHT="40" NOWRAP><B><%= strJudClassName %>コメント</B></TD> -->
<% 
		If IsNumeric(Trim(lngRecogLevel)) Then
			If Trim(lngRecogLevel) > 0 Then
%>	
				<TD HEIGHT="30" NOWRAP><B><%= strJudClassName %>コメント　　～<%= "&nbsp;" %>認識レベル<%= lngRecogLevel %><%= "&nbsp;" %>～</B></TD>
<%
			Else 
%>
...
'### 2004/11/12 Add by Gouda@FSIT 生活指導コメントの表示分類
'		lngCount = objJudCmtStc.SelectJudCmtStcList(strJudClassCd, strArrKey, lngStartPos, lngGetCount, strArrJudCmtCd, strArrJudCmtStc, strArrJudClassCd, strArrJudClassName, , , 1)
