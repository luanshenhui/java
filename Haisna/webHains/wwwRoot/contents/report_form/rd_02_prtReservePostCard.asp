...
<!--
   function LoadForm() {
      ...
      Rdviewer.FileOpen("http://157.104.16.194:8090/RDServer/form/<%= strUrl %>", "/rp [<%= l_rsvNo %>] [<%= l_clsDateFrom %>] [<%= l_clsDateTo %>] [<%= l_orgCD %>]  [<%= l_csCD %>] [<%= l_perID %>] [<%= l_act %>] [<%= l_addrdiv %>] <%= strWhere %> /rpsrc [3] /rpapersize [43]");
