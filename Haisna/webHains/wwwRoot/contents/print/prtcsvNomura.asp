...
'◆ 団体コード
    If IsEmpty(Request("OrgCd11")) Then
        'strOrgCd11	= "25006"
        'strOrgCd12	= "00000"
        'strOrgName1 = "野村證券健保"
    Else
...
''''''''オブジェクトのインスタンス作成（プロジェクト名.クラス名）
        Set objPrintCls = Server.CreateObject("HainsAbsenceListNomura.AbsenceListSTD")
''''''''プロジェクト名はソースを開いて確認。

''''''''団体一覧表ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
''''''''## 2005.12.09 張 抽出条件に受診区分と請求書出力区分を追加した為修正 ##
        'Ret = objPrintCls.PrintAbsenceListSTD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgGrpCd, strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52)

        Ret = objPrintCls.PrintAbsenceListSTD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgGrpCd, strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52, strCslDivCd, strBillPrint)
...
<TITLE>標準（野村證券形式）フォーマット CSV出力</TITLE>
...
<!--- タイトル -->
    <TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
        <TR>
            <TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■標準（野村證券形式）フォーマット</SPAN></B></TD>
        </TR>
    </TABLE>
<%
'エラーメッセージ表示
