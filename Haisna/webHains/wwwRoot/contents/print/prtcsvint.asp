...
'''''''''''''''''		'オブジェクトのインスタンス作成（プロジェクト名.クラス名）
		Set objPrintCls = Server.CreateObject("HainsAbsenceListint.AbsenceListint")
'''''''''''''''''			'プロジェクト名はソースを開いて確認。
'''''''''''''''''
'''''''''''''''''		'団体一覧表ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
         Ret = objPrintCls.PrintAbsenceListint(Session("USERID") ,cdate(strSCslDate), cdate(strECslDate))
...
<!--- タイトル -->
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
		<TR>
			<TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■</SPAN>インテラセット提出用CSV出力</SPAN></B></TD>
		</TR>
	</TABLE>
<%
'エラーメッセージ表示
