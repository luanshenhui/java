...
Function Print()
	...
	If Not IsArray(CheckValue()) Then
        ...
        '情報漏えい対策用ログ書き出し
        Call putPrivacyInfoLog("PH017", "ご案内書送付チェックリストの印刷を行った")
...
'#### 2010.05.12 SL-UI-Y0101-202 MOD START ####'
'		strURL = "/webHains/contents/report_form/rd_06_prtInstructionList.asp"
...
'		strURL = strURL & "&p_strOrgCd1=" & strOrgCd1 
'		strURL = strURL & "&p_strOrgCd2=" & strOrgCd2 
'		Response.Redirect strURL
'		Response.End

        'オブジェクトのインスタンス作成（プロジェクト名.クラス名）
        Set objPrintCls = Server.CreateObject("HainsprtInstructionList.prtInstructList")
        'ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
        Ret = objPrintCls.PrintOut(Session("USERID"), strSCslDate, strECslDate, strCsCd, strOrgCd1, strOrgCd2)
        Print = Ret
'#### 2010.05.12 SL-UI-Y0101-202 MOD END ####'
...
<BODY ONLOAD="javascript:setElement();checkRunState();">
...
<!--- タイトル -->
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
		<TR>
			<TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■</SPAN>ご案内書送付チェックリスト</SPAN></B></TD>
		</TR>
	</TABLE>
<%
'エラーメッセージ表示
