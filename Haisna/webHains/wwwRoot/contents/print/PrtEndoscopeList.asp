...
Dim objCommon
	If Not IsArray(CheckValue()) Then
		...
		'情報漏えい対策用ログ書き出し
		Call putPrivacyInfoLog("PH023", "内視鏡受付一覧の印刷を行った")
...
'		Print = Ret
Set objCommon = Server.CreateObject("HainsCommon.Common")
'#### 2010.05.27 SL-UI-Y0101-208 MOD START ####'
'strURL = "/webHains/contents/report_form/rd_14_PrtEndoscopeList.asp"
...
'Response.End
        ...
        'オブジェクトのインスタンス作成（プロジェクト名.クラス名）
        Set objPrintCls = Server.CreateObject("HainsprtEndoscopeList.prtEndoscopeList")
        'ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
        Ret = objPrintCls.PrintOut(UID, strSCslDate, strECslDate)
        Print = Ret
'#### 2010.05.27 SL-UI-Y0101-208 MOD END ####'
	End If
...
<!--- タイトル -->
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
		<TR>
			<TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■内視鏡受付一覧</SPAN></B></TD>
		</TR>
	</TABLE>
<%
'エラーメッセージ表示
