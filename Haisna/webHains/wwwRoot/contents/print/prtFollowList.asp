...
<%
'-------------------------------------------------------------------------------
'   フォローアップ対象者リスト (Ver0.0.1)
'   AUTHER  :
'-------------------------------------------------------------------------------
'       管理番号：SL-UI-Y0101-216
'       修正日  ：2010.04.26
'       担当者  ：ASC)福地
...
'#### 2010.04.26 SL-UI-Y0101-216 ADD START ####'
	Dim objPrintCls	'COMコンポーネント
	Dim Ret			'関数戻り値
'#### 2010.04.26 SL-UI-Y0101-216 ADD END ####'
    Dim objCommon
    If Not IsArray(CheckValue()) Then

		'情報漏えい対策用ログ書き出し
		Call putPrivacyInfoLog("PH032", "フォローアップ対象者リストの印刷を行った")

'#### 2010.04.26 SL-UI-Y0101-216 MOD START ####'
'        Set objCommon = Server.CreateObject("HainsCommon.Common")
'        strURL = "/webHains/contents/report_form/rd_39_prtFollowList.asp"
...
'        Response.End
		...
		Set objPrintCls = Server.CreateObject("HainsprtFollowList.prtFollowList")
		'ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
		Ret = objPrintCls.PrintOut(UID, _
								   strSSendDate)
		Print = Ret

'#### 2010.04.26 SL-UI-Y0101-216 MOD END ####'
...
<!--- タイトル -->
    <TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
        <TR>
            <TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■</SPAN><FONT COLOR="#000000">フォローアップ対象者リスト</FONT></B></TD>
        </TR>
    </TABLE>
<%
'エラーメッセージ表示
