...
Dim strObject								'出力対象
'Dim strInsurance							'健保 0:なし 1:あり
Dim strReturn								'改ページ条件 0:なし 1:室部 2:所属
...
Function Print()
	...
	If Not IsArray(CheckValue()) Then
		...
		'情報漏えい対策用ログ書き出し
		Call putPrivacyInfoLog("PH033", "ワークシート前回値参照（血清） の印刷を行った")

'#### 2010.05.26 SL-UI-Y0101-217 MOD START ####'
''■■■■■■■■■■ 画面項目にあわせて編集
''		'オブジェクトのインスタンス作成（プロジェクト名.クラス名）
''		Set objPrintCls = Server.CreateObject("HainsIntendedList.IntendedList")
''		'プロジェクト名はソースを開いて確認。
'		'団体一覧表ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
''		Ret = objPrintCls.PrintFirstIntendedList(Session("USERID"), , strSCslDate, strECslDate, strCsCd, strOrgCd1,_
''											strOrgCd2, strOrgBsdCd, strOrgRoomCd, strSOrgPostCd, strEOrgPostCd,_
''											strReturn)
''■■■■■■■■■■
''		Print = Ret
'Set objCommon = Server.CreateObject("HainsCommon.Common")
'strURL = "/webHains/contents/report_form/rd_11_prtWorkSheetLast.asp"
...
'strURL = strURL & "&p_Gcd=" & "X501"
'Response.Redirect strURL
'Response.End

		'オブジェクトのインスタンス作成（プロジェクト名.クラス名）
		Set objPrintCls = Server.CreateObject("HainsprtWorkSheetLast.prtWorkSheetLast")
		'ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
		Ret = objPrintCls.PrintOut(UID, strSCslDate, strECslDate)

		Print = Ret

'#### 2010.05.26 SL-UI-Y0101-217 MOD END ####'
...
<!--- タイトル -->
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
		<TR>
			<TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■</SPAN>ワークシート前回値参照</B></TD>
		</TR>
	</TABLE>
<%
'エラーメッセージ表示
...
<!--- コース -->
<!-- ##2004/1/3 DEL グループ削除 (NSC)birukawa
	<TABLE BORDER="0" CELLPADDING="1" CELLSPACING="2">
		<TR>
			<TD>□</TD>
			<TD WIDTH="90" NOWRAP>グループ</TD>
			<TD>：</TD>
