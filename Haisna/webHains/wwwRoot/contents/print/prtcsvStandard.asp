...
'''''''''''''''''		'オブジェクトのインスタンス作成（プロジェクト名.クラス名）
		'Set objPrintCls = Server.CreateObject("HainsAbsenceListlife1.AbsenceListlife1")
		Set objPrintCls = Server.CreateObject("HainsAbsenceListSTD.AbsenceListSTD")
'''''''''''''''''			'プロジェクト名はソースを開いて確認。
'''''''''''''''''
'''''''''''''''''		'団体一覧表ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
       'Ret = objPrintCls.PrintAbsenceListlife1(Session("USERID") ,cdate(strSCslDate), 'cdate(strECslDate))



		Ret = objPrintCls.PrintAbsenceListSTD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgCd11, strOrgCd12, strOrgCd21, strOrgCd21, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52)
...
<!--- タイトル -->
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
		<TR>
			<TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■標準 フォーマット</SPAN></B></TD>
		</TR>
	</TABLE>
<%
'エラーメッセージ表示
