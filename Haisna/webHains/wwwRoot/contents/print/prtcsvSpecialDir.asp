...
'### 2012/10/31 張 2012年10月1日からの問診票変更に伴う項目区分設定（2012年10月1日基準）Start ###
    If CDate(GetVer201210()) <= CDate(strSCslDate)  Then
        ...
        End If
    End If
'### 2012/10/31 張 2012年10月1日からの問診票変更に伴う項目区分設定（2012年10月1日基準）End   ###
...
Function Print()
    ...
    If Not IsArray(CheckValue()) Then
        ...
        '情報漏えい対策用ログ書き出し
        Call putPrivacyInfoLog("PH111", "特定健診項目健診結果CSV出力（大和総研形式）②よりファイル出力を行った")

''''''''''''''''''■■■■■■■■■■ 画面項目にあわせて編集
'''''''''''''''''       'オブジェクトのインスタンス作成（プロジェクト名.クラス名）
        Set objPrintCls = Server.CreateObject("HainsAbsenceListSPD.AbsenceListSPD")
'''''''''''''''''       'プロジェクト名はソースを開いて確認。
'''''''''''''''''
'''''''''''''''''       '団体一覧表ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
''   Ret = objPrintCls.PrintAbsenceListSPD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgGrpCd, strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52)

'''''''''''''''''       '2009.03.03 (李) 健診コース区分、出力項目区分パラメーター追加       
''   Ret = objPrintCls.PrintAbsenceListSPD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgGrpCd, strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52, strCourseCd, strSpItem)

'''''''''''''''''       '2009.06.01 (李) 団体請求対象のみ出力するためパラメーター追加

'### 2013.02.06 張 抽出条件に受診区分追加の為修正 START ###

'   Ret = objPrintCls.PrintAbsenceListSPD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgGrpCd, strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52, strCourseCd, strSpItem, strOrgBill, strBillOrgCd1, strBillOrgCd2)

'### 2017/02/03 張 出力条件に請求書区分（strBillPrint）追加の為パラメータ追加 ###
'   Ret = objPrintCls.PrintAbsenceListSPD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgGrpCd, strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52, strCourseCd, strSpItem, strCslDivCd, strOrgBill, strBillOrgCd1, strBillOrgCd2)
   Ret = objPrintCls.PrintAbsenceListSPD(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgGrpCd, strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52, strCourseCd, strSpItem, strCslDivCd, strBillPrint, strOrgBill, strBillOrgCd1, strBillOrgCd2)

'### 2013.02.06 張 抽出条件に受診区分追加の為修正 END   ###
...
<!--- タイトル -->
    <TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
        <TR>
            <TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■特定健診結果CSV出力（大和総研） </SPAN></B></TD>
        </TR>
    </TABLE>
<%
'エラーメッセージ表示
        ...
        <TR>
            ...
            <TD>
                <SELECT NAME="SpItem">
                    ...
                    <% '### 2012/10/31 張 XML変換用データ抽出機能に合わせて修正 Start ### %>
                    <OPTION VALUE="0">特定健診項目</OPTION>
                    <!--OPTION VALUE="1">特定健診項目</OPTION-->
                    <% '### 2012/10/31 張 XML変換用データ抽出機能に合わせて修正 End   ### %>
                </SELECT>
            </TD>
        </TR>
    </TABLE>
...
<% '### 2013.02.06 抽出条件に受診区分追加の為追加 START %>
    <!-- 請求書出力区分 -->
    <BR>
    <TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1">
        <TR>
...
<% '### 2013.02.06 抽出条件に受診区分追加の為追加 END   %>
