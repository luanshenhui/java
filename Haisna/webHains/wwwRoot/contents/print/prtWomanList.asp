...
'#### 2010.04.26 SL-UI-Y0101-214 ADD START ####'
	Dim objPrintCls	'COMコンポーネント
	Dim Ret			'関数戻り値
'#### 2010.04.26 SL-UI-Y0101-214 ADD END ####'
    Dim objCommon
    If Not IsArray(CheckValue()) Then

		'情報漏えい対策用ログ書き出し
		Call putPrivacyInfoLog("PH030", "女性受診者リストの印刷を行った")

'#### 2010.04.26 SL-UI-Y0101-214 MOD START ####'
'        Set objCommon = Server.CreateObject("HainsCommon.Common")
'        strURL = "/webHains/contents/report_form/rd_36_prtWomanList.asp"
...
'        Response.End
		...
		Set objPrintCls = Server.CreateObject("HainsprtWomanList.prtWomanList")
		'ドキュメントファイル作成処理（オブジェクト.メソッド名(引数)）
		Ret = objPrintCls.PrintOut(UID, _
								   strSSendDate)
		Print = Ret

'#### 2010.04.26 SL-UI-Y0101-214 MOD END ####'
...
<!--- タイトル -->
    <TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
        <TR>
            <TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■</SPAN><FONT COLOR="#000000">女性受診者リスト</FONT></B></TD>
        </TR>
    </TABLE>
<%
'エラーメッセージ表示
