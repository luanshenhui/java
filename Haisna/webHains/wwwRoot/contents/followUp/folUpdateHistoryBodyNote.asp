...
'フォローノート用
Dim vntNUpdDate            '更新日時
Dim vntNUpdUser            '更新者
Dim vntNUpdUserName        '更新者氏名
Dim vntNUpdDiv             '処理区分
Dim vntNRsvNo              '予約番号
Dim vntNJudClassCd         '判定分類コード
Dim vntNJudClassName       '判定分類名称
Dim vntNBefore       '更新前値
...
'End If
		...
		If strFollowNoteFlg = "1" Then
			' ## 検索条件に予約番号(lngRsvNo)追加 2004.01.07
			lngNoteCount = objFollowUp.SelectFollowNoteLogList( _
								...
								lngRsvNo, _
			        				vntNUpdDate, _
			        				vntNUpdUser,        vntNUpdUserName, _
			        				vntNUpdDiv, _
			        				vntNRsvNo, _
			        				vntNJudClassCd,     vntNJudClassName, _
			        				vntNBefore)
		End If
	End If
...
<%
	IF strAct <> "" Then
	If strFollowNoteFlg = "1" Then
	If lngNoteCount <= 0 Then
%>
		検索条件を満たす履歴は存在しませんでした。<BR>
<%
	Else
%>
		検索結果は <FONT COLOR="#ff6600"><B><%= lngNoteCount %></B></FONT>件ありました。
		<TABLE BORDER="0" CELLSPACING="2" CELLPADDING="2">
			<TR BGCOLOR="#cccccc">
				...
				<TD NOWRAP>判定名称</TD>
				<TD NOWRAP>処理</TD>
				<TD WIDTH="400" NOWRAP>更新前</TD>
			</TR>
<%
			For i = 0 To UBound(vntNUpdDate)
				'更新分類名称セット
				strUpdClassName = "ノート"

				'項目名称セット
				strItemName = vntNJudClassName(i)

				'処理区分名称セット
				Select Case vntNUpdDiv(i)
					Case "I"
						strUpdDivName = "挿入"
					Case "U"
						strUpdDivName = "更新"
...
%>
				<TR VALIGN="top">
					<TD NOWRAP BGCOLOR="#eeeeee"><%= vntNUpdDate(i) %></TD>
					<TD NOWRAP BGCOLOR="#eeeeee"><%= vntNUpdUserNAME(i) %></TD>
					...
					<TD NOWRAP BGCOLOR="#eeeeee" WIDTH="400"><%= vntNBefore(i) %></TD>
				</TR>
<%
			Next
%>
