...
'データベースアクセス用オブジェクト
Dim objCommon           '共通クラス
Dim objWebOrgRsv        'web予約情報(団体向け)アクセス用
...
'#### 2010.06.23 SL-UI-Y0101-108 ADD START ####'
Dim lngMosFlg		'申込区分(0:指定なし、1:新規、2:キャンセル)
'#### 2010.06.23 SL-UI-Y0101-108 ADD END ####'
...
Do
    ...
    'web予約の検索
'#### 2010.08.07 SL-UI-Y0101-108 MOD START #### 
''#### 2010.06.23 SL-UI-Y0101-108 MOD START ####'
''    lngCount = objWebOrgRsv.SelectWebOrgRsvList( _
''        dtmStrCslDate, _
''        dtmEndCslDate, _
''        strKey,        _
''        dtmStrOpDate,  _
...
'        strRsvNo       _
'    )
''#### 2010.06.23 SL-UI-Y0101-108 MOD END ####'
    lngCount = objWebOrgRsv.SelectWebOrgRsvList( _
        dtmStrCslDate, _
        dtmEndCslDate, _
        strKey,        _
...
'#### 2010.08.07 SL-UI-Y0101-108 MOD END ####'
...
// web予約情報登録画面呼び出し
function callWebOrgRsvWindow( cslDate, webNo ) {
    var opened = false;     // 画面が開かれているか

    // すでにガイドが開かれているかチェック
    if ( winWebOrgRsv != null ) {
        if ( !winWebOrgRsv.closed ) {
            opened = true;
        }
    }

    // web予約情報登録画面のURL編集
    var url = 'webOrgRsvMain.asp';
    ...
    url = url + '&orgCd1='     + '<%= strOrgCd1 %>';
    url = url + '&orgCd2='     + '<%= strOrgCd2 %>';
...
'#### 2010.10.28 SL-UI-Y0101-108 MOD END ####'
    ...
    if ( opened ) {
        winWebOrgRsv.focus();
        winWebOrgRsv.location.replace( url );
    } else {
        winWebOrgRsv = open( url, '', 'width=1000,height=680,status=yes,directories=no,menubar=no,resizable=yes,toolbar=no' );
    }
...
<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="700">
    <TR>
        <TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="reserve">■</SPAN><FONT COLOR="#000000">web団体予約情報の検索</FONT></B></TD>
    </TR>
</TABLE>
<%
    'エラーメッセージの編集
...
%>
<!-- '#### 2010.06.23 SL-UI-Y0101-108 MOD START ####' -->
<!--                <TD NOWRAP><A HREF="javascript:callWebOrgRsvWindow('<%= strCslDate(i) %>','<%= strWebNo(i) %>')"><%= strName %></A></TD>
-->
