...
function getBody() {
    ...
    // 国籍コード設定
    for ( var i = 0; i < bodyForm.nationcd.length; i++ ) {
        // 選択すべき要素であれば選択状態にする
        if ( bodyForm.nationcd.options[ index ].value == detailForm.nationCd.value ) {
            bodyForm.nationcd.options[ index ].selected = true;
        }
        index++;
    }
...
function setBody() {
    ...
    detailForm.nationCd.value   = bodyForm.nationcd.value;
    // 住所情報の編集
    for ( var i = 0; i < bodyForm.zipCd.length; i++ ) {
...
<%
    'ヘッダ画面のURL編集
    strURL = "webOrgRsvEditPersonHeader.asp"
...
<%
    'ボディ画面のURL編集
    strURL = "webOrgRsvEditPersonBody.asp"
    strURL = strURL & "?readOnly=" & strReadOnly
%>
