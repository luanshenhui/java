...
<%
'-----------------------------------------------------------------------------
'		メンタルヘルス　タイプＡチェック画面表示処理(Ver0.0.1)
'		AUTHER  : S.Sugie@C's
'-----------------------------------------------------------------------------
Option Explicit 
%>
<!--#INCLUDE VIRTUAL="/webhains/includes/mental/mhCommon.inc"-->
<!--#INCLUDE VIRTUAL="/webhains/includes/mental/mhConsulItem10.inc"-->
<%
'-------------------------------------------------------------------------------
' 共通宣言部
'-------------------------------------------------------------------------------
...
strMode = CInt(Request("ModeItem10"))
...
Do
	Select Case strMode
		...
		Case MH_Mode_Result
			'総合評価に遷移
			Session("FromAsp") = "mhConsulItem10.asp"
	...
	'セッションにセット
	Session("Refusal10") = Request("Check10")
	For i = 0 To UBound(aryQuestion)
		Session("Q10-" & i + 1) = Request("A10_" & i + 1)
	Next
...
function MoveCheck(strActMode) {
	...
	var i,j;
	var chkOn;
	var myForm = document.ConsulItem10;	// 自画面のフォームエレメント
	
	switch (strActMode) {
		case <%=MH_Mode_Back%>:
		case <%=MH_Mode_Next%>:
		case <%=MH_Mode_Result%>:
			if (!myForm.Check10.checked){
				//回答拒否じゃない場合、必須チェック
				for (i=0; i<=<%=UBound(aryQuestion)%>; i++) {
					//必須チェック
					chkOn = false;
					for (j=0; j<eval("myForm.A10_" + (i + 1) + ".length"); j++) {
						if (eval("myForm.A10_" + (i + 1) + "[" + j + "]" + ".checked")) {
							chkOn  = true
							break;
						}
					}
	...
	//submit
	myForm.ModeItem10.value = strActMode;
	myForm.submit();
...
<BODY>
<CENTER>
<FORM NAME="ConsulItem10" ACTION="<%= Request.ServerVariables("SCRIPT_NAME") %>" METHOD="post">
<DIV CLASS='page' NOWRAP>10/<%=MH_TotalPage%></DIV>
<HR ALIGN=center>
<DIV CLASS='title' NOWRAP>Ⅹ．あなたのタイプＡチェック</DIV>
<HR ALIGN=center>
<TABLE>
...
<%
strCheck = ""
If Session("Refusal10") = 1 Then
	strCheck = "CHECKED"
End If
%>
<TR><TD ALIGN=center NOWRAP><INPUT TYPE='checkbox' NAME='Check10' VALUE='1' <%=strCheck%>>
<%=MH_RefusalText3%>
</TD></TR>
</TABLE>
<HR ALIGN=center>
...
<TR CLASS='head'><TD COLSPAN='12'>あなたの物事や状況に対する振る舞いをお伺いします</TD></TR>
<%
For i = 0 To UBound(aryQuestion)
%>
	<TR CLASS='question'>
		<TD COLSPAN='11'><%=aryQuestion(i)%></TD>
		<TD COLSPAN='1' ><A HREF="JavaScript:CheckClear('<%="A10_" & i + 1%>')"><IMG SRC=<%=MH_ImagePath & "checkclear.gif"%> BORDER="0" WIDTH="77" HEIGHT="24" ALT="選択解除"></A></TD>
	</TR>
	<TR>
<%
	For j = 0 To UBound(aryAnswer)
		strCheck = ""
		If not isEmpty(Session("Q10-" & i + 1 )) And not isNull(Session("Q10-" & i + 1 )) Then
			If CInt(Session("Q10-" & i + 1 )) = j + 1 Then
				strCheck = "CHECKED"
			End If
		End If
%>
		<TD NOWRAP><INPUT TYPE='radio' NAME='<%="A10_" & i + 1%>' VALUE='<%=j + 1%>' <%=strCheck%>></TD>
...
<INPUT TYPE="hidden" NAME="ModeItem10"   VALUE="">
