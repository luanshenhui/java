...
<%
'-----------------------------------------------------------------------------
'		メンタルヘルス　精神健康度チェック画面表示処理(Ver0.0.1)
'		AUTHER  : S.Sugie@C's
'-----------------------------------------------------------------------------
Option Explicit 
%>
...
function MoveCheck(strActMode) {
	...
	var i,j;
	var chkOn;
	var myForm = document.ConsulItem4;	// 自画面のフォームエレメント
...
<HR ALIGN=center>
<DIV CLASS='title' NOWRAP>Ⅳ．あなたの精神健康度チェック</DIV>
<HR ALIGN=center>
<TABLE>
...
<TR CLASS='head'><TD COLSPAN='8'>この数週間の健康状態についてお伺いします</TD></TR>
<%
For i = 0 To UBound(aryQuestion)
%>
	<TR CLASS='question'>
		<TD COLSPAN='7'><%=aryQuestion(i)%></TD>
		<TD COLSPAN='1' ><A HREF="JavaScript:CheckClear('<%="A4_" & i + 1%>')"><IMG SRC=<%=MH_ImagePath & "checkclear.gif"%> BORDER="0" WIDTH="77" HEIGHT="24" ALT="選択解除"></A></TD>
	</TR>
	<TR>
<%
	For j = 0 To 3
