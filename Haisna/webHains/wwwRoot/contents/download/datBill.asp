...
Dim strOrgCd11          '請求先団体コード１１
Dim strOrgCd12          '請求先団体コード１２
Dim strOrgCd21          '請求先団体コード２１
Dim strOrgCd22          '請求先団体コード２２
Dim strOrgCd31          '請求先団体コード３１
Dim strOrgCd32          '請求先団体コード３２
Dim strOrgCd41          '請求先団体コード４１
Dim strOrgCd42          '請求先団体コード４２
Dim strOrgCd51          '請求先団体コード５１
Dim strOrgCd52          '請求先団体コード５２

Dim strOrgName1         '請求先団体名称１
Dim strOrgName2         '請求先団体名称２
Dim strOrgName3         '請求先団体名称３
Dim strOrgName4         '請求先団体名称４
Dim strOrgName5         '請求先団体名称５

Dim strBillNo           '請求書No.
Dim strIsDispatch       '請求書発送区分
Dim strIsPayment        '未収区分
...
Function Print()
    ...
    If Not IsArray(CheckValue()) Then
        ...
        '情報漏えい対策用ログ書き出し
        Call putPrivacyInfoLog("PH105", "データ抽出 請求書明細情報抽出（COMPANY連携用）よりファイル出力を行った")

'''■■■■■■■■■■ 画面項目にあわせて編集
'''オブジェクトのインスタンス作成（プロジェクト名.クラス名）
        Set objPrintCls = Server.CreateObject("HainsAbsenceListBill.AbsenceListBill")

        Ret = objPrintCls.PrintAbsenceListBill(Session("USERID"), CDate(strSCslDate), CDate(strECslDate), strOrgCd11, strOrgCd12,strOrgCd21, strOrgCd22, strOrgCd31, strOrgCd32, strOrgCd41, strOrgCd42, strOrgCd51, strOrgCd52, strBillNo, strIsDispatch, strIsPayment)
...
<TITLE>団体請求データCSV出力（経理システム連携用）</TITLE>
...
<!--
    ...
    function check(){
        ...
        {
            //if(orgCd11.value == "" && orgCd21.value == "" && orgCd31.value == "" && orgCd41.value == "" && orgCd51.value == "") {
            //    alert("請求団体を選択してください！！！");
            //    return;
            //}
            submit();
...
<!--- タイトル -->
    <TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
        <TR>
            <TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="print">■団体請求データCSV出力（経理システム連携用）</SPAN></B></TD>
        </TR>
    </TABLE>
<%
'エラーメッセージ表示
