...
Do
    ...
    '何らかの処理を行う場合
     If blnSave Or blnNext Then
        ...
        '次web予約情報のキー取得
        If blnNext Then
            SelectWebRsvNext dtmCslDate, lngWebNo, dtmNextCslDate, lngNextWebNo
        Else
    ...
    'web予約情報読み込み
    SelectWebRsv dtmCslDate, lngWebNo
...
'
'-------------------------------------------------------------------------------
Sub SelectWebRsv(dtmParaCslDate, lngParaWebNo)
...
'	'CanDateを関数最後尾に移動し、新WEB予約システム導入に伴う項目追加対応
'	''#### 2010.08.06 SL-UI-Y0101-107 MOD START ####
'	''※CanDateの対応で、RsvNoの前にカンマ１つ追加
'	''objWebRsv.SelectWebRsv dtmParaCslDate,  _
'	''                       lngParaWebNo,    _
'	''                       strRegFlg,       _
'	''                       strCsCd,         _
'	''                       strRsvGrpCd,     _
...
'
'-------------------------------------------------------------------------------
Sub SelectWebRsvNext(dtmParaCslDate, lngParaWebNo, dtmParaNextCslDate, lngParaNextWebNo)
...
Sub SelectPerson(strParaPerId)
    ...
    '個人情報読み込み
    objPerson.SelectPerson_lukes strPerId, , , , , strRomeName
...
<!--
    ...
    <TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0">
        ...
        <TR>
            <TD><IMG SRC="/webhains/images/ico_w.gif" WIDTH="16" HEIGHT="16" ALT=""></TD>
            <TD VALIGN="BOTTOM"><SPAN STYLE="color:#ff9900;font-weight:bolder;font-size:14px;">カナ姓を入力して下さい。</SPAN></TD>
        </TR>
    </TABLE>
-->
    <SPAN ID="errMsg">
...
%>
    ...
    <TABLE BORDER="0" CELLPADDING="1" CELLSPACING="0">
        ...
        <TR>
            <TD>団体</TD>
            <TD>：</TD>
            <TD ID="orgGuide"><A HREF="javascript:callOrgGuide()"><IMG SRC="/webhains/images/question.gif" WIDTH="21" HEIGHT="21" ALT="団体検索ガイドを表示します"></A></TD>
            <TD NOWRAP ID="dispOrgName"></TD>
        </TR>
        <TR>
            <TD HEIGHT="2"></TD>
        </TR>
...
%>
    ...
    document.getElementById('orgGuide').innerHTML = '';
<%
    '受診区分も選択不可とする
%>
...
If strRegFlg = REGFLG_REGIST Then
%>
    top.personal.location.replace( 'webRsvPersonal.asp?readOnly=1' );
<%
Else
%>
    top.personal.location.replace( 'webRsvPersonal.asp' );
<%
End If
