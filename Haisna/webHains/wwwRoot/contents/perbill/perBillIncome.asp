...
<!-- '### 2004/10/28 Add by gouda@FSIT 計上日を追加する -->
<!-- #include virtual = "/webHains/includes/editNumberList.inc" -->
<!-- '### 2004/10/28 End -->
<!-- '### 2004/11/30 Update by gouda@FSIT 計上日を関数にて取得に変更 -->
...
Dim	strDispMode         '表示モード(1:まとめて入金、0:それ以外）
...
Dim strKeyDate          '元の入金日
Dim lngKeySeq           '元の入金Ｓｅｑ

Dim strMaxDmdDate       '一番新しい請求日

Dim strPaymentDate      '入金日
Dim strPaymentYear      '入金日（年）
Dim strPaymentMonth     '入金日（月）
Dim strPaymentDay       '入金日（日）
...
'### 2004/11/2 Add by gouda@FSIT 計上日を追加する
'締め日の取得
...
' ### 2004/11/09 Update by gouda@FSIT 入金日のチェックを追加する
        '入金日の編集
...
'## 2003.12.20 Mod By T.Takagi@FSIT 領収書印刷対応
...
'		End If
        ...
        'エラーがなければ印刷のみを行うHTMLを編集し、完了後自身をREPLACE。
        'エラーがなければ領収書印刷画面を表示し、完了後呼び出し元を再表示。 ### 2003.12.25 start
        strHTML = "<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.01 Transitional//EN"">"
strHTML = strHTML & vbCrLf & "<HTML lang=""ja"">"
    ...
    If strDmdDate = "" OR IsNull(strDmdDate) Then 
        ...
        For i=0 To lngBillNoCnt
            If vntDmdDate(i) <> Empty  Then
                ...
                ''' 名前を取得する処理追加 2003.12.18 add START
                ...
                Else
                    ...
                    End If
                End If
                ''' 名前を取得する処理追加 2003.12.18 add END
...
'	lngChangePrice = (CLng(lngCredit) + CLng(lngHappy_Ticket) + CLng(lngCard) + CLng(lngJdebit) + CLng(lngCheque)) - CLng(lngPriceTotal)
    ...
    '### 2004/11/29 Update by gouda@FSIT 計上日を関数にて取得に変更
...
'			.AppendArray vntArrMessage, "入金日が請求日より過去です。"
'		End If

        If Not IsDate(Trim(strPaymentDate)) Then
            .AppendArray vntArrMessage, "入金日の入力形式が正しくありません。"
        Else
            If CDate(strMaxDmdDate) > CDate(strPaymentDate) Then
                .AppendArray vntArrMessage, "入金日は請求日よりも過去の日付に設定することはできません。"
            End If
        End If
        ' ### 2004/11/09 Update End

        ' ### 2004/11/05 Add by gouda@FSIT 計上日を追加する
        ...
        Else
            If IsDate(Trim(strPaymentDate)) Then
                If CDate(strPaymentDate) > CDate(strCalcDate) Then
                    .AppendArray vntArrMessage, "計上日は入金日よりも過去の日付に設定することはできません。"
                End If
            End If
        End If
        ' ### 2004/11/09 Add End
        ...
        if lngChangePrice < 0 Then
            .AppendArray vntArrMessage, "入金金額が請求金額に達していないため保存できません。"
        End If
    End With
...
//	varChangePrice = (varCredit + varHappy_Ticket + varCard + varJdebit + varCheque) - varPriceTotal;
    ...
    if( varChangePrice >= 0 ){
        document.getElementById('changeprice').innerHTML = 'おつりは　' + formatCurrency(varChangePrice) + '　です';
    } else {
        //未受付の場合
        if ( dayid == '' ){
            ...
            if ( comedate == '' ){
                ...
                document.getElementById('changeprice').innerHTML = '請求金額に達していません';
            }
        }
    }
...
// '### 2004/11/8 Add by gouda@FSIT 計上日を追加する -->
// 確定ボタン押下時の処理
...
<BODY ONLOAD="JavaScript:document.entryForm.credit.focus()" ONUNLOAD="javascript:windowClose()">
    ...
    <TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="100%">
        <TR>
            <TD HEIGHT="15" BGCOLOR="#ffffff" NOWRAP><SPAN CLASS="demand">■</SPAN><B>入金情報</B></TD>
        </TR>
    </TABLE>
    <!-- 引数情報 -->
...
%>
    ...
    <TABLE BORDER="0" CELLPADDING="1" CELLSPACING="0">
        <TR>
            <TD COLSPAN="3" NOWRAP HEIGHT="25"><SPAN STYLE="color:#cc9999">●</SPAN>請求金額を確認してください。</TD>
        </TR>
        <TR>
...
%>
        ...
        <tr>
            <td width="114"><input type="checkbox" name="checkCheque" value="5" <%= IIf(strCheckCheque <> "", " CHECKED", "") %>  ONCLICK="javascript:checkChequeAct()" border="0">小切手・フレンズ</td>
            <td>：</td>
            <td width="479">
                <table border="0" cellspacing="0" cellpadding="1">
                    <tr>
...
<%
            If lngChangePrice >= 0 Then
%>
                <TD WIDTH="409"><B><FONT SIZE="6" COLOR="#ff8c00"><SPAN ID="changeprice" >おつりは　<%= FormatCurrency(lngChangePrice) %>　です</SPAN></FONT></B></TD>
<%
            Else
                '未受付の場合
...
<%
                    Else
%>
                        <TD WIDTH="409"><B><FONT SIZE="6" COLOR="#ff8c00"><SPAN ID="changeprice">請求金額に達していません</SPAN></FONT></B></TD>
<%
                    End If
                End If
            End If
...
<!-- '### 2004/11/9 Add by gouda@FSIT 計上日を追加する -->
        <TR>
...
%>
                <TD><A HREF="javascript:deleteData()"><IMG SRC="/webHains/images/delete.gif" WIDTH="77" HEIGHT="24" ALT="この入金情報を削除します" border="0"></A></TD>
<%
                End If
            End If
%>
...
<%
'### 2004/05/19 Mod by H.Ishihara@FSIT 聞き方が足りない
'### 2004.02.18 Mod by H.Ishihara@FSIT 入金済みなのにクッキーの値を出している
'					If IsNull(strPaymentDate) = True Then
