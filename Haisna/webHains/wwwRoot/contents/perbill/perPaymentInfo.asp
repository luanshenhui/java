...
Dim vntCloseDate			'請求書番号（締め日）
Dim vntCloseBillSeq			'請求書番号（請求書Seq）
Dim vntCloseBranchNo		'請求書番号（枝番）
...
'請求締め情報用変数
Dim strDispBillNo			'アンカー＆表示用請求書No
Dim strWorkNo				'BillSeq０詰め用作業変数

strMessage = ""

'## 2004.03.27 Added By Ishihara@FSIT お連れさま情報の取得
'お連れ様情報取得用変数
Dim lngFriendsCount			'お連れ様情報取得件数
Dim strArrRsvNo				'予約番号
Dim strArrPerId				'個人ID
Dim strArrCsName			'コース名
Dim strArrLastName			'漢字名（姓）
Dim strArrFirstName			'漢字名（名）
Dim strArrLastKName			'カナ名（姓）
Dim strArrFirstKName		'カナ名（名）
Dim strArrDayID				'当日ID
'## 2004.03.27 Added End
...
Do
	...
	'締め管理情報取得
'### 2004/2/2 Updated by Ishihara@FSIT 聖路加専用
...
'## 2004.03.27 Added By Ishihara@FSIT お連れさま情報の取得
	'お連れ様情報の取得
	lngFriendsCount = objConsult.SelectFriends(strCslDate, _
											   lngRsvNo, _
											   ,    _
											   ,        _
											   strArrRsvNo,      _
...
//		winIncome1 = window.open( url, '', 'width=600,height=650,status=yes,directories=no,menubar=no,resizable=yes,toolbar=no,scrollbars=yes');
		winIncome1 = window.open( url, '', 'width=800,height=650,status=yes,directories=no,menubar=no,resizable=yes,toolbar=no,scrollbars=yes');
// ## 2003.12.20 Mod End
	}
}
...
// ### 2004.01.20 返金情報が入金情報と同じフォーマットになったのでサイズを変更
//		winHenkin = window.open( url, '', 'width=600,height=450,status=yes,directories=no,menubar=no,resizable=yes,toolbar=no,scrollbars=yes');
		winHenkin = window.open( url, '', 'width=600,height=650,status=yes,directories=no,menubar=no,resizable=yes,toolbar=no,scrollbars=yes');
	}
}
...
//	url = url + '&p_ScslDate=' + dmdDate;
//	url = url + '&p_BilSeq='   + billSeq;
//	url = url + '&p_BilBan='   + branchNo;
//	url = url + '&p_Option='   + '0';
//	open( url );
	var url = '/webHains/contents/perbill/prtPerBill.asp';
	url = url + '?reqdmddate=' + dmdDate;
	url = url + '&reqbillseq='   + billSeq;
	url = url + '&reqbranchno='   + branchNo;
	url = url + '&prtKbn='   + '0';
	open( url );
...
<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1" BGCOLOR="#999999" WIDTH="650">
	<TR HEIGHT="16">
		<TD HEIGHT="16" BGCOLOR="#ffffff" NOWRAP><B><SPAN CLASS="demand">■</SPAN><FONT COLOR="#000000">個人受診金額情報</FONT></B></TD>
	</TR>
</TABLE>
...
<BR>
	...
	<TABLE BORDER="0" CELLPADDING="1" CELLSPACING="0" WIDTH="746">
		<TR>
		...
		<TR VALIGN="TOP">
			...
			<TD NOWRAP></TD>
			<TD NOWRAP ROWSPAN="2" VALIGN="BOTTOM"><INPUT TYPE="IMAGE" SRC="../../images/b_prev.gif" BORDER="0" WIDTH="53" HEIGHT="28" ALT="表示する"></TD>
		</TR>
		<TR VALIGN="TOP">
	...
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="1">
		<TR>
			...
			<SPAN STYLE="color:#cc9999">●</SPAN><FONT COLOR="black">お連れ様情報</FONT></TD>
		</TR>
<%
	If lngFriendsCount > 0 Then
%>
		<TR BGCOLOR="CCCCCC">
			<TD>当日ID</TD>
			<TD>個人ID</TD>
			<TD>コース名</TD>
			<TD COLSPAN="2">受診者名</TD>
		</TR>
<%
		For i = 0 To lngFriendsCount - 1

			If strArrRsvNo(i) <> lngRsvNo Then
%>
				<TR BGCOLOR="#EEEEEE">
					<TD><%= strArrDayID(i) %></TD>
					<TD><%= strArrPerId(i) %></TD>
					<TD><%= strArrCsName(i) %></TD>
					<TD><%= strArrLastName(i) & "　" & strArrFirstName(i) %></TD>
					<TD><FONT COLOR="#999999">（<%= strArrLastKName(i) & "　" & strArrFirstKName(i) %>）</FONT></TD>
				</TR>
<%
			End If
		Next
...
<!-- 2004.01.02 全て入金済みの場合はこのボタンは非表示  start -->
<%
...
<%
            ...
            End If
%>
<!-- 2004.01.02 全て入金済みの場合はこのボタンは非表示  end -->
		</TR>
		<TR>
...
%>
				<% if Session("PAGEGRANT") = "2" or Session("PAGEGRANT") = "4" then %>
                    <TD NOWRAP ALIGN="RIGHT"><A HREF="JavaScript:PrintWindow(<%= lngRsvNo %>, '<%= vntDmdDate(i) %>', <%= vntBillSeq(i) %>, <%= vntBranchNo(i) %>)"><IMG SRC="/webHains/images/b_Prt.gif" WIDTH="51" HEIGHT="24" ALT="この請求書を印刷します"></A></TD>
                <% End If %>
				</TR>
<%
	...
	<TABLE BORDER="0" CELLPADDING="2" CELLSPACING="2">
        ...
        <% if Session("PAGEGRANT") = "2" or Session("PAGEGRANT") = "4" then %>
			<TD NOWRAP ALIGN="LEFT" VALIGN="BOTTOM">
				<A HREF="JavaScript:OptionWindow()"><IMG SRC="/webHains/images/b_ChangeSet.gif" WIDTH="110" HEIGHT="24" ALT="受診セットを変更します"></A>
			</TD>
			<TD NOWRAP ALIGN="LEFT" VALIGN="BOTTOM">
				<A HREF="JavaScript:otherIncomeWindow()"><IMG SRC="/webHains/images/b_OtherSet.gif" WIDTH="110" HEIGHT="24" ALT="セット外請求明細情報を追加します"></A>
			</TD>
			<TD NOWRAP ALIGN="LEFT" VALIGN="BOTTOM">
				<A HREF="JavaScript:OmitTaxAct()"><IMG SRC="/webHains/images/b_delTax.gif" WIDTH="110" HEIGHT="24" ALT="全ての個人明細において消費税を免除します"></A>
			</TD>
		<% End If %>
...
<%
		...
		For i = 0 To lngCloseCount - 1
			...
			strDispBillNo = ""
			If Trim(vntCloseDate(i)) <> "" Then
				strDispBillNo = Mid(vntCloseDate(i), 1, 4) & Mid(vntCloseDate(i), 6, 2) & Mid(vntCloseDate(i), 9, 2)
				strDispBillNo = strDispBillNo & Mid(strWorkNo, 1, Len(strWorkNo) - Len(vntCloseBillSeq(i)) ) & vntCloseBillSeq(i)
				strDispBillNo = strDispBillNo & vntCloseBranchNo(i)
			End If
%>
			<TR BGCOLOR="#EEEEEE">
				<TD NOWRAP COLSPAN="2"><%= vntCloseOrgName(i) %></TD>
				<TD NOWRAP><A HREF="/webHains/contents/demand/dmdBurdenModify.asp?billNo=<%= strDispBillNo %>" TARGET="_blank"><%= strDispBillNo %></A></TD>
			</TR>
<%
		Next
%>
