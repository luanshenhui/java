...
'### 2004.01.23 バグ修正（返金、入金の区別に使用しているので不定の場合は０をセットしておく）
lngBranchNo   = IIf(lngBranchNo = "", 0, lngBranchNo )
...
'			Err.Raise 1000, , "の取消しに失敗しました。（請求書No　= " & objCommon.FormatString(strDmdDate, "yyyymmdd") & objCommon.FormatString(lngBillSeq, "00000") & lngBranchNo &" )"
		Else
'### 2005/03/07 Updated by Ishihara@FSIT 当画面は呼び出し元がない。よって削除された場合新規画面へ遷移。
...
'											vntOtherLineName )
	...
	If IsNull(strPaymentDate) = False Then
		...
		End If
'### 2004.01.20 返金のときは返金らしいコメントにする
		If lngBranchNo = 1 Then
			objCommon.AppendArray strArrMessage, "返金済のため修正できません。"
		Else
...
//	url = url + '?billcount=0&mode=update';
...
<%
	...
	Else
		'修正モードで入金情報あり？
'### 2005/03/07 Updated by Ishihara@FSIT テストせずにリリースする奴は死んでください。
'		If strMode = "update" And IsNull(strPaymentDate) = False Then
		If strMode = "update" And lngPaymentFlg <> 0 Then
...
%>
						...
						<TD><%= strDay %></TD>
						<TD>&nbsp;日</TD>
						<INPUT TYPE="HIDDEN" NAME="year"  VALUE="<%= Clng("0" & strYear) %>">
						<INPUT TYPE="HIDDEN" NAME="month" VALUE="<%= Clng("0" & strMonth) %>">
						<INPUT TYPE="HIDDEN" NAME="day"   VALUE="<%= Clng("0" & strDay) %>">
<%
						Else
%>
...
<%
				...
				If lngBranchNo = 1 Then
%>
					<A HREF="JavaScript:perBillIncomeWindow('<%= vntPerId(i) %>', '<%= strDmdDate %>', <%= lngBillSeq %>, <%= lngBranchNo %>)">返金情報</A></TD>
<%
				Else
%>
...
<!-- 振込み　追加 2003.12.25 -->
				<TD NOWRAP ALIGN="right"><B><%= IIf( lngTransfer <> "", FormatCurrency(lngTransfer), "") %></B></TD>
