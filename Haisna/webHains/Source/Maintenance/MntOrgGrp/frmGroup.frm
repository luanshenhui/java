...
Begin VB.Form frmGroup 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cboSearchChar 
         Height          =   300
         ItemData        =   "frmGroup.frx":0094
         Left            =   1680
         List            =   "frmGroup.frx":00B6
         Style           =   2  'ﾄﾞﾛｯﾌﾟﾀﾞｳﾝ ﾘｽﾄ
         TabIndex        =   13
         Top             =   1950
         Visible         =   0   'False
   ...
   Begin VB.Image Image1 
      ...
      Index           =   0
      Left            =   180
      Picture         =   "frmGroup.frx":00D8
      Top             =   180
      Width           =   480
   End
   Begin VB.Label LabelCourseGuide 
      Caption         =   "団体をグループとしてとりまとめます。"
      Height          =   255
      Left            =   840
      TabIndex        =   23
      Top             =   300
...
'2006.02.03 Add by 李
Private aryUseGrpCd()          As String       'UseGrp
...
Private Sub cmdAddItem_Click()
    ...
    Dim objItemGuide    As mntorgGuide.ItemGuide   '項目ガイド表示用
    ...
    'オブジェクトのインスタンス作成
    Set objItemGuide = New mntorgGuide.ItemGuide
    ...
    If lngItemCount > 0 Then
        ...
        For i = 0 To lngItemCount - 1
            ...
            If mintGrpDiv = MODE_RESULT Then
                '検査項目の場合
                strItemCdString = vntItemCd(i) & vntItemName(i)
            ...
            If CheckExistsItemCd(lsvItem, strItemKey) = False Then
                ...
                'なければ追加する
                Set objItem = lsvItem.ListItems.Add(, strItemKey, vntItemCd(i))
...
Private Sub InitializeForm()
    ...
    'グループの種類
        ...
        lblGrpDiv.Caption = "団体グループ項目を管理します"
...
Private Function EditGrp() As Boolean
    ...
    'オブジェクトのインスタンス作成
    Set objGrp = CreateObject("HainsOrgGrp.Grp")
    ...
    Do
        ...
        'グループテーブルレコード読み込み
'###### 2006.02.05 UseGrp追加 Add by 李　###########
        If objGrp.SelectGrp_ORG(mstrGrpCd, vntGrpName, vntSystemGrp, vntUseGrp) = False Then
        ...
        cboUseGrp.ListIndex = CInt(vntUseGrp)
...
'##add end
    ...
    'グループ内検査項目検索
'    If mintGrpDiv = MODE_RESULT Then
        lngCount = objGrp.SelectGrp_ORG_ItemList(mstrGrpCd, vntItemCd, vntItemCd1, vntItemName)
'    Else
'        '依頼項目の場合
'        lngCount = objGrp.SelectGrp_ORG_ItemList(mstrGrpCd, vntItemCd, vntItemName, vntClassName)
'
'    End If
    ...
    With objHeader
        .Clear
        .Add , , "コード1", 1000, lvwColumnLeft
        .Add , , "コード2", 1000, lvwColumnLeft
        .Add , , "団体名称", 4000, lvwColumnLeft
    End With
...
'        End If
          strItemKey = mstrListViewKey & vntItemCd(i) & vntItemCd1(i)
        Set objItem = lsvItem.ListItems.Add(, strItemKey, vntItemCd(i))

           With objItem
                objItem.SubItems(1) = vntItemCd1(i)
                objItem.SubItems(2) = vntItemName(i)
          End With
...
'            Else
                vntItemCd(j) = lsvItem.ListItems(i).Text
                vntSuffix(j) = lsvItem.ListItems(i).SubItems(1)
                vntseq(j) = lsvItem.ListItems(i).SubItems(2)
                vntrslCaption(j) = ""
...
'        End If
    ...
    lngRet = objGrp.RegistGrp_ORG(IIf(txtGrpCd.Enabled, "INS", "UPD"), _
...
'
Private Function EditUseGrp() As Boolean
    Dim objItem                 As Object           'コースアクセス用
    Dim lngCount                As Long             'レコード数
    Dim i                       As Long             'インデックス
    Dim j                       As Integer
    ...
    cboUseGrp.AddItem ""
    For i = 0 To lngCount - 1
        ReDim Preserve aryUseGrpCd(i)
        aryUseGrpCd(i) = i + 1
        cboUseGrp.AddItem vntFreeField1(i)
    Next i
