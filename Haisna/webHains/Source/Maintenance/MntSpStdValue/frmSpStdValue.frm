...
Begin VB.Form frmSpStdValue 
   BorderStyle     =   3  '固定ﾀﾞｲｱﾛｸﾞ
   Caption         =   "特定健診基準値テーブルメンテナンス"
   ClientHeight    =   6855
   ClientLeft      =   1335
   ClientTop       =   2565
   ClientWidth     =   12315
   ...
   Begin TabDlg.SSTab TabMain 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "男性"
      TabPicture(0)   =   "frmSpStdValue.frx":0000
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame1"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "女性"
      TabPicture(1)   =   "frmSpStdValue.frx":001C
   ...
   Begin MSComctlLib.ImageList imlToolbarIcons 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmSpStdValue.frx":0038
            Key             =   "CLOSE"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmSpStdValue.frx":0192
            Key             =   "OPEN"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmSpStdValue.frx":02EC
            Key             =   "MYCOMP"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmSpStdValue.frx":0446
            Key             =   "DEFAULTLIST"
         EndProperty
      EndProperty
   End
   Begin VB.Image Image1 
      ...
      Index           =   4
      Left            =   120
      Picture         =   "frmSpStdValue.frx":05A0
      Top             =   120
      Width           =   720
   End
   ...
   End
End
Attribute VB_Name = "frmSpStdValue"
...
Private mblnHistoryUpdated      As Boolean          'TRUE:特定健診用基準値履歴更新あり、FALSE:特定健診用基準値履歴更新なし
Private mblnItemUpdated         As Boolean          'TRUE:特定健診用基準値詳細更新あり、FALSE:特定健診用基準値詳細更新なし

Private mintBeforeIndex         As Integer          '履歴コンボ変更キャンセル用の前Index
Private mblnNowEdit             As Boolean          'TRUE:編集処理中、FALSE:処理なし

Private mstrArrSpStdValueMngCd()  As String         '特定健診用基準値管理コード（コンボボックス対応用）
Private mcolSpStdValueRecord      As Collection     '特定健診用基準値レコードのコレクション
Private mcolSpStdValue_cRecord    As Collection     '特定健診基準値詳細レコードのコレクション（読み込み直後のみ使用）
...
' 備考　　 :
'
Private Function EditSpStdValue(strItemCd As String, _
                              strSuffix As String, _
                              blnCopy As Boolean, _
                              Optional colWorkCollection As Collection) As Boolean

    Dim objSpStdValue       As Object           '特定健診用基準値管理アクセス用
    ...
    Dim objSpStdValue_Record  As SpStdValue_Record  '特定健診用基準値レコードオブジェクト
    
    On Error GoTo ErrorHandle
    
    'COPYモードでないなら、コレクション初期化
    If blnCopy = False Then
        Set mcolSpStdValueRecord = Nothing
        Set mcolSpStdValueRecord = New Collection
    End If
    ...
    Do
        ...
        '特定健診用基準値管理テーブルレコード読み込み
        lngCount = objSpStdValue.SelectSpStdValueList("", _
                                                  strItemCd, _
                                                  strSuffix, _
        ...
        If lngCount = 0 Then
            ...
            If blnCopy = False Then
                'COPYモードでないなら、新規作成
                '特定健診用基準値管理テーブルが存在しない場合（新規作成モード）
                Call AddNewSpStdValue
            Else
        ...
        Else
            ...
            '特定健診用基準値管理テーブルが存在する場合（更新モード）
        
            '読み込み内容の編集
            For i = 0 To lngCount - 1
                
                Set objSpStdValue_Record = Nothing
                Set objSpStdValue_Record = New SpStdValue_Record
                
                'オブジェクト作成
                With objSpStdValue_Record
                    .SpStdValueMngCd = vntSpStdValueMngCd(i)
                    .ItemCd = vntItemCd(i)
                    .Suffix = vntSuffix(i)
                ...
                If blnCopy = False Then
                    ...
                    'コンボボックス対応配列の作成
                    ReDim Preserve mstrArrSpStdValueMngCd(i)
                    mstrArrSpStdValueMngCd(i) = KEY_PREFIX & objSpStdValue_Record.SpStdValueMngCd
                    
                    'コレクション追加
                    mcolSpStdValueRecord.Add objSpStdValue_Record, KEY_PREFIX & objSpStdValue_Record.SpStdValueMngCd
                
                Else
                    'COPYモードなら、引数のコレクションに格納
                    
                    'コレクション追加
                    colWorkCollection.Add objSpStdValue_Record, KEY_PREFIX & objSpStdValue_Record.SpStdValueMngCd
...
' 備考　　 :
'
Private Function GetSpStdValue_c(strSpStdValueMngCd As String, blnCopy As Boolean) As Boolean

    Dim objSpStdValue           As Object       '特定健診用基準値管理アクセス用
    
    Dim vntSpStdValueCd         As Variant      '特定健診用基準値コード
    ...
    Dim objSpStdValue_C_Record  As SpStdValue_C_Record    '特定健診用基準値詳細レコードオブジェクト
    
    On Error GoTo ErrorHandle
        
    '現在表示している値のクリア (COPYモードのときもクリア）
    Set mcolSpStdValue_cRecord = Nothing
    Set mcolSpStdValue_cRecord = New Collection

    Do
        '特定健診用基準値管理コードが指定されていない場合は何もしない
        If (strSpStdValueMngCd = "") Or (strSpStdValueMngCd = "0") Then
            Ret = True
            Exit Do
        End If
        ...
        '特定健診用基準値管理テーブルレコード読み込み
        lngCount = objSpStdValue.SelectSpStdValue_cList(strSpStdValueMngCd, _
                                                    vntSpStdValueCd, _
                                                    vntGender, _
                                                    vntStrAge, _
                                                    vntEndAge, _
        ...
        For i = 0 To lngCount - 1
            '読み込み内容をオブジェクトにセット
            Set objSpStdValue_C_Record = New SpStdValue_C_Record
            With objSpStdValue_C_Record
                
                If blnCopy = True Then
                    'Copyモードの場合、特定健診用基準値管理コードはもとのまま、かつ特定健診用基準値コードはセットしない（新規イメージ）
                    .SpStdValueMngCd = mcolSpStdValueRecord(mstrArrSpStdValueMngCd(cboHistory.ListIndex)).SpStdValueMngCd
                    .SpStdValueCd = ""
                Else
                    .SpStdValueMngCd = strSpStdValueMngCd
                    .SpStdValueCd = vntSpStdValueCd(i)
                End If
            ...
            'コレクションに追加
            mcolSpStdValue_cRecord.Add objSpStdValue_C_Record, KEY_PREFIX & vntSpStdValueCd(i)
...
ErrorHandle:
    ...
    GetSpStdValue_c = False
    MsgBox Err.Description, vbCritical
    
End Function

'
' 機能　　 : 特定健診用基準値詳細情報の表示（コレクションから）
'
' 引数　　 : なし
'
' 戻り値　 : TRUE:正常終了、FALSE:異常終了
...
On Error GoTo ErrorHandle
    ...
    'リストの編集
    For Each objSpStdValue_C_Record In mcolSpStdValue_cRecord
        With objSpStdValue_C_Record
            ...
            objItem.SubItems(8) = .SpStdValueCd
        
        End With
        mintUniqueKey = mintUniqueKey + 1
    
    Next objSpStdValue_C_Record
    
    'オブジェクト廃棄
    Set objSpStdValue_C_Record = Nothing
...
' 備考　　 :
'
Private Function RegistSpStdValue() As Boolean

On Error GoTo ErrorHandle

    Dim objSpStdValue           As Object       '特定健診用基準値管理アクセス用
    Dim Ret                     As Long
    Dim objCurSpStdValue_Record As SpStdValue_Record
    ...
    '特定健診用基準値
    Dim lngSpStdValueMngCd      As Long

    '特定健診用基準値詳細の配列関連
    Dim intItemCount            As Integer
    Dim vntSpStdValueCd         As Variant
    ...
    If mstrArrSpStdValueMngCd(cboHistory.ListIndex) = "K0" Then
        blnNewRecordFlg = True
    End If

    '現在のカレント情報をオブジェクトにセット
    Set objCurSpStdValue_Record = mcolSpStdValueRecord(mstrArrSpStdValueMngCd(cboHistory.ListIndex))
    
    '特定健診用基準値管理テーブルレコードの登録
    With objCurSpStdValue_Record
        ...
        '特定健診用基準値管理コードのセット
        lngSpStdValueMngCd = .SpStdValueMngCd
        
        '特定健診用基準値詳細テーブルの配列セット
        ...
        '特定健診用基準値データの登録
        Ret = objSpStdValue.RegistSpStdValue_All(lngSpStdValueMngCd, _
                                             .ItemCd, _
                                             .Suffix, _
                                             .strDate, _
                                             .endDate, _
    ...
    If blnNewRecordFlg = True Then
        ...
        '新しい特定健診用基準値履歴オブジェクトを作成
        Set objCurSpStdValue_Record = Nothing
        Set objCurSpStdValue_Record = New SpStdValue_Record
        With objCurSpStdValue_Record
            .SpStdValueMngCd = lngSpStdValueMngCd
            .ItemCd = strEscItemCd
            .Suffix = strEscSuffix
        ...
        mcolSpStdValueRecord.Remove (mstrArrSpStdValueMngCd(cboHistory.ListIndex))
        mcolSpStdValueRecord.Add objCurSpStdValue_Record, KEY_PREFIX & lngSpStdValueMngCd
        
        'コンボボックスの値も変更
        mstrArrSpStdValueMngCd(cboHistory.ListIndex) = KEY_PREFIX & lngSpStdValueMngCd
...
ErrorHandle:
    ...
    If blnBeforeUpdatePoint = True Then
        MsgBox "特定健診用基準値書き込み処理に失敗しました。原因としては以下の事由が考えられます。" & vbLf & vbLf & _
...
Private Sub EditArrayForUpdate(intItemCount As Integer, _
    ...
    Dim vntArrSpStdValueCd()    As Variant
    ...
    For intListViewIndex = 0 To 1
        ...
        For i = 1 To obTargetListView.ListItems.Count
            ...
            ReDim Preserve vntArrSpStdValueCd(intArrCount)
            ...
            With obTargetListView.ListItems(i)
                ...
                vntArrSpStdValueCd(intArrCount) = .SubItems(8)
...
Private Sub cboHistory_Click()
    ...
    Do
        ...
        If (mblnItemUpdated = True) Or (mblnHistoryUpdated = True) Then
            strMsg = "特定健診用基準値の設定内容が更新されています。履歴データを再表示すると変更内容が破棄されます" & vbLf & _
                     "よろしいですか？"
            intRet = MsgBox(strMsg, vbYesNo + vbDefaultButton2 + vbExclamation)
            If intRet = vbNo Then
        ...
        '特定健診用基準値詳細情報の編集
        If GetSpStdValue_c(mcolSpStdValueRecord(mstrArrSpStdValueMngCd(cboHistory.ListIndex)).SpStdValueMngCd, False) = False Then
            Exit Do
        End If
...
'#### 2010.07.16 SL-HS-Y0101-001 ADD START ####　COMP-LUKES-0016（非互換検証）
                    objItem.SubItems(8) = ""
'#### 2010.07.16 SL-HS-Y0101-001 ADD END ####　　COMP-LUKES-0016（非互換検証）
...
'#### 2010.07.16 SL-HS-Y0101-001 ADD END ####　　COMP-LUKES-0016（非互換検証）
    ...
    'オブジェクトの廃棄
    Set frmEditSpStdValueItem = Nothing
...
Private Function ApplyData(blnOkMode As Boolean) As Boolean
    ...
    '特定健診用基準値管理テーブルの登録
    If RegistSpStdValue() = False Then
        Exit Function
    End If
    ...
    '特定健診用基準値詳細情報の編集（画面再表示を行うことにより基準値コードを再取得）
    If GetSpStdValue_c(mcolSpStdValueRecord(mstrArrSpStdValueMngCd(cboHistory.ListIndex)).SpStdValueMngCd, False) = False Then
        Exit Function
    End If
...
Private Sub cmdEditHistory_Click()
    ...
    With frmEditSpStdValueHistory
        
        'プロパティセット
        .strDate = objCurSpStdValue_Record.strDate
        .endDate = objCurSpStdValue_Record.endDate
        .CsCd = objCurSpStdValue_Record.CsCd
        ...
        If .Updated = True Then
            ...
            'オブジェクト内容を更新
            objCurSpStdValue_Record.strDate = .strDate
            objCurSpStdValue_Record.endDate = .endDate
            objCurSpStdValue_Record.CsCd = .CsCd
...
Private Sub cmdNewHistory_Click()
    ...
    Do
        ...
        '特定健診用基準値詳細情報の編集（新規なので本来不要だが先頭でメモリクリアしておしまい）
        If GetSpStdValue_c(mcolSpStdValueRecord(mstrArrSpStdValueMngCd(cboHistory.ListIndex)).SpStdValueMngCd, False) = False Then
            Exit Do
        End If
        
        '特定健診用取得基準値情報のリストビュー格納（新規なので本来不要だが先頭でメモリクリアしておしまい）
        If EditListViewFromCollection() = False Then
            Exit Do
        End If
...
Private Sub cmdOtherItemCopy_Click()
    ...
    If lngItemCount > 0 Then
        ...
        '特定健診用基準値履歴管理情報の編集
        If EditSpStdValue(CStr(vntItemCd(0)), CStr(vntSuffix(0)), True, colWorkCollection) = False Then
            Exit Sub
        End If

        '履歴数が１つ以上ある場合、選択ダイアログ表示
        If colWorkCollection.Count > 0 Then
            With frmSelectCopySpStdValue
        ...
        '特定健診用基準値詳細情報の編集
        If GetSpStdValue_c(colWorkCollection(i).SpStdValueMngCd, True) = False Then
            Exit Sub
        End If

        '特定健診用取得基準値情報のリストビュー格納
        If EditListViewFromCollection() = False Then
            Exit Sub
        End If
...
Private Sub Form_Load()
    ...
    Do
        ...
        '特定健診用基準値履歴管理情報の編集
        If EditSpStdValue(mstrItemCd, mstrSuffix, False) = False Then
            Exit Do
        End If
    
        cboHistory.ListIndex = 0
    
        '特定健診用基準値管理コードが設定されている場合、その値でコンボ初期表示
        If mstrSpStdValueMngCd <> "" Then
            
            For i = 0 To UBound(mstrArrSpStdValueMngCd)
                If mstrArrSpStdValueMngCd(i) = KEY_PREFIX & mstrSpStdValueMngCd Then
...
Friend Property Get SpStdValueMngCd() As String

    SpStdValueMngCd = mstrSpStdValueMngCd

End Property

Friend Property Let SpStdValueMngCd(ByVal vNewValue As String)
...
' 備考　　 :
'
Private Sub AddNewSpStdValue()
    
    Dim objSpStdValue_Record  As SpStdValue_Record  '特定健診用基準値レコードオブジェクト
    Dim intArrCount         As Integer
    
    '配列数の取得
    intArrCount = mcolSpStdValueRecord.Count
    
    '配列を拡張（コレクションの数で作成すると必然的に+1になる）
    ReDim Preserve mstrArrSpStdValueMngCd(intArrCount)
    ...
    '配列に退避
    mstrArrSpStdValueMngCd(intArrCount) = KEY_PREFIX & objSpStdValue_Record.SpStdValueMngCd
...
Private Sub CopyItem(Index As Integer, Cancel As Boolean)
    ...
    For i = 1 To lsvItem(intOtherIndex).ListItems.Count
        ...
        'アイテムコピー（特定健診用基準値コード設定欄はコピーしちゃだめ）
