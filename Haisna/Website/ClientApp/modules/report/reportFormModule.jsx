...
const URL_FOR_REPORTING_BASE = '/api/v1/reports';
...
// 会計台帳
export const {
  getAccountBookDataRequest,
  getAccountBookDataSuccess,
  getAccountBookDataFailure,
} = createActions(
  'GET_ACCOUNT_BOOK_DATA_REQUEST',
  'GET_ACCOUNT_BOOK_DATA_SUCCESS',
  'GET_ACCOUNT_BOOK_DATA_FAILURE',
);
...
export const createReport = (conditions, meta) => (dispatch) => {
  // パラメーターの定義
  ...
  // 出力用APIの呼び出し
  axios.post(`${URL_FOR_REPORTING_BASE}/${meta.reportName}`, params)
    ...
    .then((res) => {
      ...
      open(`/api/v1/reports/${res.data.printseq}`);
    })
    .catch((data) => {
      if (data.response) {
        if (data.response.status === 400) {
          return dispatch(createReportFailure({
