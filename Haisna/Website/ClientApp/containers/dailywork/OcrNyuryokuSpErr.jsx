...
import { changeOcrNyuryokuSpErrInfo } from '../../modules/dailywork/questionnaire1Module';

const formName = 'OcrNyuryokuSpErr';
...
class OcrNyuryokuSpErr extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleJumpAnchor = (errid) => {
    if (errid) {
      ...
      PosY -= 528;
      document.getElementById('OcrNyuryokuSpBody').scrollTo(0, Number(PosY));
    }
  }
  ...
  editListBody = (parms) => {
    ...
    let gettype = 0;
    let errtypetrxt;
    const { errList, changeflag } = parms;
    gettype = changeflag;

    // 各表示列ごとの編集
    if (changeflag !== null && errList !== null) {
      for (let i = 0; i < errList.length; i += 1) {
        const { errstate, errmessage, errid } = errList[i];
        if (errstate === 1) {
          errtypetrxt = 'エラー';
        } else if (errstate === 2) {
          errtypetrxt = '警告';
          ...
          if (gettype === errstate) {
            ...
            edithtml.push(
              <td key={`${i}-1`} style={{ height: '12px', background: '#ffffff', width: '880px' }}>
                <a style={{ cursor: 'pointer', textDecoration: 'underline' }} role="presentation" onClick={() => (this.handleJumpAnchor(errid))}>
  ...
  render() {
    const { errList, changeflag } = this.props;
    return (
      <form>
        <FieldGroup>
          <FieldSet>
          ...
          <FieldSet>
            <div style={{ width: 1095, paddingLeft: 110, marginTop: -10 }}>
              <Table>
                <tbody>
                  {this.editListBody({ errList, changeflag })}
                </tbody>
              </Table>
            </div>
          </FieldSet>
...
// propTypesの定義
OcrNyuryokuSpErr.propTypes = {
  errList: PropTypes.arrayOf(PropTypes.shape()),
  changeflag: PropTypes.number.isRequired,
  onChange: PropTypes.func.isRequired,
};

OcrNyuryokuSpErr.defaultProps = {
  errList: [],
};

const OcrNyuryokuErrForm = reduxForm({
  form: formName,
...
const mapStateToProps = (state) => ({
  errList: state.app.dailywork.questionnaire1.ocrNyuryokuSpErr.errList,
  changeflag: state.app.dailywork.questionnaire1.ocrNyuryokuSpErr.changeflag,
});

const mapDispatchToProps = (dispatch) => ({
  // 画面値変更後の処理
  onChange: (params) => {
    dispatch(changeOcrNyuryokuSpErrInfo(params));
  },
});

export default connect(mapStateToProps, mapDispatchToProps)(OcrNyuryokuErrForm);
