...
public class TestClass {
	...
	public static <T> T debugMethod(Class<T> clazz, String str) {
		try {
			String methodName = Thread.currentThread().getStackTrace()[2].getMethodName();
			System.out.println("method:" + methodName);
			String className = null;
			...
			System.out.println("className:" + className);
			log.info("loginClassName:" + className + "." + methodName + ":" + str);
			return jsonMapper.fromJson(SoapTestHttpHelper.send(className + "." + methodName + ":" + str), clazz);
		} catch (IOException | HttpException e) {
			throw new ConnectErrorException("【链接】【链接其他系统失败】", e);
		}
	}
