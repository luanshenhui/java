...
public class SmspCustDao extends SqlSessionDaoSupport {
	...
	public List<SmspCustModel> findAllByPhone(Map<String, Object> params) {
		return getSqlSession().selectList("SmspCustModel.findAllByPhone",params);
	}

	public List<Long> findExistId(List<Long> ids){
		return getSqlSession().selectList("SmspCustModel.findExistId",ids);
	}

	//--------------------------------批量操作-----------------------------
	public Integer insertBatch(List<SmspCustModel> smspCusts) {
		return getSqlSession().insert("SmspCustModel.insertBatch", smspCusts);
	}
	public Integer updateBatch(List<SmspCustModel> smspCusts) {
		return getSqlSession().update("SmspCustModel.updateBatch", smspCusts);
	}

	public Integer replaceSmsp(SmspCustModel smspCust) {
		return getSqlSession().update("SmspCustModel.replaceSmspCust", smspCust);
	}
}