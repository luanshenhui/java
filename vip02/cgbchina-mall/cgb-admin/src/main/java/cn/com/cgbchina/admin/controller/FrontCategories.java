...
public class FrontCategories {
    ...
    public Boolean removeMapping(@RequestParam("fid") Long frontCategoryId, @RequestParam("bid") Long backCategoryId,
        ...
        List<CategoryMapping>  mappingList = findMappingListCache(frontCategoryId);
        if(mappingList ==null){
            return true;
        }

        for(CategoryMapping map:mappingList){
            if(map.getBackCategoryId().equals(backCategoryId)){
                path = map.getPath();
            }
        }
    ...
    private List<CategoryMapping> findMappingListCache(final Long frontCategoryId) {

        Set<String> jsons = this.jedisTemplate.execute(new JedisTemplate.JedisAction<Set<String>>() {
            public Set<String> action(Jedis jedis) {
                return jedis.smembers(KeyUtil.categoryMapping(frontCategoryId));
            }
        });
        List result = Lists.newArrayListWithCapacity(jsons.size());
        if (!jsons.isEmpty()) {
            for (String json : jsons) {
                if (!Strings.isNullOrEmpty(json)) {
                    result.add(JsonMapper.JSON_NON_EMPTY_MAPPER.fromJson(json, CategoryMapping.class));
                }
            }
            return result;
        }
