...
public class PointsImport {
    ...
    public String importGoodsData(MultipartFile file, HttpServletResponse response) {
        ...
        try {
            ...
            //校验数据
            Response<Boolean> results = checkGiftsImport(goodsDtoList);
    ...
    private Response<Boolean> checkGiftsImport(List<GiftsImportDto> goodsDtoList){
        ...
        for(GiftsImportDto goodsDto : goodsDtoList){
            ...
            try{
                //礼品数据校验
                checkGiftModel(failReason,goodsDto.getGoodsModel());
            }catch (Exception e){
    ...
    private void checkGiftModel(GiftsImportDto.failReason failReason , GoodsModel goodsModel){
...
//        goodsModel.getAutoOffShelfTime();// 自动下架时间
        ...
        goodsImport.checkVendorId(failReason,goodsModel.getVendorId(),goodsModel.getProductId(),Contants.BUSINESS_TYPE_JF);//校验供应商编码
