...
public class Spus {
    ...
    @ResponseBody
    public List<GoodsBrandModel> findAllBrands(String channel) {
        Response<List<GoodsBrandModel>> result = brandService.allBrandsByChannel(channel);
        if(!result.isSuccess()) {
    ...
    @RequestMapping(value = "/spus/{spuId}/goods", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Long findGoodsBySpu(@PathVariable("spuId") Long spuId) {
        Response<Spu> spuR = spuService.findById(spuId);
        if(!spuR.isSuccess()) {
            log.error("failed to find spu by id ({}),error code:{}", spuId, spuR.getError());
            throw new ResponseException(500, messageSources.get(spuR.getError()));
        }
        Response<Long> goodsR = goodsService.findGoodsBySpu(spuId);
        if(!goodsR.isSuccess()) {
            log.error("failed to  find goods by id ({}), error code:{}",spuId, goodsR.getError());
            throw new ResponseException(500, messageSources.get(goodsR.getError()));
        }
        return goodsR.getResult();
    }
}
