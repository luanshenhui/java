 ...
 */
@Controller
@RequestMapping("/api/admin/pointsBenefit")
@Slf4j
public class PointsDepositRate {
	...
	public Response<Boolean> create(CouponScaleDto couponScaleDto) {
		try {
			...
			if(!result.isSuccess()){
				log.error("update.couponScale.error，error:{}",result.getError());
				throw new ResponseException(Contants.ERROR_CODE_500, messageSources.get(result.getError()));
			}
			return result;
		} catch (IllegalArgumentException e) {
			log.error("update.couponScale.error，error:{}", Throwables.getStackTraceAsString(e));
			throw new ResponseException(Contants.ERROR_CODE_500, messageSources.get(e.getMessage()));
		} catch (Exception e) {
			log.error("update.couponScale.error，error:{}", Throwables.getStackTraceAsString(e));
			throw new ResponseException(Contants.ERROR_CODE_500, messageSources.get("update.couponScale.error"));
		}
	}
}
