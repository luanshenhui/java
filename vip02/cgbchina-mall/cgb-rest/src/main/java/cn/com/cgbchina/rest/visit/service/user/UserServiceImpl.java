...
import cn.com.cgbchina.rest.visit.vo.user.LoginCardInfoVO;
...
import cn.com.cgbchina.rest.visit.vo.user.QueryUserInfoVO;
...
public class UserServiceImpl implements UserService {
	...
	public LoginResult login(LoginInfo notify) {
		...
		EBankModel<LoginInfoVO> model = SOAPUtils.createEBankModel(idGenarator.genarateSenderSN(), sendVo);
	...
	public RegisterResult register(RegisterInfo notify) {
		String creditCard = "0";
		if (creditCard.equals(notify.getIsCreditCard())) {// true:是信用卡
			// FIXME:这个地方写的不对，不应该在这强行设置
			notify.setCheckCertType(null);
			...
			if (notify.getCardValidPeriod() == null) {
				throw new RuntimeException("[信用卡时cardvalidperiod 必填]");
			}
		} else {
		...
		EBankModel<RegisterInfoVO> model = SOAPUtils.createEBankModel(idGenarator.genarateSenderSN(), sendVo);

		RegisterInfoVO content = model.getContent();
		content.setPinBlockFlag("N");
		content.setCheckType("1");
		content.setPasswordType("3");
		content.setCheckPwdType("0");
		...
		List<LoginCardInfo> infoList = new ArrayList<LoginCardInfo>();
		List<LoginCardInfoVO> list = resultOther.getIAccountList();
		if (list != null && list.size() > 0) {
			for (LoginCardInfoVO loginCardInfoVO : list) {
				LoginCardInfo info = new LoginCardInfo();
				info.setBranchNo(loginCardInfoVO.getBranchNo());
				info.setCardNo(loginCardInfoVO.getCardNo());
				info.setCardType(loginCardInfoVO.getCardType());
				infoList.add(info);
			}
		}
		result.setLoginCardInfos(infoList);
		return result;
	}
	...
	public MobileValidCodeResult getMobileValidCode(MobileValidCode code) {
		...
		EBankModel<MobileValidCodeVO> model = SOAPUtils.createEBankModel(idGenarator.genarateSenderSN(), sendVo);
		Map<String, String> outMap = bankVo2XMLProcessImpl.packing(model, Map.class);
	...
	public UserInfo getCousrtomInfo(QueryUserInfo userInfo) {
		QueryUserInfoVO sendVo = BeanUtils.copy(userInfo, QueryUserInfoVO.class);

		EBankModel<QueryUserInfoVO> model = SOAPUtils.createEBankModel(idGenarator.genarateSenderSN(), sendVo);
		Map<String, String> outMap = bankVo2XMLProcessImpl.packing(model, Map.class);
	...
	public CheckChannelPwdResult checkChannelPwd(ChannelPwdInfo info) {
		...
		EBankModel<ChannelPwdInfoVO> model = SOAPUtils.createEBankModel(idGenarator.genarateSenderSN(), sendVo);
		Map<String, String> outMap = bankVo2XMLProcessImpl.packing(model, Map.class);
