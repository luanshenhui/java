...
public class EBankXml2VoProcessImpl<T> implements PackProcess<String, T> {
	...
	public T packing(String r, Class<T> t) {
		...
		T result = null;
		try {
			ReflectResult<T> reflectResult=new ReflectResult<>(t);
			...
			ReflectUtil.packing(t, reflectResult, "ec", ec);
			ReflectUtil.packing(t, reflectResult, "em", em);
			...
			while (child.hasNext()) {
				...
				if ("iColl".equals(nodeName)) {
					...
					for (Element n : nodes) {
						...
						while (it.hasNext()) {
							...
							ReflectUtil.packing(t, reflectResult, key, value);
						}
					}
				...
				if ("cd".equals(nodeName)) {
					...
					for (Element n : nodes) {
						...
						// ReflectUtil.packing(t, result, key, value);
						// }
							...
							for (Element elt : nodes2) {
								...
								for (Element elt2 : nodes3) {
									...
									if (key1 != null && value1 != null) {
										ReflectUtil.packing(t, reflectResult, key1, value1);
									}
								}
							}
						}
				...
				ReflectUtil.packing(t, reflectResult, nodeName, nodeValue);
