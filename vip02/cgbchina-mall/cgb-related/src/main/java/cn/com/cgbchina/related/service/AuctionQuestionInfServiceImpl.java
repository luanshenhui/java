...
public class AuctionQuestionInfServiceImpl implements AuctionQuestionInfService {
	...
	public Response<Boolean> findLinkUrl(String linkUrl,Integer type) {
		...
		try {
			Integer count = auctionQuestionInfDao.findLinkUrl(linkUrl,type);
			if (count > 0) {
	...
	public Response<List<String>> findLinkUrl() {
		...
		try {
			List<String> urlList = auctionQuestionInfDao.findAllUrl();
			response.setResult(urlList);
		} catch (Exception e) {
			log.error("fail to find all link url ,cause:{}", Throwables.getStackTraceAsString(e));
			response.setError(e.getMessage());
		}
		return response;
	}
