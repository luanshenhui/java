...
public class VendorUserServiceImpl implements VendorUserService {
	...
	@Resource
	VendorRoleMenuDao vendorRoleMenuDao;
	...
	public Response<VendorModel> vendorLogin(String userName, String password) {
		...
		try {
			...
			// 用户是否被停用
			if (!Contants.VENDOR_COMMON_STATUS_0102.equals(vendorModel.getStatus())) {
	...
	public Response<VendorModel> findVendorById(Long vendorUserId) {
		...
		try {
			...
			VendorModel user = vendorDao.findById(vendorUserId);
			// 查询结果
			if (user == null) {
	...
	public Response updatePwdByCode(VendorModel vendorModel) {
		...
		try {
			// 首次登录成功后，更新数据库
			vendorUserManager.updatePwdByCode(vendorModel);
	...
	public Response<VendorModel> findByVendorCode(String code) {
		Response<VendorModel> result = Response.newResponse();
		try {
			// 首次登录成功后，更新数据库
			...
			log.error("failed to find userInfo,cause:{}", Throwables.getStackTraceAsString(e));
			result.setError("vendorUser.modifyPwd.fail");
			return result;
		}
	}
	...
	public Response<List<Long>> queryRole(String id) {
		...
		try {
			...
			//权限菜单list
			List<Long> roleMenuList = Lists.newArrayList();
			//查询角色id的list
			roleIdList = vendorRoleRefDao.getRoleIdByUserId(id);
			if(roleIdList.size() == 0){
				response.setResult(roleMenuList);
				return response;
			}
			//查询权限菜单的list
			roleMenuList = vendorRoleMenuDao.getMenuByRoleId(roleIdList);
			response.setResult(roleMenuList);
			return response;
		} catch (IllegalArgumentException e) {
			log.error("failed to find resource,cause{}", Throwables.getStackTraceAsString(e));
			response.setError(e.getMessage());
			return response;
		} catch (Exception e) {
			log.error("failed to find resource,cause{}", Throwables.getStackTraceAsString(e));
			response.setError("failed to find resource");
			return response;
		}
	}
}
