...
public class EspPublishInfDao extends SqlSessionDaoSupport {
	...
	public List<EspPublishInfModel> findByIds(List ids) {
		return getSqlSession().selectList("EspPublishInfModel.findByIds", ImmutableMap.of("list", ids));
	}

	public Integer updateStatus(EspPublishInfModel espPublishInf) {
		return getSqlSession().update("EspPublishInfModel.updateStatus", espPublishInf);
	}

	public List<EspPublishInfModel> findPublishIsNow(){
		return getSqlSession().selectList("EspPublishInfModel.findPublishIsNow");
	}
}