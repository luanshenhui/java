...
public class CardProServiceImpl implements CardProService {
    ...
    public Response<Map<String, Boolean>> changeCardPro(CardPro cardPro, User user) {
        ...
        try {
            ...
            if (cardPro.getId() == null) {
                cardProManager.insert(cardPro);
            } else {
                cardPro.setDelFlag(Contants.DEL_FLAG_0);
                cardPro.setIsBinding(0);
                cardProManager.update(cardPro);
            }
    ...
    public Response<CardPro> findCardProById(String id) {
        ...
        try {
            ...
            //编辑的时候不适宜从缓存取 modify by niufw
            CardPro cardPro = cardProDao.findById(Long.parseLong(id));
...
//            }
        ...
        try {
            for (String id : parts) {
                ...
                if (cardProModel.isPresent()) {
                    ...
                    cardProManager.update(paramCardPro);
                }
            }
            response.setSuccess(true);
            Map<String, Boolean> result = new HashMap<String, Boolean>();
     ...
     */
    @Override
    public Response<List<CardPro>> findByParams(Map<String,Object> paramMap){
        Response<List<CardPro>> response = Response.newResponse();
        try{
            List<CardPro> dataList = cardProDao.findByParams(paramMap);
            response.setResult(dataList);
        }catch(Exception e){
            log.error("CardProServiceImpl.findByParams.error,cause:{}", Throwables.getStackTraceAsString(e));
            response.setError("CardProServiceImpl.findByParams.error");
            return response;
        }
        return response;
    }
