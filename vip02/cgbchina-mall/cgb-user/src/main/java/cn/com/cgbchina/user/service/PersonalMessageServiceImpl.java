...
public class PersonalMessageServiceImpl implements PersonalMessageService {
	...
	public Response<Pager<PersonalMessageDto>> findAll(@Param("pageNo") Integer pageNo, @Param("size") Integer size,
		...
		try {
			String lastDate = DateTime.now().minusMonths(3).toString(DateHelper.YYYYMMDD);
			paramMap.put("lastDate",lastDate);//消息记录显示3个月内
			// 取得分页数据
			...
			if (pager.getTotal() > 0) {
				...
				for (PersonalMessageModel person : personList) {
					...
					personalMessageDto.setOrderId(person.getOrderId());
					// 判断type是否为空
     ...
     */
	@Override
	public Response<Boolean> updateToRead(Long id){
		Response<Boolean> response = Response.newResponse();
		try {
			Integer count = personalMessageManager.updateToRead(id);
			response.setResult(Boolean.TRUE);
		}catch (Exception e){
			log.error("update message to read error", Throwables.getStackTraceAsString(e));
			response.setError(e.getMessage());
		}
		return response;
	}
