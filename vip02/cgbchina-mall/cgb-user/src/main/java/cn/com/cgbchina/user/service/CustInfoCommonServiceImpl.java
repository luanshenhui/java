...
public class CustInfoCommonServiceImpl implements CustInfoCommonService {
    ...
    public Response<ACustToelectronbankModel> getMaxCardLevelCustInfoByCertNbr(String certNbr,List<String> cardNbrs) {
        Response<ACustToelectronbankModel> response = new Response<ACustToelectronbankModel>();
        try {
            ACustToelectronbankModel aCustToelectronbankModel;
            if (Strings.isNullOrEmpty(certNbr)){
                certNbr = aCardCustToelectronbankDao.findCertNbrByCardNbrs(cardNbrs);
            }
            if (!Strings.isNullOrEmpty(certNbr)){
                aCustToelectronbankModel = aCustToelectronbankDao.findMaxCardLevelInfo(certNbr);
                response.setResult(aCustToelectronbankModel);
            }else{
                response.setError("getMaxCardLevelCustInfoByCertNbr.error");
            }
        } catch (Exception e) {
            log.error("getMaxCardLevelCustInfoByCertNbr.error", Throwables.getStackTraceAsString(e));
            response.setError("getMaxCardLevelCustInfoByCertNbr.error");
        }
        return response;
    }
    ...
    public Response<String> calMemberLevel(String certNbr, String cardLevel, String vipTp) {
        ...
        try {
            ...
            if (Contants.LEVEL_CODE_4.equals(cardLevel)) {
                // 若为顶级卡,返回增值白金/顶级级别
                memberLevel = Contants.MEMBER_LEVEL_DJ;
                ...
                if (cardInfoList != null) {
                    for (ACardCustToelectronbankModel aCardCustToelectronbankModel : cardInfoList) {
                        LocalCardRelateModel localCardRelateModel = localCardRelateDao
                                .findByFormatId(aCardCustToelectronbankModel.getCardFormatNbr());
                        if (localCardRelateModel != null
                                && Contants.INCREMENT_BJ.equals(localCardRelateModel.getProCode())) {
                            // 若为增值白金卡板,返回顶级卡级别
                            memberLevel = Contants.MEMBER_LEVEL_DJ;
                            response.setResult(memberLevel);
                            return response;
                        }
                    }
                }
                ...
                if ("VV".equals(vipTp) || "P1".equals(vipTp)) {
                    ...
                    memberLevel = Contants.MEMBER_LEVEL_TJ;
                }
                ...
                if ("VV".equals(vipTp) || "P1".equals(vipTp)) {
                    ...
                    memberLevel = Contants.MEMBER_LEVEL_VIP;
                } else {
                    // 返回金普卡等级
                    memberLevel = Contants.MEMBER_LEVEL_JP;
                }
            }
            response.setResult(memberLevel);
        } catch (Exception e) {
            log.error("calMemberLevel.error", Throwables.getStackTraceAsString(e));
            response.setError("calMemberLevel.error");
        }
        return response;
    }
