...
public class UserInfoServiceImpl implements UserInfoService {
    ...
    @Resource
    private SMSSendFrequencyDao smsSendFrequencyDao;
...
//        String userId = user.getId();
     ...
     */
    @Override
    public Response<String> checkPin(String mobileNo) {
        Response<String> response = Response.newResponse();
        try {
            //查询上一次发短信记录
            String pinHistory = smsSendFrequencyDao.getKey(mobileNo, "firstRegister");
            String times = "0";
            String nowTime = DateTime.now().toString(dateTimeFormatCal);
            if (StringUtils.isNotEmpty(pinHistory)) {
                //如果返回记录不为空，对返回结果进行拆分
                List<String> smsHistory = Splitter.on("|").trimResults().splitToList(pinHistory);
                String timeHistory = smsHistory.get(0);
                times = smsHistory.get(1);//同一天次数
                LocalDate localDateHistory = dateTimeFormatCal.parseLocalDate(timeHistory);
                LocalDate localDateNow = dateTimeFormatCal.parseLocalDate(nowTime);
                //如果是同一天且
                if (localDateHistory.isEqual(localDateNow)) {
                    //次数大于等于6次，返回错误false
                    if (Integer.parseInt(times) >= 6) {
                        //如果大于等于6次，返回错误
                        response.setResult("more6");
                        return response;
                    }
                    //是同一天，次数小于6
                    //两次请求验证码时间大于等于60秒 并且times参数不变
                    Date historyDate = DateHelper.string2Date(timeHistory, "yyyy-MM-dd HH:mm:ss");
                    Date nowDate = DateHelper.string2Date(nowTime,"yyyy-MM-dd HH:mm:ss");
                    Long diff = nowDate.getTime()-historyDate.getTime();
                    long diffSeconds = diff / 1000;
                    if(diffSeconds < 60){
                        response.setResult("less60");
                        return response;
                    }
                    //不是同一天 初始化次数
                }else {
            ...
            //如果返回记录为空,创建记录
            //如果是同一天且次数小于6次，返回正确true
            //如果不是同一天，返回正确true
            times = String.valueOf(Integer.parseInt(times) + 1);
            String value = nowTime + "|" + times;
            smsSendFrequencyDao.create(mobileNo, "firstRegister", value);
            response.setResult("normal");
            return response;
        } catch (Exception e) {
            log.error("failed to find PIN,cause:{}", Throwables.getStackTraceAsString(e));
            response.setError("PIN.query.fail");
            return response;
        }
    }
}
