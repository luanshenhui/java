...
import cn.com.cgbchina.user.manager.MemberLoginHistoryManager;
...
public class MallUserServiceImpl implements MallUserService {
	...
	@Resource
	private MemberLoginHistoryManager memberLoginHistoryManager;
	@Resource
	...
	public Response<Long> mallLoginLog(String custId, String clientIP, String clientMacAdress, String status) {
		...
		try {
			...
			memberLoginHistoryManager.insert(memberLogonHistoryModel);
	...
	public Response updateLogoutTime(Long id) {
		...
		try {
			memberLoginHistoryManager.updateLogoutTime(id);
	...
	public Response<Boolean> selectLoginLog(String custId) {
		...
		try {
			checkArgument(notNull(custId), "login.custId.empty");
			// 查询登录日志
			Long  memberLogSize= memberLogonHistoryDao.findSuccessByCustId(custId);
			if (memberLogSize != null && memberLogSize> 0) {
