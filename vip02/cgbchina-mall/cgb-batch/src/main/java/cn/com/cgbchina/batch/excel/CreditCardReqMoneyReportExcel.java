 ...
 * 
 * @author xiewl
 * @version 2016年6月16日 上午10:44:16
 */
@Slf4j
@Component
public class CreditCardReqMoneyReportExcel {

	@Value("${creditCardReqMoney.tempPath}")
	private String templatePath; // 报表模版路径

	@Value("${creditCardReqMoney.day.outpath}")
	private String dayOutputPath; // 日报表导出路径
	@Value("${creditCardReqMoney.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${creditCardReqMoney.month.outpath}")
	private String monthOutputPath; // 月报表导出路径

	@Value("${creditCardReqMoney.day.fileName}")
	private String dayReportName;// 日报表文件名
	@Value("${creditCardReqMoney.week.fileName}")
	private String weekReportName;// 周报表文件名
	@Value("${creditCardReqMoney.month.fileName}")
	 ...
	 * 
	 */
	@Report(id = ReportConstant.YG_CREDIT_REQCASH_22, name = "信用卡请款对账明细日报表",
			type = ReportType.DAY, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportCreditCardReqMoneyForDay(String batchDate, String vendorId, String vendorNm) {
		...
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_CREDIT_REQCASH_22,
			...
			log.error("导出信用卡请款对账明细报表（日） 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
	 ...
	 */
	@Report(id = ReportConstant.YG_CREDIT_REQCASH_61, name = "信用卡请款对账明细周报表",
			type = ReportType.WEEK, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportCreditCardReqMoneyForWeek(String batchDate, String vendorId, String vendorNm) {
		Response<Boolean> rps = new Response<>();
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_CREDIT_REQCASH_61,
			...
			log.error("导出信用卡请款对账明细报表（周）处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	 ...
	 */
	@Report(id = ReportConstant.YG_CREDIT_REQCASH_62, name = "信用卡请款对账明细月报表",
			type = ReportType.MONTH, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportCreditCardReqMoneyForMonth(String batchDate, String vendorId, String vendorNm) {
		Response<Boolean> rps = new Response<>();
		try {
			Date startCommDate = BatchDateUtil.parseDate(batchDate);
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_CREDIT_REQCASH_62,
			...
			log.error("导出信用卡请款对账明细报表（月）  处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}
	...
	private String queryDataToExcels(QueryParams queryParam, String outPath, String titleName) throws IOException {
		...
		try {
			...
			List<OrderBatchDto> creditCardReqMoneyDtos = null;
			while (pageNo <= totalPages) {
				...
				creditCardReqMoneyDtos = pager.getData();
				for (OrderBatchDto orderSubBatchDto : creditCardReqMoneyDtos) {
					orderSubBatchDto.setIndex(index++);
				}
				filePath = ExcelUtilAgency.exportExcel(creditCardReqMoneyDtos, templatePath, newOutput, params);
	...
	private Pager<OrderBatchDto> getPager(int pageNo, QueryParams queryParam) {
		...
		Response<Pager<OrderBatchDto>> result = reportManager.findCreditCardReqMoney(queryParam);
		if (!result.isSuccess()) {
