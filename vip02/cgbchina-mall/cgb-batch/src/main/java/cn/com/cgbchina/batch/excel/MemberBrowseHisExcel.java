...
@Slf4j
@Component
public class MemberBrowseHisExcel {
	@Resource
	private BrowseHistoryService browseHistoryService;
	@Autowired
	private ReportManager reportManager;
	@Value("${memberBrowseHistory.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${memberBrowseHistory.month.outpath}")
	private String monthOutputPath; // 月报表导出路径
	@Value("${memberBrowseHistory.week.reportName}")
	private String weekReportName;// 周报表名称
	@Value("${memberBrowseHistory.month.reportName}")
	private String monthReportName;// 月报表名称
	@Value("${memberBrowseHistory.tempPath}")
	private String templatePath;// 模板路径

	@Report(id = ReportConstant.HY_BROWSE_09, name = "会员足迹周报表", type = Report.ReportType.WEEK)
	public Response<Boolean> exportWeekReport(String reportDate) {
		...
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.HY_BROWSE_09, weekReportName,
			...
			log.error("会员足迹报表(周) 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}

	@Report(id = ReportConstant.HY_BROWSE_10, name = "会员足迹月报表", type = Report.ReportType.MONTH)
	public Response<Boolean> exportMonthReport(String reportDate) {
		...
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.HY_BROWSE_10, monthReportName,
			...
			log.error("会员足迹报表(月) 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}
	...
	private String queryDatasAndExport(Map<String, Object> queryParams, String outputPath, String titleName) {
		...
		// 导出数据
		try {
			Response<List<MemberCountBatchDto>> response = browseHistoryService.findTopBrowseHistory(queryParams);
			if (!response.isSuccess()) {
