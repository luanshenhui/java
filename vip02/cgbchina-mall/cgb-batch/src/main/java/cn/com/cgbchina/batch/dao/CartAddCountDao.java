...
public class CartAddCountDao extends RedisBaseDao<String> {
	...
	public Long update(final CartAddCountDto cartAddCountDto) {
		...
		if (cartAddCountDto != null) {
			...
			String malltype = "";
			keyCartAddCount = addDateStr + ":" + weekStr + ":" + cartAddCountDto.getGoodsId() + ":" + malltype;
			if (exist(keyCartAddCount)) {
				...
				if (!Strings.isNullOrEmpty(exsitCartCount)) {
					try {
						cartConutL = Long.valueOf(exsitCartCount) + 1;
					} catch (Exception e) {
						e.printStackTrace();
						cartConutL = new Long(0);
					}
				}
	...
	public String delete(CartAddCountDto cartAddCountDto){
		if (cartAddCountDto != null) {
			...
			String weekStr = String.valueOf(cartAddCountDto.getWeek());
			String malltype = "";
			String  keyCartAddCount = addDateStr + ":" + weekStr + ":" + cartAddCountDto.getGoodsId() + ":" + malltype;
		...
		if (!Strings.isNullOrEmpty(id)) {
			...
			try {
				if (params.length == 5) {
					...
					cartAddCountDto.setCountNum(valueCartAddCount == null ? 0 : Long.valueOf(valueCartAddCount));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
