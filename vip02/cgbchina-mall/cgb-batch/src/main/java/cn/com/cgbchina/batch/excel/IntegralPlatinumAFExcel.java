...
@Slf4j
@Component
public class IntegralPlatinumAFExcel {
	@Autowired
	private ReportManager reportManager;
	@Value("${integralPlatinumAF.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${integralPlatinumAF.month.outpath}")
	private String monthOutputPath; // 月报表导出路径
	@Value("${integralPlatinumAF.week.reportName}")
	private String weekReportName;// 周报表名称
	@Value("${integralPlatinumAF.month.reportName}")
	private String monthReportName;// 月报表名称
	@Value("${integralPlatinumAF.tempPath}")
	private String templatePath;// 模板路径
	@Value("${excel.MaxNum}")
	private int size;

	@Report(id = ReportConstant.JF_PLATINUM_ANNUALFEE_17, name = "积分兑换周报表（白金卡换年费）", type = Report.ReportType.WEEK)
	public Response<Boolean> exportWeekReport(String reportDate) {
		...
		try {
			...
			QueryParams queryParams = ReportTaskUtil.initParams(startDate, endDate,
					ReportConstant.GoodsXids.PLATINUM_ANNUAL_FEE);
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.JF_PLATINUM_ANNUALFEE_17,
			...
			log.error("积分兑换周报表(白金卡换年费)（周）处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}

	@Report(id = ReportConstant.JF_PLATINUM_ANNUALFEE_51, name = "积分兑换月报表（白金卡换年费）", type = Report.ReportType.MONTH)
	public Response<Boolean> exportMonthReport(String reportDate) {
		...
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.JF_PLATINUM_ANNUALFEE_51,
			...
			log.error("积分兑换周报表(白金卡换年费)（月）处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}
