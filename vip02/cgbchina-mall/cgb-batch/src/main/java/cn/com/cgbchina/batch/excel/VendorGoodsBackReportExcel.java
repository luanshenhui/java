 ...
 * 
 * @author xiewl
 * @version 2016年6月16日 上午11:53:59
 */
@Slf4j
@Component
public class VendorGoodsBackReportExcel {

	@Value("${vendorGoodsBack.tempPath}")
	private String templatePath; // 报表模版路径

	@Value("${vendorGoodsBack.day.outpath}")
	private String dayOutputPath; // 日报表导出路径
	@Value("${vendorGoodsBack.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${vendorGoodsBack.month.outpath}")
	private String monthOutputPath; // 月报表导出路径

	@Value("${vendorGoodsBack.day.fileName}")
	private String dayReportName;// 日报表文件名
	@Value("${vendorGoodsBack.week.fileName}")
	private String weekReportName;// 周报表文件名
	@Value("${vendorGoodsBack.month.fileName}")
	 ...
	 * 
	 */
	@Report(id = ReportConstant.YG_VENDOR_RETURN_08, name = "商户退货明细日报表",
			type = ReportType.DAY, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportVendorGoodsBackForDay(String batchDate, String vendorId, String vendorNm) {
		...
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_VENDOR_RETURN_08,
			...
			log.error("商户退货明细（日） 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
	 ...
	 * 
	 */
	@Report(id = ReportConstant.YG_VENDOR_RETURN_12, name = "商户退货明细周报表",
			type = ReportType.WEEK, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportVendorGoodsBackForWeek(String batchDate, String vendorId, String vendorNm) {
		Response<Boolean> rps = new Response<>();
		try {
			Date startCommDate = BatchDateUtil.parseDate(batchDate);
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_VENDOR_RETURN_12,
			...
			log.error("商户退货明细(周) 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}
	 ...
	 * 
	 */
	@Report(id = ReportConstant.YG_VENDOR_RETURN_16, name = "商户退货明细月报表",
			type = ReportType.MONTH, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportVendorGoodsBackForMonth(String batchDate, String vendorId, String vendorNm) {
		Response<Boolean> rps = new Response<>();
		try {
			Date startCommDate = BatchDateUtil.parseDate(batchDate);
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_VENDOR_RETURN_16,
			...
			log.error("商户退货明细(月) 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}
	...
	private String queryDataToExcels(QueryParams queryParam, String outPath, String titleName) throws IOException {
		...
		try {
			...
			List<OrderBatchDto> vendotGoodsBackDtos = null;
			while (pageNo <= totalPages) {
				...
				vendotGoodsBackDtos = pager.getData();
				for (OrderBatchDto orderSubBatchDto : vendotGoodsBackDtos) {
					orderSubBatchDto.setIndex(index++);
				}
				filePath = ExcelUtilAgency.exportExcel(vendotGoodsBackDtos, templatePath, newOutput, params);
	...
	private Pager<OrderBatchDto> getPager(int pageNo, QueryParams queryParam) {
		...
		Response<Pager<OrderBatchDto>> result = reportManager.findVendorGoodsBack(queryParam);
		if (!result.isSuccess()) {
