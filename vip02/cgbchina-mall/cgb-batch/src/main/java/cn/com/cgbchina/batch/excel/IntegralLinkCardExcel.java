...
@Slf4j
@Component
public class IntegralLinkCardExcel {
	@Autowired
	private ReportManager reportManager;
	@Value("${integralLinkCard.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${integralLinkCard.month.outpath}")
	private String monthOutputPath; // 月报表导出路径
	@Value("${integralLinkCard.week.reportName}")
	private String weekReportName;// 周报表名称
	@Value("${integralLinkCard.month.reportName}")
	private String monthReportName;// 月报表名称
	@Value("${integralLinkCard.tempPath}")
	private String templatePath;// 模板路径
	@Value("${excel.MaxNum}")
	private int size;

	@Report(id = ReportConstant.JF_LINK_CARD_30, name = "积分兑换周报表（联通卡）", type = Report.ReportType.WEEK)
	public Response<Boolean> exportWeekReport(String reportDate) {
		...
		try {
			...
			QueryParams queryParams = ReportTaskUtil.initParams(startDate, endDate, ReportConstant.GoodsXids.LINK_CARD);
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.JF_LINK_CARD_30, weekReportName,
			...
			log.error("积分兑换报表（联通卡）(周) 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}

	@Report(id = ReportConstant.JF_LINK_CARD_62, name = "积分兑换月报表（联通卡）", type = Report.ReportType.MONTH)
	public Response<Boolean> exportMonthReport(String reportDate) {
		...
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.JF_LINK_CARD_62, monthReportName,
			...
			log.error("积分兑换报表（联通卡）(月) 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
			rps.setError(Throwables.getStackTraceAsString(e));
			return rps;
		}
	}
	...
	private String queryDatasAndExport(QueryParams queryParams, String outputPath, String titleName) {
		...
		try {
			int index = 1;
			int pageNo = 1;// 页码
			Response<Map<String, String>> response = reportManager.findBankCity();
			if(!response.isSuccess()){
				log.error("获取发卡城市时出错: {}", response.getError());
				throw new RuntimeException(response.getError());
			}
			Map<String, String> bankCities = response.getResult();
			...
			while (pageNo <= totalPages) {
				...
				for (IntegralExchangeDto integralExchangeDto : integralExchangeDtos) {
					...
					// 发卡城市
					String bankCity = "";
					if(!Strings.isNullOrEmpty(integralExchangeDto.getEntryCard())){
						bankCity = bankCities.get(integralExchangeDto.getEntryCard().substring(7, 9));
					}
					integralExchangeDto.setCardCity(bankCity);
				}
