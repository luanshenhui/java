 ...
 * @author xiewl on 2016年5月10日
 */
@Slf4j
@Component
public class VendorSellDetailReportExcel {
	@Value("${vendorSellDetail.tempPath}")
	private String templatePath; // 报表模版路径

	@Value("${vendorSellDetail.day.outpath}")
	private String dayOutputPath; // 日报表导出路径
	@Value("${vendorSellDetail.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${vendorSellDetail.month.outpath}")
	private String monthOutputPath; // 月报表导出路径

	@Value("${vendorSellDetail.day.fileName}")
	private String dayReportName;// 日报表文件名
	@Value("${vendorSellDetail.week.fileName}")
	private String weekReportName;// 周报表文件名
	@Value("${vendorSellDetail.month.fileName}")
	 ...
	 */
	@Report(id = ReportConstant.YG_VENDOR_SELL_30, name = "商户销售明细日报表(供应商)",
			type = ReportType.DAY, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportVendorSellDetailForDay(String batchDate, String vendorId, String vendorNm) {
		...
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_VENDOR_SELL_30, dayReportName,
	 ...
	 */
	@Report(id = ReportConstant.YG_VENDOR_SELL_40, name = "商户销售明细周报表(供应商)",
			type = ReportType.WEEK, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportVendorSellDetailForWeek(String batchDate, String vendorId, String vendorNm) {
		Response<Boolean> rps = new Response<>();
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_VENDOR_SELL_40, weekReportName,
	 ...
	 */
	@Report(id = ReportConstant.YG_VENDOR_SELL_50, name = "商户销售明细月报表(供应商)",
			type = ReportType.MONTH, ordertypeId = Contants.BUSINESS_TYPE_YG)
	public Response<Boolean> exportVendorSellDetailForMonth(String batchDate, String vendorId, String vendorNm) {
		Response<Boolean> rps = new Response<>();
		try {
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.YG_VENDOR_SELL_50,
	...
	private String queryDataToExcels(QueryParams queryParam, String outPath, String titleName) throws IOException {
		...
		try {
			...
			List<OrderBatchDto> vendorSellDtos = null;
			while (pageNo <= totalPages) {
				...
				vendorSellDtos = pager.getData();
				for (OrderBatchDto orderSubBatchDto : vendorSellDtos) {
					orderSubBatchDto.setIndex(index++);
				}
				filePath = ExcelUtilAgency.exportExcel(vendorSellDtos, templatePath, newOutput, params);
	...
	private Pager<OrderBatchDto> getPager(int pageNo, QueryParams queryParam) {
		...
		Response<Pager<OrderBatchDto>> result = reportManager.findVendorSellDetail(queryParam);
		if (!result.isSuccess()) {
