...
@Slf4j
@Component
public class IntegralGiftCodeExcel {
	@Autowired
	private ReportManager reportManager;

	@Value("${integralGiftCode.tempPath}")
	private String templatePath;// 模板路径
	@Value("${integralGiftCode.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${integralGiftCode.week.reportName}")
	private String weekReportName;// 周报表名称
	@Value("${excel.MaxNum}")
	private int size;

	@Report(id = ReportConstant.JF_INTOGROUP_GIFTCODE_20, name = "积分兑换周报表（录入组赠品代码）", type = Report.ReportType.WEEK)
	public Response<Boolean> exportWeekReport(String reportDate) {
		...
		try {
			...
			QueryParams queryParams = ReportTaskUtil.initParams(startDate, endDate, ReportConstant.GoodsXids.INTO_GIFTCODE);
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.JF_INTOGROUP_GIFTCODE_20,
			...
			log.error("积分兑换周报表（录入组赠品代码）处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
