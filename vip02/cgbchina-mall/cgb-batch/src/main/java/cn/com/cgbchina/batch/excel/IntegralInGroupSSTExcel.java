...
@Slf4j
@Component
public class IntegralInGroupSSTExcel {
	@Autowired
	private ReportManager reportManager;
	@Value("${integralSST.week.outpath}")
	private String weekOutputPath; // 周报表导出路径
	@Value("${integralSST.week.reportName}")
	private String weekReportName;// 周报表名称
	@Value("${integralSST.tempPath}")
	private String templatePath;// 模板路径
	@Value("${excel.MaxNum}")
	private int size;

	public Response<Boolean> exportInGroupSSTForWeek(String reportDate) {
		...
		try {
			...
			QueryParams queryParams = ReportTaskUtil.initParams(startDate, endDate, ReportConstant.GoodsXids.INTO_SST);
			...
			// 插入一条报表记录
			ReportModel reportModel = ReportTaskUtil.createReportModel(ReportConstant.JF_INTOGROUP_SST_19, weekReportName,
			...
			log.error("积分兑换周报表（录入组瞬时通） 处理失败..... erro:{}", Throwables.getStackTraceAsString(e));
