...
@Slf4j
@Service
public class OrderCreateMainAndSubServiceImpl implements OrderCreateMainAndSubService{
     ...
     */
    @Override
    public Response<OrderMainModel> createOrderMain_new(OrderMainDto orderMainDto, OrderCommitSubmitDto orderCommitSubmitDto, User user) {
        Response<OrderMainModel> response = Response.newResponse();
        try {
            OrderMainModel orderMainModel = new OrderMainModel();
            Date nowDate = DateTime.now().toDate();
            // 单品总数量
            Integer totalNum = orderMainDto.getTotalNum();
            // 积分总数
            ...
            orderMainModel.setSourceId(orderMainDto.getSourceId());// 订单来源渠道id00: 商城01: callcenter02: ivr渠道03: 手机商城
            orderMainModel.setSourceNm(orderMainDto.getSourceNm());
            ...
            orderMainModel.setCommDate(DateHelper.getyyyyMMdd(nowDate));// 业务日期
            orderMainModel.setCommTime(DateHelper.getHHmmss(nowDate));// 业务时间
            ...
            if (Contants.SUB_ORDER_TYPE_00.equals(goodsType)) {
                orderMainModel.setBpCustGrp(orderCommitSubmitDto.getBpCustGrp());// 送货时间01: 工作日、双休日与假日均可送货02:只有工作日送货（双休日、假日不用送）03:只有双休日、假日送货（工作日不用送货）
                MemberAddressModel memberAddressModel = orderMainDto.getMemberAddressModel();
            ...
            } else if(Contants.SUB_ORDER_TYPE_02.equals(goodsType)){
            ...
            log.error("OrderFQMainService createOrderMain error:{}", Throwables.getStackTraceAsString(e));
            response.setError("OrderFQMainService createOrderMain error");
            return response;
        }
    }
     ...
     */
    @Override
    public Response<OrderMainModel> createJfOrderMain_new(OrderMainDto orderMainDto, OrderCommitSubmitDto orderCommitSubmitDto, User user) {
        Response<OrderMainModel> response = Response.newResponse();
        try {
            OrderMainModel orderMainModel = new OrderMainModel();
            Date nowDate = DateTime.now().toDate();
            ...
            // 商品类型实物o2o虚拟
            String goodsType = orderMainDto.getGoodsType();
            orderMainModel.setOrdertypeId(Contants.BUSINESS_TYPE_JF);// 业务类型idyg:广发商城(一期)jf:积分商城fq:广发商城(分期)
            orderMainModel.setOrdertypeNm(Contants.BUSINESS_TYPE_NM_JF);// 业务类型idyg:广发商城(一期)jf:积分商城fq:广发商城(分期)
            orderMainModel.setIsmerge(orderCommitSubmitDto.getMergeFlag());// 是否合并支付0是1否
            orderMainModel.setCardno(orderCommitSubmitDto.getCardNo());// 卡号
            orderMainModel.setSourceId(orderMainDto.getSourceId());// 订单来源渠道id00: 商城01: callcenter02: ivr渠道03: 手机商城
            orderMainModel.setSourceNm(orderMainDto.getSourceNm());
            orderMainModel.setTotalNum(totalNum);// 商品总数量
            ...
            log.error("OrderJFMainService createJFOrderMain error:{}", Throwables.getStackTraceAsString(e));
            response.setError("OrderJFMainService createJFOrderMain error");
            return response;
        }
    }
        ...
        try {
            ...
            for (ItemModel itemModel : orderMainDto.getItemModels()) {
                for (OrderCommitInfoDto orderCommitInfoDto : orderMainDto.getOrderCommitInfoDtoListMultimap().get(itemModel.getCode())) {
                    ...
                    BigDecimal totalMjPrice = new BigDecimal(0);
                    for (int index = 0; index < orderCommitInfoDto.getItemCount(); index++) {
                        ...
                        subCnt++;
                        orderSubModel.setOrderId(orderMainModel.getOrdermainId() + StringUtils.leftPad(String.valueOf(subCnt), 2, "0"));
                        ...
                        String stagesNum = StringUtils.isBlank(orderCommitInfoDto.getInstalments()) ? "1" : orderCommitInfoDto.getInstalments();
                        if (orderMainDto.getPromotionGoodsPaywayModelMap() != null &&
                                orderMainDto.getPromotionGoodsPaywayModelMap().containsKey(orderCommitInfoDto.getPayWayId())) {
                            PromotionPayWayModel tblGoodsPaywayModel = orderMainDto.getPromotionGoodsPaywayModelMap().get(orderCommitInfoDto.getPayWayId());
                            ...
                            orderSubModel.setPaywayCode(tblGoodsPaywayModel.getPaywayCode() == null ? "" : tblGoodsPaywayModel.getPaywayCode());// 支付方式代码0001: 现金0002: 积分0003: 积分+现金0004:手续费0005: 现金+手续费0006: 积分+手续费0007:积分+现金+手续费
                            String PaywayName = "";
                        ...
                        } else if (orderMainDto.getGoodsPaywayModelMap() != null && orderMainDto.getGoodsPaywayModelMap().containsKey(orderCommitInfoDto.getPayWayId())) {
                            TblGoodsPaywayModel tblGoodsPaywayModel = orderMainDto.getGoodsPaywayModelMap().get(orderCommitInfoDto.getPayWayId());
                            orderSubModel.setGoodsPaywayId(tblGoodsPaywayModel.getGoodsPaywayId());// 商品支付编码
                            orderSubModel.setSpecShopno(tblGoodsPaywayModel.getCategoryNo());// 邮购分期类别码
                            orderSubModel.setCalMoney(tblGoodsPaywayModel.getCalMoney());// 清算总金额
                            String paywayCode = tblGoodsPaywayModel.getPaywayCode();
                        ...
                        if (!orderCommitInfoDto.isFixFlag() && orderCommitInfoDto.getSinglePrice() != 0l) {
                            orderSubModel.setUitdrtamt(new BigDecimal(orderCommitInfoDto.getJfCount()).divide(new BigDecimal(orderCommitInfoDto.getSinglePrice()), 2, BigDecimal.ROUND_DOWN).
                                    divide(new BigDecimal(orderCommitInfoDto.getItemCount()), 2, BigDecimal.ROUND_DOWN));// 本金减免金额
                        } else {
                            orderSubModel.setUitdrtamt(new BigDecimal(0));
                        }
                        ...
                        orderSubModel.setActType(orderCommitInfoDto.getPromotion().getPromType() == null ? null : orderCommitInfoDto.getPromotion().getPromType().toString());// 活动类型
                        orderSubModel.setActId(orderCommitInfoDto.getPromotion().getId() == null ? null : orderCommitInfoDto.getPromotion().getId().toString());// 活动Id
                        orderSubModel.setPeriodId(orderCommitInfoDto.getPromotion().getPeriodId() == null ? null : Integer.parseInt(orderCommitInfoDto.getPromotion().getPeriodId()));// 活动场次
                        ...
                        if (orderCommitInfoDto.isFixFlag() == false && orderCommitInfoDto.getSinglePrice() != 0l) {
                            totalMoney = totalMoney.subtract(new BigDecimal(orderCommitInfoDto.getJfCount()).divide(
                                    new BigDecimal(orderCommitInfoDto.getSinglePrice())).divide(
                                    new BigDecimal(orderCommitInfoDto.getItemCount()), 2, BigDecimal.ROUND_DOWN));
                        }
                        // 满减
                        if (orderCommitInfoDto.getMjDisPrice() != null && orderCommitInfoDto.getMjDisPrice().doubleValue() != 0d) {
                            BigDecimal mjP = new BigDecimal(0);
                            if (index == orderCommitInfoDto.getItemCount() - 1) {
                                mjP = orderCommitInfoDto.getMjDisPrice().subtract(totalMjPrice);
                            } else {
                                mjP = orderCommitInfoDto.getMjDisPrice().divide(new BigDecimal(orderCommitInfoDto.getItemCount().toString()), 2, BigDecimal.ROUND_DOWN);
                                totalMjPrice = totalMjPrice.add(mjP);
                            }
                            totalMoney = totalMoney.subtract(mjP);
                            // 优惠差额
                            orderSubModel.setFenefit(orderCommitInfoDto.getOriPrice().subtract(orderCommitInfoDto.getPrice().subtract(mjP)));
                        } else {
                            // 优惠差额
                            orderSubModel.setFenefit(orderCommitInfoDto.getOriPrice().subtract(orderCommitInfoDto.getPrice()));
                        }
                        ...
                        if (orderCommitInfoDto.getInstalments() != null && !orderCommitInfoDto.getInstalments().equals("0")) {
                            orderSubModel.setInstallmentPrice(totalMoney.divide(new BigDecimal(orderCommitInfoDto.getInstalments()), 2, BigDecimal.ROUND_DOWN));// 分期价格
                        } else {
                        ...
                        orderSubModel.setIntegraltypeId(StringUtils.isEmpty(goodsModel.getPointsType()) ? "1" : goodsModel.getPointsType());// 广发商城积分类型默认1

                        if (null != orderCommitInfoDto.getPromotion().getPromItemResultList()) {
                            orderSubModel.setCostBy(orderCommitInfoDto.getPromotion().getPromItemResultList().get(0).getCostBy());
                        }
                        ...
                        orderDoDetailModel.setStatusId(orderSubModel.getCurStatusId());// 状态代码0301--待付款0316--订单状态未明0308--支付成功0307--支付失败0305--处理中0309--已发货0306--发货处理中0310--已签收0312--已撤单0304--已废单0334--退货申请0327--退货成功0335--拒绝退货申请0380--拒绝签收0381--无人签收0382--订单推送失败
                        orderDoDetailModel.setStatusNm(orderSubModel.getCurStatusNm());// 状态名称
     ...
     */
    @Override
    public Response<OrderSubDetailDto> createJfOrderSubDoDetail_new(OrderCommitSubmitDto orderCommitSubmitDto, OrderMainDto orderMainDto,
        ...
        try {
            ...
            for (ItemModel itemModel : orderMainDto.getItemModels()) {
                for (OrderCommitInfoDto orderCommitInfoDto : orderMainDto.getOrderCommitInfoDtoListMultimap().get(itemModel.getCode())) {
                    ...
                    int itemCount = orderCommitSubmitDto.getGoodsType().equals("01") ? orderCommitInfoDto.getItemCount() : 1;
                    for (int index = 0; index < orderCount; index++) {
                        ...
                        orderSubModel.setGoodsPaywayId(tblGoodsPaywayModel.getGoodsPaywayId());// 商品支付编码
                        orderSubModel.setSpecShopno(tblGoodsPaywayModel.getCategoryNo());// 邮购分期类别码
                        orderSubModel.setCalMoney(new BigDecimal(itemCount).multiply(tblGoodsPaywayModel.getCalMoney()));// 清算总金额
                        String paywayCode = tblGoodsPaywayModel.getPaywayCode();
                        orderSubModel.setPaywayCode(tblGoodsPaywayModel.getPaywayCode() == null ? "" : tblGoodsPaywayModel.getPaywayCode());// 支付方式代码0001: 现金0002: 积分0003: 积分+现金0004:手续费0005: 现金+手续费0006: 积分+手续费0007:积分+现金+手续费
                        String PaywayName = "";
                        if (StringUtils.isNotEmpty(paywayCode)) {
                        ...
                        orderSubModel.setCardtype("W");// 借记卡信用卡标识 未明
                        orderSubModel.setCustCartId(orderCommitInfoDto.getCartId());// 此订单对应的购物车id
                        if (orderCommitInfoDto.getPromotion().getPromType() != null && orderCommitInfoDto.getPromotion().getPromType() == 30) {
                            orderSubModel.setMiaoshaActionFlag(new Integer(1));
                        } else {
                            orderSubModel.setMiaoshaActionFlag(new Integer(0));
                        }
                        ...
                        orderSubModel.setIncTakePrice(orderSubModel.getInstallmentPrice());// 退单时收取指定金额手续费(未用)
                        orderSubModel.setSingleBonus(orderCommitInfoDto.getJfCount() / orderCommitInfoDto.getItemCount());
                        orderSubModel.setBonusTotalvalue(orderCommitInfoDto.getJfCount() / orderCommitInfoDto.getItemCount() * itemCount);
                        ...
                        if (orderCommitSubmitDto.getGoodsType().equals("01")) {
                            ...
                            orderVirtual.setVirtualCardType(orderCommitInfoDto.getCardType());//卡类
                            orderVirtual.setExtend2(orderCommitInfoDto.getCardFormat());//记录卡板
                            ...
                            orderVirtual.setVirtualAllMileage(new Integer(orderSubModel.getGoodsNum().intValue() * mileage));
                            ...
                            orderVirtual.setSerialno(orderCommitInfoDto.getSerialno()); //客户所输入的保单号
