...
public class GoodsImport {
    ...
    private Response<Boolean> checkGoodsImport(List<GiftsImportDto> goodsDtoList,User user){
        ...
        for(GiftsImportDto goodsDto : goodsDtoList){
            ...
            try{
                //礼品数据校验
                checkGoodsModel(failReason,goodsDto.getGoodsModel(),user);
            }catch (Exception e){
            ...
            for(GiftsImportDto.GiftItemDto itemDto : itemdtos){
                ...
                try{
                    checkItemDto(failReasonItem,itemDto, goodsDto.getGoodsModel(),user);
                }catch (Exception e){
    ...
    private void checkGoodsModel(GiftsImportDto.failReason failReason , GoodsModel goodsModel,User user){
        checkSpu(failReason,goodsModel.getProductId());//校验产品
        checkVendorId(failReason,user.getVendorId(),goodsModel.getProductId(),Contants.BUSINESS_TYPE_YG);//校验供应商编码
        ...
        checkMailOrderCode(failReason,goodsModel.getMailOrderCode(),user.getVendorId());//校验邮购分期类别码
    ...
    private void checkItemDto(GiftsImportDto.failReason failReason , GiftsImportDto.GiftItemDto itemDto, GoodsModel goodsModel,User user) {
        ...
        checkInstallmentNumber(failReason,itemDto.getInstallmentNumber(),user.getVendorId());
        if(!Strings.isNullOrEmpty(itemDto.getStagesCode())){
            checkMailOrderCode(failReason,itemDto.getStagesCode(),user.getVendorId());
        }
    ...
    private GiftsImportDto creatGoodsImport(GiftsImportDto giftsImportDto){
        ...
        try {
            ...
            goodsModel.setCreateType(Contants.CREATE_TYPE_1);//供应商创建
    ...
    public void checkVendorId(GiftsImportDto.failReason failReason,String vendorId,Long spuId,String ordertypeId){
        ...
        if (equal(Contants.VENDOR_COMMON_STATUS_0101, vendorInfoDto.getVendorModel().getStatus())) {
            failReason.setFailReason("该供应商已被停用或删除 ");
        }
        ...
        if (!authorizeResponse.isSuccess() || !authorizeResponse.getResult()) {
            failReason.setFailReason("您不可使用该品牌发布商品");
        }
    }
