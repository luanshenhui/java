 ...
 */
public class Seg {
	private final static Logger log = LoggerFactory.getLogger(Seg.class);

	private Map<Character, Map> d = new TreeMap<Character, Map>();
	private Set<Character> stopWords = new HashSet<Character>();

	public void useDefaultDict() throws Exception {
		try {
			Set<String> words = Resources.readLines(Resources.getResource("dic/main.dic"), Charsets.UTF_8,
			...
			Resources.readLines(Resources.getResource("dic/suffix.dic"), Charsets.UTF_8,
					new LineProcessor<Set<Character>>() {
						@Override
						public boolean processLine(String s) throws IOException {
			...
			if (p.containsKey((char) 11)) {
				if (j <= 2) {
					...
					if ((z - i < 2) && stopWords.contains(xsuffix)
							&& (mem2 == null || (mem2 != null && mem2[0] - i > 1))) {
						mem = null;
