        ...
        if (session != null) {
            ...
            if (userId != null) {
                ...
                if (obj != null) {
                    ...
                    user.setMobile(Strings.nullToEmpty((String)(loginResult.get("mobileNo"))));
                    user.setCertNo(Strings.nullToEmpty((String)(loginResult.get("certNo"))));
                    user.setCertType(Strings.nullToEmpty((String)(loginResult.get("certType"))));
                    user.setAddress(Strings.nullToEmpty((String)(loginResult.get("address"))));
                    user.setEmail(Strings.nullToEmpty((String)(loginResult.get("email"))));
                    user.setPhoneNo(Strings.nullToEmpty((String)(loginResult.get("phoneNo"))));
                    user.setZipCode(Strings.nullToEmpty((String)(loginResult.get("zipCode"))));
                    List<HashMap> mapList = (List<HashMap>) loginResult.get("loginCardInfos");
                    if(mapList != null){
                        List<UserAccount> accountList = new ArrayList<>();
                        for (HashMap map : mapList) {
                            UserAccount userAccount = new UserAccount();
                            userAccount.setBranchNo(Strings.nullToEmpty((String)(map.get("branchNo"))));
                            userAccount.setCardNo(Strings.nullToEmpty((String)(map.get("cardNo"))));
                            userAccount.setCardType(
                                    UserAccount.CardType.fromNumber(Integer.valueOf((String)(map.get("cardType")))));
