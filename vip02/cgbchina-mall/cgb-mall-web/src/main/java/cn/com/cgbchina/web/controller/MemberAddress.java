...
public class MemberAddress {
	...
	public List<MemberAddressModel> find() {
		try {
			...
			if (!response.isSuccess()) {
				log.error("member.address.query.error，error:{}", response.getError());
	...
	public Response<Boolean> create(MemberAddressDto memberAddressDto) {
		try {
			...
			memberAddressDto.setCreatOper(user.getCustId());
	...
	public Response<Boolean> update(MemberAddressDto memberAddressDto) {
		try {
			...
			User user = UserUtil.getUser();
			memberAddressDto.setCustId(user.getCustId());
			memberAddressDto.setModifyOper(user.getCustId());
			...
			if (!result.isSuccess()) {
				log.error("update.address.error，error:{}", result.getError());
				throw new ResponseException(Contants.ERROR_CODE_500, messageSources.get(result.getError()));
			}
			return result;
		} catch (IllegalArgumentException e) {
	...
	public Response<Boolean> delete(@PathVariable("componentId") Long id) {
		try {
			...
			if (!result.isSuccess()) {
				log.error("delete.address.error，error:{}", result.getError());
				throw new ResponseException(Contants.ERROR_CODE_500, messageSources.get(result.getError()));
			}
			return result;
		} catch (IllegalArgumentException e) {
	...
	public Response<Boolean> setDefault(Long id, String custId) {
		try {
			...
			if (!result.isSuccess()) {
				log.error("default.address.error，error:{}", result.getError());
				throw new ResponseException(Contants.ERROR_CODE_500, messageSources.get(result.getError()));
			}
			return result;
		} catch (IllegalArgumentException e) {
