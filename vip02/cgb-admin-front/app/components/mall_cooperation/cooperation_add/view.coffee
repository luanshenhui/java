...
#    密码校验及加密结束
    ...
    $("#rate-list").find(".js-rate-info").each ->
      ...
      if !/^\d+(\.\d{0,2})?$/.test $(@).find(".fixedamtFee").val()  #固定金额手续费
        $(@).find(".fixedamtFee").parent().append("<span class=\"fixedamtFee-required-error required\"><i>&times;</i> 请输入最多2位小数的非负数</span>")
        result = false
        return
...
module.exports = MallCooperationAdd
