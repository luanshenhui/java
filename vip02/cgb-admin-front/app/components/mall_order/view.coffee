...
class orderManage
  ...
  constructor: ->
    ...
    @getOrderExportUrl()
  that = this
  orderTransTemplate = App.templates["mall_order_trans"]
  returnDetailTemplate = App.templates["mall_order_return"]

  bindEvent: ->
    that = this
    $(@orderReset).on "click", @reset
    $(".order_manage").on "click", @orderTrans, @transWatch
    $(".order_manage").on "click", @orderRevoke, @revokeWatch
    $(".order_manage").on "click", @orderReturn, @returnOrder
    $(".order_manage").on "click", ".js-batchOutOrder", @exportOrder
...
#查看退货详情
  returnOrder: (event)->
    ...
    $.ajax
      url: Store.context + "/api/admin/OrderManage/backGoods"
      type: "POST"
      data: {
        orderId: id
      }
...
#查看物流
  transWatch: (event)->
    ...
    id = $(@).closest("tr").data("id")
    $.ajax
      url: Store.context + "/api/admin/OrderManage/logistics"
      type: "POST"
      data: {
        orderId: id
      }
...
#查看撤单详情
  ...
  setYear: ->
    ...
    $(".datepicker").datepicker({format:"YYYY-MM-DD",maxDate: new Date(date), yearRange: [1984, year]})
