...
class item
  new Pagination(".paginationsb").total($(".paginationsb").data("total")).show()
  constructor: ->
    @itemNew = ".js-item-new"
  ...
  itemTemplate = App.templates["_item"]
  editItemTemplate = App.templates["item_edit"]
  deleteItemTemplate=App.templates["item_delete"]
  that = this
  ...
  deleteConfirm:(evt)->
    $.ajax
     url:"/aip/item/"+$(".js-delete-confirm").data("id")
     type:"DELETE"
     success:(data)->
       window.location.reload()
module.exports = inside_message
