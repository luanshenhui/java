...
spuManageTemplate = App.templates.category_edit.templates["spu_manage"]
attributeManageTemplate = App.templates.category_edit.templates["attributes_manage"]
categoryItemTemplate = App.templates.category_list.templates["_category_item"]
categoryTemplate = App.templates.category_list.templates["category"]
categoryBind = App.templates.front_category_bind.templates["front_category_bind"]
...
class backCategories
  ...
  addCategory: ->
    ...
    if !/^([\u4e00-\u9fa5]+|[\u4e00-\u9fa5a-zA-Z_0-9]+)$/.test newCategory.find(".js-new-category").val()
      that.tip($(newCategory), "error", "down", "不能含特殊字符")
      $(".tip").css("top", -35)
      newCategory.find(".js-new-category").focus()
    else
      $.ajax
  ...
  updateCategory: ->
    ...
    if name is ""
      ...
      $(".tip").css("top", 40)
      return
    if !/^([\u4e00-\u9fa5]+|[\u4e00-\u9fa5a-zA-Z_0-9]+)$/.test name
      that.tip($(parent), "error", "up", "不能含特殊字符")
      $(".tip").css("top", 40)
      return
