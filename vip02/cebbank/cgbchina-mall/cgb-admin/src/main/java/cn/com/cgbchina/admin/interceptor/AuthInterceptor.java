...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if ("/api/admin/user/modifyPwd".equals(requestURI) && "post".equals(method)) {
			return true;
		}
		//TODO put db data into cache
...
//			String[] strLinks = StringUtils.split(menuMagModel.getLink(), ",");
//			String strAliases = menuMagModel.getAlias();
//			for(String strLink : strLinks) {
//				strPattern.delete(0, StringUtils.length(strPattern));
//				strPattern.append("^");
//				strPattern.append(strLink);
//				strPattern.append(".*$");
//				pattern = Pattern.compile(strPattern.toString());
//				if (pattern.matcher(requestURI).matches()) {
//					if (StringUtils.isNotEmpty(strAliases)) {
//						strPattern.delete(0, StringUtils.length(strPattern));
//						strPattern.append("^.*:");
//						strPattern.append(method);
//						strPattern.append("\\|.*$");
//						pattern = Pattern.compile(strPattern.toString());
//						if (pattern.matcher(strAliases).matches()) {
//							return true;
//						}
//					}
//					else {
...
//		return false;
	...
	private void redirectToLogin(HttpServletRequest request, HttpServletResponse response) throws IOException {
		if (isAjaxRequest(request)) {
			throw new ResponseException(HttpStatus.UNAUTHORIZED.value(), "�û�δ��¼");
		}
