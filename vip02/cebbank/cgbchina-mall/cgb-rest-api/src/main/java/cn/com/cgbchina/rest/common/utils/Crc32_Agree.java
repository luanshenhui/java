...
public class Crc32_Agree {
	private static int[] crc_32_tab = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f,
	...
	/** get the crc of data */
	public static int crc32(byte[] pdata, int off, int len) {
		int crc = 0xffffffff;
		for (int i = 0; i < len; i++) {
			crc = crc_32_tab[(crc ^ pdata[i + off]) & 0xff] ^ (crc >>> 8);
		}
		return ~crc;
	}

	public static int crc32_1(byte[] pdata, int off, int len) {
		long crc = 0;
		long temp1, temp2;
		int i = 0;
		while (len-- != 0) {
			temp1 = (crc >> 8) & 0x00FFFFFFL;
			temp2 = crc_32_tab[((short) crc ^ pdata[i++]) & 0xFF];
			crc = temp1 ^ temp2;
		}
		return (int) crc;
	}

	public static int crc32_2(byte[] pdata, int off, int len) {
		int crc = 0xffffffff;
		for (int i = 0; i < len; i++) {
			crc = crc_32_tab[(crc ^ pdata[i + off]) & 0xff] ^ (crc >>> 8);
		}
	...
	public static void main(String[] args) {
		String sTest = "8200010010116151000|20020523|164900|132900|";
		byte[] byTest = sTest.getBytes();

		int nCrc = Crc32_Agree.crc32_1(byTest, 0, byTest.length);
		System.out.println("crc32_agree = " + nCrc);

		CRC32 crc = new CRC32();
		crc.update(byTest, 0, byTest.length);
		System.out.println("crc32_java = " + crc.getValue());
	}
