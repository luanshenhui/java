...
public class DataTransfer {
	...
	private byte[] natpRecvFile(byte[] allDatas, DataInputStream din) throws UnknownHostException, IOException {
		...
		return allDatas;
		/*
		 * String hostIp = queryHostIp(host); int port = queryHostPort(host); Socket hostSock = new Socket(hostIp,
		 * port);
		 * 
		 * // 进行代理服务 DataInputStream hostInput = new DataInputStream(hostSock.getInputStream()); DataOutputStream
		 * hostOuput = new DataOutputStream(hostSock.getOutputStream()); DataOutputStream clientOutput = new
		 * DataOutputStream(getOutputStream()); hostOuput.writeInt(allDatas.length); hostOuput.write(allDatas);
		 * hostOuput.flush(); while (true) { try { int len = hostInput.readInt(); byte[] recv = new byte[len];
		 * readFully(hostInput, recv); clientOutput.writeInt(len); clientOutput.write(recv); clientOutput.flush(); len =
		 * din.readInt(); recv = new byte[len]; readFully(din, recv); hostOuput.writeInt(len); hostOuput.write(recv);
		 * hostOuput.flush(); } catch(IOException e){ if( e.toString().equals("java.io.EOFException") ) return null;
		 * else throw e; } }
		 */
	}
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			if (transfer.getProtoclVersion() == 2)
				continue;
			// System.out.println(StringTool.toHexTable(recv));
