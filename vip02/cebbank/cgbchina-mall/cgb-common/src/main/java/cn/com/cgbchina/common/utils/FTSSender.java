...
public class FTSSender {
	...
	private FTSSender(String host, int port, int timeout) {
		this.host = host;
		this.port = port;
	...
	public static FTSSender newInstance(String host, int port) {
		return new FTSSender(host, port, 60);
	}

	public static FTSSender newInstance(String host, int port, int timeout) {
		// 先检查实例是否存在，如果不存在才进入下面的同步块
		return new FTSSender(host, port, timeout);
	}
