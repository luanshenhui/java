  ...
  nextCategory: ->
    ...
    if categoryData.hasChild is true
      $.get Store.context + "/api/admin/backCategories/#{categoryData.id}/children", (data) ->
        categoryTemplate = popCategory = App.templates.fixed_category.templates["_fixed_category"]({extras: {"level": parseInt(categoryData.level) + 1, "parentId": categoryData.id}, data: data})
