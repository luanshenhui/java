...
import com.cebbank.ccis.cebmall.common.controller.CaptchaGenerator;
import com.cebbank.ccis.cebmall.common.controller.LoginRedirector;
import com.cebbank.ccis.cebmall.user.dto.LoginDto;
import com.cebbank.ccis.cebmall.user.dto.UserInfoDto;
import com.cebbank.ccis.cebmall.user.model.UserInfoModel;
import com.cebbank.ccis.cebmall.user.service.UserInfoService;
...
public class AdminUsers {
	...
	public LoginDto login(@RequestParam("userName") String userName, @RequestParam("password") String password,
		...
		loginDto.setUrl("/console/index");
		Response<UserInfoDto> userInfo = userInfoService.findUserById(userName);
...
//		}
	...
	public void modifyManagerPwd(@RequestParam("userName") String userName,
		...
		if (Strings.isNullOrEmpty(passwordFirst) || Strings.isNullOrEmpty(passwordNew)
			...
			throw new ResponseException(500, messageSources.get("param.not.null.fail"));
		}
		Response<UserInfoModel> userInfo = userInfoService.findUserInfoById(userName);
		if (!userInfo.isSuccess()) {
			log.error("user.password.incorrect,userId:{}", userName);
		...
		try {
			...
			if (!res.isSuccess()) {
				...
				throw new ResponseException(500, messageSources.get(res.getError()));
			}
			// 存储信息
			User user = userInfo.getResult();
			request.getSession().setAttribute(CommonConstants.SESSION_USER_ID, user.getId());
	...
	public String assign(@PathVariable("id") String id, UserInfoModel userInfoDto,HttpServletRequest request) {
		...
		log.error("failed to assign userInfo {},error code:{}", userInfoDto,"");
		throw new ResponseException(500, messageSources.get(""));
	}
	...
	public Boolean pwdTimeout(HttpServletRequest request) {
		...
		try {
			...
			throw new ResponseException(500, messageSources.get("adminUser.find.fail"));
		}
	}
	...
	public String getRandomCode(HttpServletRequest request) {
		try {
			...
			throw new ResponseException(500, messageSources.get("vendorUser.logout.fail"));
		}
