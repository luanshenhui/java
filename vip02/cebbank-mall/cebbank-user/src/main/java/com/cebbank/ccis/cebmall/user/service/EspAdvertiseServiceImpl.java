...
import com.cebbank.ccis.cebmall.user.dto.EspAdvertiseDto;
import com.cebbank.ccis.cebmall.user.manager.EspAdvertiseManager;
...
//	BackCategoryService backCategoriesService;
	...
	public Response<Boolean> create(EspAdvertiseModel espAdvertiseModel, String partitionsKeyword,
		...
		try {
			...
			if ("JF".equals(espAdvertiseModel.getOrdertypeId())
					&& "3".equals(espAdvertiseModel.getLinkType())) {
				espAdvertiseModel.setKeyword(partitionsKeyword);
			}
			if ("JF".equals(espAdvertiseModel.getOrdertypeId())
					&& "4".equals(espAdvertiseModel.getLinkType())) {
			...
			throw new ResponseException(500, "default.create.error");
		}
		return response;
	}
	...
	public Response<Boolean> update(String id, EspAdvertiseModel espAdvertiseModel, String partitionsKeyword,
		...
		try {
			...
			espAdvertiseModel.setPublishStatus("21");// 发布状态 21是等待发布
			espAdvertiseModel.setIsStop("0");// 是否启用标志 0-停止，1-启用
			espAdvertiseModel.setDelFlag(0);
			//清空非所需数据
			...
			log.error("create.error,error code:{}", Throwables.getStackTraceAsString(e));
			throw new ResponseException(500, "default.update.error");
		}
		return response;
	}
	...
	public Response<Boolean> updateAdvetiseStatus(EspAdvertiseModel espAdvertiseModel) {
		Response<Boolean> response = new Response<Boolean>();
		try {
			espAdvertiseModel.setPublishStatus("00");// 00：已发布
            espAdvertiseModel.setIsStop("1");// 1 启用
