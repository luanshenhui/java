	...
	ac.prototype = {$c:0, init:0, T:0, i:-1, d:1, last_value:"", custom_uri:"", initialize:function (t, f, options) {
		...
		var ml = function (n) {
			...
			if (loaded) {
				...
				document.write("<ol id='" + n + "_" + x + "' style='position:absolute;left:-9999;top:-9999px' class='" + x + "'></ol>");
			}
			var e = $(n + "_" + x);
			return e;
		};
		...
		document.write("<div id='" + id + "' class='autocomplete_icon' style='position:absolute;width:11px;height:11px;left:-99px;top:-99px" + top + "'></div>");
		var icon = $(id);
