...
public class Send {
    ...
    public static void sendMessage(Session session, MessageProducer producer)  
        ...
        for (int i = 1; i <= SEND_NUMBER; i++) {  
            TextMessage message = session.createTextMessage("ActiveMq 发送的消息" + i+DateUtil.DateToString(new Date(), "yyyy-MM-dd HH:mm:ss"));  
            // 发送消息到目的地方  
            System.out.println("发送消息：" + "ActiveMq 发送的消息" + i +DateUtil.DateToString(new Date(), "yyyy-MM-dd HH:mm:ss"));  
