 ...
 * AdminRole entities. Transaction control of the save(), update() and delete()
...
public class AdminRoleDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory
			.getLogger(AdminRoleDAO.class);
	...
	public AdminRole findById(gfvh.AdminRoleId id) {
		log.debug("getting AdminRole instance with id: " + id);
		try {
			AdminRole instance = (AdminRole) getSession().get("gfvh.AdminRole",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(AdminRole instance) {
		log.debug("finding AdminRole instance by example");
		try {
			List results = getSession().createCriteria("gfvh.AdminRole").add(
	...
	public AdminRole merge(AdminRole detachedInstance) {
		...
		try {
			AdminRole result = (AdminRole) getSession().merge(detachedInstance);
