...
//					request.setAttribute("title", expFoodProdCheckVo.getCheck_menu_type()+": "+list.get(0).getCheck_title());
	    ...
	    public String toTextViewQt(HttpServletRequest request,ExpFoodProdCheckVo expFoodProdCheckVo){
	    	...
	    	VideoEventModel v1=new VideoEventModel();
	    	VideoEventModel v2=new VideoEventModel();
	    	try {
	    		if(StringUtils.isNotEmpty(request.getParameter("type")) && request.getParameter("type").equals("2")){//行政检查
	    			//TODO
	    			param.put("DocType", "V_SP_A_C_QT");
	    			v1.setProcType("O_1_1");
	    			v2.setProcType( "O_1_2");
	    		}else if(StringUtils.isNotEmpty(request.getParameter("type")) && request.getParameter("type").equals("3")){//行政检查
	    			param.put("DocType", "V_SP_A_D_QT");
	    			v1.setProcType("O_2_1");
	    			v2.setProcType("O_2_2");
	    		}else{//许可
	    			param.put("DocType", "V_SP_F_D_QT");
	    			v1.setProcType("O_3_1");
	    			v2.setProcType("O_3_2");
	    		}
	    		param.put("ProcMainId", expFoodProdCheckVo.getApply_no());
	    		v1.setProcMainId(expFoodProdCheckVo.getApply_no());
	    		v2.setProcMainId(expFoodProdCheckVo.getApply_no());
	    		List<CheckDocModel> list=expFoodPOFService.getQtOption(param);
				List<VideoEventModel> videolist1= expFoodPOFService.selectVideoEventModelList(v1);
				List<VideoEventModel> videolist2= expFoodPOFService.selectVideoEventModelList(v2);
...
//	    		doclist=list;
				request.setAttribute("list", list);
				request.setAttribute("videolist1", videolist1);
				request.setAttribute("videolist2", videolist2);
			} catch (Exception e) {
		...
		public void downPdfQt(HttpServletRequest request,
				...
				try {
					...
					document.add(createParagraph("其他",new Font(BaseFont.createFont("STSong-Light",
								"UniGB-UCS2-H", BaseFont.NOT_EMBEDDED), 13,
								Font.NORMAL),Element.ALIGN_CENTER));
					...
					/***********************正式内容************************/
					//TODO
					List<CheckDocModel> doclist=new ArrayList<CheckDocModel>();
					StringBuffer str = new StringBuffer();
					this.getQtModel(request,expFoodProdCheckVo,doclist,str);
...
//						String name=this.getFile(doclist.get(i));
		...
		private void getQtModel(HttpServletRequest request,Object obj, List<CheckDocModel> doclist, StringBuffer str) {
		 	Map<String,Object> param = new HashMap<String, Object>();
	    	VideoEventModel v1=new VideoEventModel();
	    	VideoEventModel v2=new VideoEventModel();
	    	try {
	    		...
	    		ExpFoodProdCheckVo expFoodProdCheckVo=(ExpFoodProdCheckVo) obj;
	    		param.put("ProcMainId", expFoodProdCheckVo.getApply_no());
	    		v1.setProcMainId(expFoodProdCheckVo.getApply_no());
	    		v2.setProcMainId(expFoodProdCheckVo.getApply_no());
	    		List<CheckDocModel> list=expFoodPOFService.getQtOption(param);
	    		...
	    		doclist.addAll(list);
	    		String fileTypeName = this.getfileFont(videolist1,videolist2);
	    		str.append(fileTypeName);
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}
		...
		private String getfileFont(List<VideoEventModel> videolist1,
				List<VideoEventModel> videolist2) {
			String fileName="";
			for(VideoEventModel v:videolist1){
				fileName+="查看图片文件\n";
			}
			for(VideoEventModel v:videolist2){
				fileName+="查看视频文件\n";
			}
			if(fileName.equals("")){
				return "无文件";
