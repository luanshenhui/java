     ...
     */
 	...
 	public String doApproval(HttpServletRequest request,
    	...
    	try{
    		...
    		CheckDocsRcdModel ck = new CheckDocsRcdModel();
    		ck.setProcMainId(request.getParameter("license_dno"));
    		List<CheckDocsRcdModel> lks = origPlaceFlowService.getOptionList(ck);
    		
    		if(null != lks && lks.size()>0){
    			List<WaringBaseBean> lw = new ArrayList<>();
    			for (CheckDocsRcdModel checkDocsRcdModel : lks) {
    				lw.add(new WaringBaseBean(checkDocsRcdModel.getDocType(), "", checkDocsRcdModel.getProcMainId(), "", user.getOrg_code()));
    			}
    ...
    @RequestMapping("/doSubmit")
  	public String doSubmit(HttpServletRequest request){
    	...
    	try{
    		...
    		WarningUtils.insertWaringEvent(request, "V_PT_H_L_ZDY_4", license_dno, license_dno, user.getOrg_code(), false);
        } catch (Exception e) {
...
//			Boolean bo=ElectronicSealUtil.getElectronicSealPDF(fileName, "3");
	...
	public String addPeson2(ExpFoodProdPsnRdmDTO dto, Model model,
        ...
        if(queryType !=null && queryType.equals("query")){
        	...
        	// 初始化领导随机人员
//        	String approval_users_name = new String(request.getParameter("approval_users_name").getBytes("ISO-8859-1"),"utf-8");
        	String approval_users_name = request.getParameter("approval_users_name");
        	if(approval_users_name!=null && !approval_users_name.equals("")){
				...
				for (int k = 0; k < arr.length; k++) {
					if(k!=0){
						...
						pseonInfoDto.setBel_scope(pseonInfoDto.getBel_scope_name());
	...
	public String toPsyEdit(HttpServletRequest request) throws Exception {
		...
		String a = request.getParameter("sczry");
//		a = new String(a.getBytes("ISO-8859-1"),"UTF-8");
		String [] ls = a.split(",");
		List<String> sczry = new ArrayList<>(10);
		for (String name : ls) {
			sczry.add(name+"("+licenseDecDbService.fetchOrgNameByPsnName(name)+")");
		}
		
		String b  = StringUtils.isNotEmpty(request.getParameter("approval_users_name2")) ? request.getParameter("approval_users_name2") : a;
//		b = new String(a.getBytes("ISO-8859-1"),"UTF-8");
		List<String> bs = new ArrayList<>(10);
		String [] lls = b.split(",");
		for (String name : lls) {
			bs.add(name+"("+licenseDecDbService.fetchOrgNameByPsnName(name)+")");
		}
		
		request.setAttribute("sczry",StringUtils.strip(sczry.toString(), "[]") );
//		String approval_users_name2 = request.getParameter("approval_users_name2");
		request.setAttribute("approval_users_name2", StringUtils.strip(bs.toString(), "[]"));
		return "xk/psyEdit";
	}
	...
	public String submitDoc(HttpServletRequest request,CheckDocsRcdModel checkDocsRcd,RedirectAttributes attr) {
		try {
	    	UserInfoDTO user = (UserInfoDTO) request.getSession().getAttribute(
					Constants.USER_KEY);
	    	checkDocsRcd.setDecUser(user.getId());
			if(com.dpn.ciqqlc.common.util.StringUtils.isEmpty(checkDocsRcd.getDocId())){
				commonServer.insertDocs(checkDocsRcd);

				if("D_SQ_SHRY".equals(checkDocsRcd.getDocType())){
					
					WarningUtils.insertWaringDoc(request, checkDocsRcd);
					CheckDocsRcdModel model=new CheckDocsRcdModel();
					model.setProcMainId(request.getParameter("license_dno"));
					model.setDocType("D_SQ_SHRY");
					List<CheckDocsRcdModel> list = origPlaceFlowService.getOptionList(model);
	...
	public String submitslDoc(HttpServletRequest request,CheckDocsRcdModel checkDocsRcd,RedirectAttributes attr) {
		try {
			if(com.dpn.ciqqlc.common.util.StringUtils.isEmpty(checkDocsRcd.getDocId())){
				commonServer.insertDocs(checkDocsRcd);
				System.out.println("D_SL:"+checkDocsRcd.getDocType());
			}else{
				commonServer.updateDocs(checkDocsRcd);
			}
			attr.addFlashAttribute("msg","success");
