...
<script type="text/javascript"> 
		...
		function pageUtil(page) {
			$("#xk_form").attr("action", "<%=request.getContextPath()%>/xk/cxList?page="+page);
			$("#xk_form").submit();
		}
	...
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">口岸卫生许可证核发</a> &gt; <a
				href="${cxt}<%=request.getContextPath()%>/xk/cxList">撤销审批</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>	
<!-- 		<div class="title">现场评审查询</div> -->
		<div class="search">
			<div class="main">
				<form action="<%=request.getContextPath()%>/xk/cxList"  method="post" id="xk_form">
					<table class="table_search" id="aa">
						...
						<tr>
							...
							<td style="padding-top:8px;width: 250px;" align="left">撤销审批状态:</td>
							<td></td>
						</tr>
						<tr>
							<td align="left" >
							...
							<td align="left">
								<select name="status" id="review_result">
									<option value="('6','11')">全部</option>
									<option value="('15')" <c:if test="${status == \"('15')\"}" >selected="selected"</c:if>>已审批</option>
									<option value="('6','11')" <c:if test="${status == \"('6','11')\"}" >selected="selected"</c:if>>未审批</option>
								</select>
							</td>
							<td></td>						
						</tr>
						            ...
						            <td>
						            	<c:if test="${row.status == 6}">
						            		<select id="zz_${row.id}" onchange="gzs('${row.id}')">
						            			<option value="1">检验检疫机构工作人员滥用职权、玩忽职守作出准予卫生许可决定</option>
												<option value="2">超越法定职权作出卫生许可决定</option>										
												<option value="4">对不具备申请资格或者不符合法定条件的申请人准予卫生许可</option>
												<option value="5">申请人以欺骗、贿赂等非法手段骗取卫生许可证</option>
												<option value="6">依法可以撤销卫生许可的其他情形</option>				
						            		</select>
					            		</c:if>
					            		<c:if test="${row.status == 11 && empty row.cx_result}">
						            		<select id="zz_${row.id}" onchange="gzs('${row.id}')">
												<option value="3" >违反法定程序作出卫生许可决定</option>												
						            		</select>
					            		</c:if>
					            		
					            		<c:if test="${row.status == 15 }">
						            			<c:if test ="${row.cx_result == 1}">检验检疫机构工作人员滥用职权、玩忽职守作出准予卫生许可决定</c:if>
												<c:if test ="${row.cx_result == 2}">超越法定职权作出卫生许可决定</c:if>
												<c:if test ="${row.cx_result == 3}">违反法定程序作出卫生许可决定</c:if>
												<c:if test ="${row.cx_result == 4}">对不具备申请资格或者不符合法定条件的申请人准予卫生许可</c:if>
												<c:if test ="${row.cx_result == 5}">申请人以欺骗、贿赂等非法手段骗取卫生许可证</c:if>				
						            			<c:if test ="${row.cx_result == 6}">依法可以撤销卫生许可的其他情形</c:if>
					            		</c:if>
					            		<br/>
										...
										<a href = "#" id="a_sdhz_${row.id}" style="display: none" onclick="winShow('${row.license_dno}','D_SDHZ','${row.comp_name}')">送达回证</a>
						            </td>
						            <td>
