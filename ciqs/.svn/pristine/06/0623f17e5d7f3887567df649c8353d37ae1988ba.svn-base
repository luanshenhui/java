...
import com.dpn.ciqqlc.common.util.WaringBaseBean;
...
public class AppServerController extends FormPdf {
	...
	public Map<String, Object> showTransportsApp() {
		...
		try {
			...
			results.put("results", resultData);
		} catch (Exception e) {
	...
	public Map<String, Object> saveText(HttpServletRequest request,
		...
		int bb=0;//不符合项报告
		try {
			List<WaringBaseBean> lw = new ArrayList<>();
			for (ExpFoodProdCheckDto dto : expFoodProdCheckDto) {
				if(null!=dto.getCheck_result() && dto.getCheck_result().equals("2") && 
						null!=dto.getCheck_proc_type() && dto.getCheck_proc_type().contains("V_SP_A_C_")){
					b++;
				}
				if(null!=dto.getCheck_result() && !dto.getCheck_result().equals("") &&
						null!=dto.getCheck_proc_type() && dto.getCheck_proc_type().contains("V_SP_A_C_")){
					a++;
				}
				if(null!=dto.getCheck_result() && dto.getCheck_result().equals("2") && 
						null!=dto.getCheck_proc_type() && dto.getCheck_proc_type().contains("V_SP_A_D_")){
					bb++;
				}
				if(null!=dto.getCheck_result() && !dto.getCheck_result().equals("") &&
						null!=dto.getCheck_proc_type() && dto.getCheck_proc_type().contains("V_SP_A_D_")){
...
//					commonServer.deleteEvent(v);
			...
			if(!expFoodProdCheckDto.isEmpty()){
				UserInfoDTO user = (UserInfoDTO) request.getSession().getAttribute(Constants.USER_KEY);
				if(a>0){
					lw.add(new WaringBaseBean("V_SP_A_C_JDB_ZDY", "", expFoodProdCheckDto.get(0).getApply_no(), "", user.getOrg_code()));
					WarningUtils.inserRule_ref(request, lw);
				}
				if(b>0){
					lw.add(new WaringBaseBean("V_SP_A_C_BFX_ZDY", "", expFoodProdCheckDto.get(0).getApply_no(), "", user.getOrg_code()));
					WarningUtils.inserRule_ref(request, lw);
				}
				if(aa>0){
					lw.add(new WaringBaseBean("V_SP_A_D_JDB_ZDY", "", expFoodProdCheckDto.get(0).getApply_no(), "", user.getOrg_code()));
					WarningUtils.inserRule_ref(request, lw);
				}
				if(bb>0){
					lw.add(new WaringBaseBean("V_SP_A_D_BFX_ZDY", "", expFoodProdCheckDto.get(0).getApply_no(), "", user.getOrg_code()));
					WarningUtils.inserRule_ref(request, lw);
				}
...
//					.selectCompanyPsn(model);
		...
		try {
			...
			if (VideoEventModel != null) {
				...
				for (VideoEventModel eventModel : VideoEventModel) {
					...
					WaringBaseBean wb = new WaringBaseBean();
					wb.setFileType(eventBean.getFileType());
					wb.setPortOrgCode(user.getOrg_code());
					wb.setProcMainId(eventBean.getProcMainId());
					wb.setProcType(eventBean.getProcType());
					wb.setUrlParam(null);
					lw.add(wb);
				}
				WarningUtils.inserRule_ref(request, lw);
	...
	public Map<String, Object> updateDealStatus(HttpServletRequest request,
		...
		try {
			...
			if (!StringUtils.isBlank(deal_type)) {
				updateMailObjCheckStatus(request,package_no, deal_type);
			}
			map.put("status", "OK");
			map.put("results", "成功");
			return map;
	...
	public Map<String, Object> xhListUpdateStatus(HttpServletRequest request,String packageNos,String dealTypes) {
		Map<String, Object> map = new HashMap<String, Object>();
		...
		try {
			if (packageNos.indexOf(',') != -1 && dealTypes.indexOf(',') != -1) {
				...
				for (int i = 0; i < packageNoArr.length; i++) {
					// 更新状态
					updateMailObjCheckStatus(request,packageNoArr[i], dealTypeArr[i]);
					// 保存集中销毁id
				...
				if (packageNos != null && !"".equals(packageNos)
					...
					updateMailObjCheckStatus(request,packageNos, dealTypes);
	 ...
	 * @return
	 */
	public void updateMailObjCheckStatus(HttpServletRequest request, String package_no, String deal_type) {
//TODO
		...
		if (deal_type.equals("FX")) {
			...
			if (flag) {
				mailObjCheckServ.updateStatus(request,fxmap);
			}
			...
			if (flag) {
				mailObjCheckServ.updateStatus(request,txmap);
			}
			...
			if (flag) {
				mailObjCheckServ.updateStatus(request,xhmap);
			}
		...
		if (deal_type.equals("CYSJ")) {
			...
			if (flag) {
				mailObjCheckServ.updateStatus(request,cysjmap1);
			}
		...
		if (deal_type.equals("CYSJ")) {
			...
			if (flag) {
				mailObjCheckServ.updateStatus(request,cysjmap2);
			}
		...
		if (deal_type.equals("CYSJ")) {
			...
			if (flag) {
				mailObjCheckServ.updateStatus(request,cysjmap3);
			}
		}
	...
	public Map<String, Object> moreDocs(HttpServletRequest request,
		...
		try {
			...
			//TODO
			WarningUtils.insertWaringDocList(request,checkDocsRcdModel);
			map.put("status", "OK");
			map.put("results", "成功");
			return map;
		} catch (Exception e) {
...
//				commonServer.updateDocsByApplyNo(checkDocsRcdModel);
				if(checkDocsRcdModel.getDocType().equals("V_SP_F_D_QT") || checkDocsRcdModel.getDocType().equals("V_SP_A_C_QT")
					...
					commonServer.insertDocs(checkDocsRcdModel);
					//TODO
					WarningUtils.insertWaringDoc(request,checkDocsRcdModel);
					map.put("status", "OK");
					map.put("results", "成功");
				}
			}else{
...
//			resultMap.put("results", dbServ.findListApp(form));
	...
	public Map<String, Object> saveMailResult(HttpServletRequest request,
		...
		try {
			...
			for(MailSteamerResultDTO model:dto){
				...
				WaringBaseBean wb = new WaringBaseBean();
				wb.setPortOrgCode(user.getOrg_code());
				wb.setProcMainId(model.getDec_master_id());
				wb.setProcType("V_JC_T_Y_JYJGBGD_ZDY");
				wb.setUrlParam(null);
				lw.add(wb);
			}
			WarningUtils.inserRule_ref(request, lw);
	...
	public Map<String, Object> saveMailChkDeal(HttpServletRequest request,
		...
		try {
			...
			for (int i = 0; i < mailSteamerChkDealModel.size(); i++) {
				...
				WaringBaseBean wb = new WaringBaseBean();
				wb.setPortOrgCode(user.getOrg_code());
				wb.setProcMainId(mailSteamerChkDealModel
						.get(i).getDec_master_id());
				wb.setProcType("V_JC_T_Y_JYCL_ZDY");
				wb.setUrlParam(null);
				lw.add(wb);
			}
			WarningUtils.inserRule_ref(request, lw);
	...
	public Map<String, Object> saveMailSamp(HttpServletRequest request,
		...
		try {
			...
			for (int i = 0; i < mailSteamerSampDTO.size(); i++) {
				...
				WaringBaseBean wb = new WaringBaseBean();
				wb.setPortOrgCode(user.getOrg_code());
				wb.setProcMainId(mailSteamerSampDTO.get(i).getDec_master_id());
				wb.setProcType("V_JC_T_Y_CY_ZDY");
				wb.setUrlParam(null);
				lw.add(wb);
			}
			WarningUtils.inserRule_ref(request, lw);
...
//		dto.setPort_org_code(user.getOrg_code());
		try {
			...
			if (count > 0) {
				...
				WarningUtils.insertWaringEvent(request, "DOC_V_CF_J_Y_1", dto.getPunish_id(), null, user.getOrg_code(), true);
			}
			map.put("status", "OK");
			map.put("results", "无数据");
			return map;
