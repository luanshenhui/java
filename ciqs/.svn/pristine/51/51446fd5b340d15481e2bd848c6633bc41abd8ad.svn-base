...
<script type="text/javascript"> 
		...
		function pageUtil(page) {
			$("#xk_form").attr("action", "${ctx}/work/flow?page="+page);
			$("#xk_form").submit();
		}
	    jQuery(function() {
...
<%@ include file="/common/headMenu_Xk.jsp"%>
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">口岸卫生许可证核发</a> &gt; <a
				href="<%=request.getContextPath()%>/work/flow">工作流程查询</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>
		<div class="search">
			<div class="main">
				<form action="<%=request.getContextPath()%>/work/flow"  method="post" id="xk_form">
					...
					<table>
						...
						<c:if test="${not empty alertList }"><c:forEach items="${alertList}" var="row">
							<tr>
								...
								<td><a href = "#" onclick="cardShow('${row.license_dno}','${row.id}')">查看</a></td>
								<td>
								<a href = "#" onclick="acceptFormShow('${row.license_dno}')">查看</a><br/>
								<a href = "#" onclick="gaizhang('${row.license_dno}','D_ACC_FORM')">(盖章)</a>
								</td>
								<td><a href = "#" onclick="bookShow('${row.license_dno}')">查看</a></td>
							</tr>
						</c:forEach></c:if>
						<thead>
							<tr>
	...
	<script type="text/javascript"> 
		...
		function acceptFormShow(no){
			window.open("<%=request.getContextPath()%>/work/acceptFormDoc?license_dno="+no);
		}
		function cardShow(no,id){
			window.open("<%=request.getContextPath()%>/work/flowCardDoc?license_dno="+no+"&id="+id);
		}
		function bookShow(no){
			window.open("<%=request.getContextPath()%>/work/bookShow?license_dno="+no);
		}
	</script>
</body>
</html>
