...
//    		request.setAttribute("obj", list);
    		FileInfoDto ff=new FileInfoDto();
			ff.setMain_id(subId);
			ff.setRemark("XZJC");
			FileInfoDto file=dbServ.selectReport(ff);
			if(null!=file){
				File f = new File(file.getFile_location());
				if (!f.exists()) {
				...
				String fileName2 = new String(f.getName().getBytes(), "ISO-8859-1");
				...
				out.println("<script>");
				out.print("alert('File not found!');location.href=document.referrer;");
				out.println("</script>");
				out.flush();
				out.close();
			}
...
//					5656//DDALSH查看年度报告上传地点
					dbServ.saveUpload(fileInfoDto);
					List<WaringBaseBean> lw = new ArrayList<>();
					lw.add(new WaringBaseBean("XZJC_NDBG_ZDY", "",apply_no, "", user.getOrg_code()));//行政检查年度报告
