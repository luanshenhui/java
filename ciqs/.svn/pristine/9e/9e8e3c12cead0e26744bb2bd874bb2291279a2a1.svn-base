						...
						<c:forEach items="${list}" var="row">
							<tr>
								...
								<td><input type="checkbox" class="checkbox" name="check"
									...
									<c:if test="${'1' == row.STEP_19_STATUS || '2' == row.STEP_19_STATUS}">disabled</c:if> />
								</td>
								<td><a
									href='javascript:jumpPage("/ciqs/generalPunishment/detail?id=${row.ID}&step=19");'>
										${row.CASE_NO} </a></td>
								<td>${row.COMP_NAME }</td>
								<%-- <td>${row.PSN_NAME }</td> --%>
								...
								<td>
<%-- 								<c:choose>
										<c:when test="${'0' == row.STEP_19_STATUS }">
 												<a
													href='javascript:jumpPage("/ciqs/generalPunishment/detail?id=${row.ID}&step=19");'>
													上传 </a>
										</c:when>
...
<!-- 								其他 -->
						       ...
						       <td>
						       		<c:if test="${row.D_GP_DCBG_XZCFAJ_SPB != '0' && row.STEP_19_STATUS == '0'}">
								      	<a href='javascript:openNewPage("/ciqs/generalPunishment/toPage?id=${row.ID}&step=19&page=gp_anjian_spb_input&docType=dcbg&pre_report_no=${row.PRE_REPORT_NO }");'>
								      		<span class="data-btn margin-auto">在线填写</span>
								      	</a>
						       		</c:if>
						       		<c:if test="${row.D_GP_DCBG_XZCFAJ_SPB != '0' && row.STEP_19_STATUS != '0'}">
								      	<a href='javascript:openNewPage("/ciqs/generalPunishment/toPage?id=${row.ID}&step=19&page=gp_anjian_spb_input&docType=dcbg&subStep=0&pre_report_no=${row.PRE_REPORT_NO }");'>
								      		<span class="data-btn margin-auto">查看</span>
								      	</a>
						       		</c:if>
						       </td>
						       <td>
									<c:if test="${row.D_GP_DCBG_Y_S_1 != '0' && row.STEP_19_STATUS == '0'}">
								      	<a href='javascript:openNewPage("/ciqs/generalPunishment/toPage?id=${row.ID}&step=19&page=gp_anjian_spb_input&docType=dcbg_yq&pre_report_no=${row.PRE_REPORT_NO }");'>
								      		<span class="data-btn margin-auto">在线填写</span>
								      	</a>
							      	</c:if>
						       		<c:if test="${row.D_GP_DCBG_Y_S_1 != '0'  && row.STEP_19_STATUS != '0'}">
								      	<a href='javascript:openNewPage("/ciqs/generalPunishment/toPage?id=${row.ID}&step=19&page=gp_anjian_spb_input&docType=dcbg_yq&subStep=0&pre_report_no=${row.PRE_REPORT_NO }");'>
								      		<span class="data-btn margin-auto">查看</span>
								      	</a>
							      	</c:if>
							   </td>
			...
			<div style="text-align: center; margin: auto; margin-top: 10px; width: 200px; padding-bottom: 10px;">
				<input type="button" class="search-btn" value="提交" onclick="javascript:submitForm19();" />
			</div>
</html>
