...
<script type="text/javascript"> 
	...
	function toLookPdf(applyid,apply_no,compName){
		window.location.href="${ctx}/expFoodPOF/lookPdf?applyId="+applyid+"&apply_no="+apply_no+"&compName="+compName;
	}
	
	function seal(applyid,apply_no,compName){
		window.location.href="${ctx}/expFoodPOF/lookSealPdf?applyId="+applyid+"&apply_no="+apply_no+"&compName="+compName;
	}
...
<body class="bg-gary">
	...
	<div id="alertBoxId" class="box-img-bg"><span class="box-content-style" id="alertContentId"></span></div>
		<form method="post" id="select_form" action="${ctx}/expFoodPOF/expFoodList">
	...
	<div class="margin-cxjg">
		<table class="margin-cxjg_table" border="0" cellspacing="0" cellpadding="0" style="table-layout: fixed;word-break:break-all">
  			<tr class="thead">
				...
				<td width="95px;">评审任务<br/>通知书</td>
				<td width="70px;">评审组<br/>状态</td>
<!-- 				<td width="80px;">不符合项跟踪报告</td> -->
			...
			<c:if test="${not empty list }">
				<c:forEach items="${list}" var="row">
					<tr class="thead_nr">
						<td width="12%" height="90" align="center"><strong>${row.applycode}</strong></td>
						...
						<td width="9%" height="90" align="center">
							<a href="#this" onclick="toLookPdf('${row.applyid}','${row.applycode}','${row.orgname}')" style="color:#014ccc;">查看</a>
							<a style="color:#014ccc" href="#this" onclick="seal('${row.applyid}','${row.applycode}','${row.orgname}')">（盖章）</a>
						</td>
