...
// 	    				 	 $("#d2").append(title)
	    				...
	    				if($.trim($("#fz_time").html())){//发证
	    					$("#name_8").html($("#fz_person").html());//存在就赋值
    						$("#time_8").html($("#fz_time").html());
	    				}else{
	    				$(".fzclass").remove();
	    				}
	    				...
	    				for(var i=0;i<rdmPsn.length;i++){
		    				 if(rdmPsn[i].rdm_type && rdmPsn[i].type=='2' && rdmPsn[i].psn_type=='0'){
		    				 	zz+=rdmPsn[i].psn_name+",";
		    				 }else if(rdmPsn[i].rdm_type && rdmPsn[i].type=='2' && rdmPsn[i].psn_type=='1'){
		    				 	zy+=rdmPsn[i].psn_name+",";
		    				 }
		    				 $("#name_3").html(rdmPsn[i].str_rdm_user);
    						 $("#time_3").html(rdmPsn[i].str_rdm_date?rdmPsn[i].str_rdm_date.substring(0,rdmPsn[i].str_rdm_date.length-3):"");
	    				 }
...
// 		 			        }
	    					...
	    					var dataid=$(this)[0].getAttribute('id');
		 			        ...
		 			        $("#flow_time").text(ciqFormatTime(1,8,"time_"));
		 			        
	    					if($("#d2").find('tr').length==1){//备案受理
	    						$("#d2").parent().prev().remove();
	    						$("#d2").parent().remove();
	    					}
	    					if(!$("#z_z").html() && !$("#z_y").html()){
	    						$("#z_z").parent().parent().parent().remove();
	    					}
	    					if($("#d3").find('tr').length==1 && !$("#z_z").html()){//评审组任务分配,都没有数据
	    						$("#d3").parent().prev().remove();
	    						$("#d3").parent().remove();
	    					}else if($("#d3").find('tr').length==1 && $("#z_z").html()){//随机人员有数据
	    						$("#d3").remove();
	    					}
	    					
	    					if($("#d4").find('tr').length==1){//文件审核
	    						$("#d4").parent().prev().remove();
	    						$("#d4").parent().remove();
	    					}
	    					
	    					if($("#d5").parent().find('table').eq(0).find('tr').length==1
	    					&& $("#d5").parent().find('table').length==1){//现场查验 都无数据
		    					$("#d5").parent().prev().remove();
	    						$("#d5").parent().remove();
	    					}else if($("#d5").parent().find('table').length>1
	    					&& $("#d5").parent().find('table').eq(0).find('tr').length==1){
	    						$("#d5").remove();
	    					}
	    					if($("#d6").find('tr').length==1){//备案材料终审
	    						$("#d6").parent().prev().remove();
	    						$("#d6").parent().remove();
	    					}
	    					if($("#d7").find('tr').length==1){//备案审批决定、归档
	    						$("#d7").parent().prev().remove();
	    						$("#d7").parent().remove();
	    					}
    			}
    		}); 
...
<div class="title-cxjg" id="module_4">文件审核</div>
	<div class="margin-chx">
		<table class="table-xqlb" border="0" cellspacing="0" cellpadding="0" id="d4">
		</table>
	</div>
<div class="title-cxjg" id="module_5">现场查验</div>
		<div class="margin-chx">
...
<div class="title-cxjg" id="module_6">备案材料终审</div>
		<div class="margin-chx">
		<table class="table-xqlb" border="0" cellspacing="0" cellpadding="0" id="d6">
		</table>
		</div>
<div class="title-cxjg" id="module_7">备案审批决定、归档</div>
		<div class="margin-chx">
...
<div class="title-cxjg fzclass" id="module_8">发证</div>
		<div class="margin-chx fzclass" >
		<table class="table-xqlb" border="0" cellspacing="0" cellpadding="0" id="d9">
			<tr>
				<td width="220" class="table_xqlbbj">编制备案编号</td>
				<td>
				<a style="color:#23527c" id="viewLook"  href="viewLook?applyid=">原始记录查看</a>
				</td>
				<td></td>
				<td></td>
				<td colspan="2"></td>
			...
			<tr>
				...
				<td>操作人</td>
				<td id="fz_person">${o.aroperson}</td>
<!-- 				<td id="fz_person">王明伟</td> -->
				<td>操作时间</td>
				<td colspan="2" id="fz_time">
				<c:if test="${not empty o.arodate}">
					<fmt:formatDate value="${o.arodate}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/>
				</c:if>
				</td>
			</tr>
		</table>
