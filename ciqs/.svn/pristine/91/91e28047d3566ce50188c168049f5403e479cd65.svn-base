...
<script type="text/javascript"> 
		...
		function pageUtil(page) {
			$("#xk_form").attr("action", "${ctx}/expFoodPOF/psyList2?page="+page);
			$("#xk_form").submit();
		}
...
<body>
	...
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">出口食品生产企业监督检查</a> &gt; <a
				href="<%=request.getContextPath()%>/expFoodPOF/psyList2">评审员管理</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>
		<div class="search">
			<div class="main">
				<form action="<%=request.getContextPath()%>/expFoodPOF/psyList2"  method="post" id="xk_form">
		...
		<div class="table">
			...
			<div class="main">
					<div class="menu">
						<ul>
							<li><a href="javascript:jumpPage('<%=request.getContextPath()%>/expFoodPOF/createPsyForm2');"><img 
						...
						<c:if test="${not empty list }">
							<c:forEach items="${list}" var="row">
								<tr>
									...
									<td>
									<a href='#' onclick="javascript:location='<%=request.getContextPath()%>/expFoodPOF/toDetailForm?psnId=${row.psnId}&type=2'">详情</a>
									|
									<a href='#' onclick="javascript:location='<%=request.getContextPath()%>/expFoodPOF/toPsyUpdateForm?psnId=${row.psnId}&type=2'">修改</a>
									</td>
								</tr>
							</c:forEach>
						</c:if>
