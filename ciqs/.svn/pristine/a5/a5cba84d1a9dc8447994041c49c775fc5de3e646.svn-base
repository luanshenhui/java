...
//					}
	...
	public String detailsList(HttpServletRequest request,ExpFoodPOFForm expFoodPOFForm,String applyid){
		EfpeCerPouductModel model=new EfpeCerPouductModel();
    	model.setApplyid(applyid);
    	EfpeCerPouductModel efpeCerPouductModel=expFoodPOFService.getEfpeCerPouduct(model);
    	if(null!=efpeCerPouductModel){
    		request.setAttribute("o", efpeCerPouductModel);
    	}
...
//				e.setWfnoteid("390de3f510314b8bbc71933838322754");
	    	...
	    	List<CheckDocModel> doclist=expFoodPOFService.getQtOption2(param);
    		List<ExpFoodProdCheckVo> checklist=expFoodPOFService.getChecklist(expFoodProdCheckVo);
    		if(!doclist.isEmpty() && !checklist.isEmpty()){//现场检查获取
    			if(doclist.get(0).getDecDate().getTime()>checklist.get(0).getCheck_date().getTime()){
    				...
    				map.put("checkPsn", checklist.get(0).getChech_psn());
    				map.put("checkTime", DateUtil.DateToString(checklist.get(0).getCheck_date(),"yyyy-MM-dd HH:mm"));
    			}
    		}else if(!doclist.isEmpty() && checklist.isEmpty()){//doc
    			map.put("checkPsn", expFoodPOFService.findUsersByCode(doclist.get(0).getDecUser()));
				map.put("checkTime", doclist.get(0).getDecDate());
    		}else if(doclist.isEmpty() && !checklist.isEmpty()){//result
    			map.put("checkPsn", checklist.get(0).getChech_psn());
...
//				request.setAttribute("title", expFoodProdCheckVo.getCheck_menu_type()+": "+list.get(0).getCheck_title());
				request.setAttribute("title", list.get(0).getCheck_title());
			}
		} catch (Exception e) {
...
//    		doclist=list;
    		List<CheckDocModel> l=new ArrayList<CheckDocModel>();
    		if(!list.isEmpty()){
    			l.add(list.get(0));
    		}
			request.setAttribute("list", l);
	...
	protected void formSet(HttpServletRequest request,PdfStamper stamper,AcroFields acroform, Object t, String actionType) {
		...
		try {
			if(null!=actionType && actionType.equals("lookPdf")){
				...
				acroform.setField("checkDate", dto.getCheckDate()!=null?dto.getCheckDate().substring(0,10):"");
				...
				acroform.setField("noticeDate", dto.getNoticeDate()!=null?dto.getNoticeDate().substring(0,10):"");
...
//					FileUtil.downloadFile(Constants.UP_LOAD_PATH+"/2017/12" + "/e527ac99664d4660a27748095d56ce14.pdf",response,true);
		...
		public void sealPdf(HttpServletRequest request,HttpServletResponse response,EfpeApplyNoticeDto notice) {
			try {
				...
				/*盖章*/
				Boolean bo=ElectronicSealUtil.getElectronicSealPDF(fileName, user.getOrg_code()+Constants.SEAL_PDF_TYPE[1]);
				if(bo){
		...
		@SuppressWarnings({ "unchecked" })
		@RequestMapping("/lookSealPdf")
		public void lookSealPdf(HttpServletRequest request,HttpServletResponse response,EfpeApplyReviewNoticeDto efpeApplyReviewNoticeDto) {
			try {
				UserInfoDTO user=(UserInfoDTO)request.getSession().getAttribute(Constants.USER_KEY);
				List<EfpeApplyReviewNoticeDto> list=expFoodPOFService.findEfpeApplyReviewNoticeDto(efpeApplyReviewNoticeDto);
				/*pdf生成*/
				String fileName = this.toPrintFile(request,response,list.isEmpty()?efpeApplyReviewNoticeDto:list.get(0),Constants.PDF_ROOT+"/psrwtzs.pdf","lookPdf");
				/*盖章*/
				Boolean bo=ElectronicSealUtil.getElectronicSealPDF(fileName, user.getOrg_code()+Constants.SEAL_PDF_TYPE[1]);
				if(bo){
					this.toPrintPlanNote(request,response,null,fileName,"",true);
		...
		public void lookPdf(HttpServletRequest request,HttpServletResponse response,EfpeApplyReviewNoticeDto efpeApplyReviewNoticeDto) {
			try {
				...
				if(!list.isEmpty()){
					...
					ExpFoodProdPsnRdmDTO dto=new ExpFoodProdPsnRdmDTO();
					dto.setApply_no(apply_no);
					dto.setType("2");
					List<ExpFoodProdPsnRdmDTO> l=expFoodPOFService.selectRdmName(dto);
					String zz="";
					String zy="";
					for(ExpFoodProdPsnRdmDTO e:l){
						if(e.getPsn_type().equals("0")){
							zz=e.getPsn_name();
						}
						if(e.getPsn_type().equals("1")){
							zy+=e.getPsn_name()+",";
						}
					}
					efpeApplyReviewNoticeDto.setUserLeader(zz);
//					efpeApplyReviewNoticeDto.setUserName(zy);
					efpeApplyReviewNoticeDto.setOrgName(compName);
					efpeApplyReviewNoticeDto.setCheckDate("7个工作日");
					this.toPrintPlanNote(request,response,efpeApplyReviewNoticeDto,Constants.PDF_ROOT+"/"+"psrwtzs.pdf","lookPdf",false);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	    ...
	    @RequestMapping("/viewLook")
	    public String viewLook(HttpServletRequest request,@RequestParam(value="applyid", required=true) String applyid){
	    	EfpeCerPouductModel model=new EfpeCerPouductModel();
	    	model.setApplyid(applyid);
	    	EfpeCerPouductModel efpeCerPouductModel=expFoodPOFService.getEfpeCerPouduct(model);
	    	request.setAttribute("obj", efpeCerPouductModel);
	    	List<EfpeCerPouductModel> list=expFoodPOFService.getEfpeCerPouductList(efpeCerPouductModel);
	    	request.setAttribute("list", list);
	    	return "expFoodProd/viewLook";
		}
