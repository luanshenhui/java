...
<!-- 		        	          无文件 -->
<!--                     </c:if> -->
 						 <c:if test="${not empty videolist1}">
	 						 <c:forEach items="${videolist1}" var="row2">
	 							 <div><a href="javascript:void(0);" onclick="toImgDetail('${row2.fileName}')">查看图片文件</a></div>
	 						 </c:forEach>
 						 </c:if>
 						  <c:if test="${not empty videolist2}">
	 						 <c:forEach items="${videolist2}" var="row2">
	 							 <div><a href="javascript:void(0);" onclick="showVideo('${row2.fileName}')">查看视频文件</a></div>
	 						 </c:forEach>
 						 </c:if>
						<c:if test="${empty videolist1 and empty videolist2}">
		        	         	 无文件
                   		</c:if> 
              </td>
              <td width="167" height="44" align="center" class="tableLine">
