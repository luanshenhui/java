				...
				<c:if test="${not empty list }">
					<c:forEach items="${list}" var="row">
						...
						<input type="checkbox" class="checkbox" name="check"
						  ...
						  <td>
						  	<a href='javascript:openNewPage("/ciqs/generalPunishment/toPage?id=${row.ID}&step=1&page=gp_shexiananjian_sbd&update=update&pre_report_no=${row.PRE_REPORT_NO }<c:if test="${row.STEP_2_STATUS == '1' }">&subStep=0</c:if>");'>
					      ...
					      <td>
									<c:choose>
										<c:when test="${not empty row.AJFKB_DOC_ID }">
											<a
												href='javascript:openNewPage("/ciqs/generalPunishment/toPage?id=${row.ID}&step=1&page=gp_xzcf_ajfkb");'>查看</a>
								  		</c:when>
									</c:choose>
					      </td>
					      <td>
