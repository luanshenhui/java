...
<script type="text/javascript"> 
		...
		function pageUtil(page) {
			$("#xk_form").attr("action", "/ciqs/xk/findAddpesons?page="+page);
			$("#xk_form").submit();
		}
		
		function userShow(url,id,org_code) {
		   /*  if($("#peson_num_"+id).val() == ""){
		    	alert("请输入人数!");
		    	return;
		    }
		    if(isNaN($("#peson_num_"+id).val())){
		    	alert("请输入数字!");
		    	return;
		    } */
		    url+="&pid="+id+"&sl_org_code="+org_code;
			window.open(url);
		}
		...
		function setTy(license_dno){
			$("#xk_form").attr("action", "<%=request.getContextPath()%>/xk/setTy?license_dno="+license_dno);
			$.blockUI({ message: $('#showDel2'), css: { width: '275px' } });
		}
		
		function scsShow(no,id){
		    var url = "<%=request.getContextPath()%>/xk/toPsyEdit?license_dno="+no+"&sczry="+$("#"+id).val();
			window.open(url);
		}
	...
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">口岸卫生许可证核发</a> &gt; <a
				href="${cxt}/ciqs/xk/findAddpesons">随机人员查询</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>
<!-- 		<div class="title">卫生许可证查询</div> -->
		<div class="search">
			<div class="main">
				<form action="/ciqs/xk/findAddpesons"  method="post" id="xk_form">
					...
					<table>
						<thead>
							<tr>
								...
								<td>受理局</td>
								<!-- <td>人数</td> -->
								<td style="width:130px">人员姓名</td>
								<td style="width:90px">审查派员申请书</td>
								<td style="width:90px">随机分配</td>
								<td>操作</td>
							</tr>
						</thead>
						<c:if test="${not empty list }">
							<c:forEach items="${list}" var="row">
									...
									<%-- <td>
										<input id="peson_num_${row.id}" type="text" style="width:60px" <c:if test ="${not empty row.approval_users_name}">readonly="readonly"</c:if>/>
									</td> --%>
									<td>
										<input id="peson_name_2${row.id}" type="text" style="width:200px" value="${row.approval_users_name}" readonly="readonly"/>
										<input id="peson_name_${row.id}" type="hidden" style="width:200px" value="${row.approval_users_name}" readonly="readonly"/>
									</td>
									<td>
						            ...
						            <td>
						            	<input type="button" style="width:60px" value="随机分配" onclick="userShow('/ciqs/xk/addpeson?apply_no=${row.license_dno}','${row.id}','${row.org_code}')" <c:if test ="${not empty row.approval_users_name}">disabled="disabled"</c:if>/>
						            ...
						            <td>
						            	<c:if test="${row.status == 2}">
						            		<a href="#" onclick="setPeson('peson_name_${row.id}','${row.license_dno}')">提交</a>
						            	</c:if>
						            	<c:if test="${row.status >= 3}">
						            		已提交
						            	</c:if>
						            	<%-- <c:if test="${row.exam_status != 1}">
						            		<a href="#" onclick="setTy('${row.license_dno}')">同意</a>
						            	</c:if>
						            	<c:if test="${row.exam_status == 1}">
						            		同意
						            	</c:if> --%>
