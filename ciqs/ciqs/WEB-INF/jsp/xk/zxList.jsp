...
<script type="text/javascript"> 
		...
		function pageUtil(page) {
			$("#xk_form").attr("action", "<%=request.getContextPath()%>/xk/zxList?page="+page);
			$("#xk_form").submit();
		}
	...
	<div class="dpn-content">
		<div class="crumb">
			当前位置：<a href="#">口岸卫生许可证核发</a> &gt; <a
				href="${cxt}<%=request.getContextPath()%>/xk/zxList">注销审批</a>
		</div>
		<jsp:include page="/common/message.jsp" flush="true"/>	
<!-- 		<div class="title">现场评审查询</div> -->
		<div class="search">
			<div class="main">
				<form action="<%=request.getContextPath()%>/xk/zxList"  method="post" id="xk_form">
					<table class="table_search" id="aa">
						...
						<tr>
							...
							<td style="padding-top:8px;width: 250px;" align="left">注销审批状态:</td>
							<td></td>
						</tr>
						<tr>
							<td align="left" >
							...
							<td align="left">
								<select name="status" id="review_result">
									<option value="('6','12')">全部</option>
									<option value="('16')" <c:if test="${status == \"('16')\"}" >selected="selected"</c:if>>已审批</option>
									<option value="('6','12')" <c:if test="${status == \"('6','12')\"}" >selected="selected"</c:if>>未审批</option>
								</select>
							</td>
							<td></td>						
						</tr>
						            ...
						            <td>
						            	...
						            	<c:if test="${row.status == 6}">
						            		<select id="zz_${row.id}" onchange="gzs('${row.id}')">
						            			<option value="1">卫生许可有效期届满未延续</option>
												<option value="2">法人或者其他组织依法终止</option>
												
												<option value="4">卫生许可依法被撤销、撤回，或者卫生许可证件依法被吊销</option>
												<option value="5">因不可抗力导致卫生许可事项无法实施</option>
					            		...
					            		<c:if test="${row.status == 12}">      		
						            		<select id="zz_${row.id}" onchange="gzs('${row.id}')">			            		
												<option value="3">被许可人申请注销卫生许可</option>
						            		</select>
					            		</c:if>
					            		
					            		<c:if test="${row.status != 6 && row.status != 12 && not empty row.zx_result}">
						            	
						            			<c:if test ="${row.zx_result == 1}">卫生许可有效期届满未延续</c:if>
												<c:if test ="${row.zx_result == 2}">法人或者其他组织依法终止</c:if>
												<c:if test ="${row.zx_result == 3}">被许可人申请注销卫生许可</c:if>
												<c:if test ="${row.zx_result == 4}">卫生许可依法被撤销、撤回，或者卫生许可证件依法被吊销</c:if>
												<c:if test ="${row.zx_result == 5}">因不可抗力导致卫生许可事项无法实施</c:if>				
						            			<c:if test ="${row.zx_result == 6}">法律、法规规定的应当注销卫生许可的其他情形</c:if>
					            		</c:if>
					            		<br/>
