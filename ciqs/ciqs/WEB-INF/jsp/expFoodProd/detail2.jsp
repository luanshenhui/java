...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table-xqlb">
    ...
    <tr class="table_xqlbnr">
      ...
      <a href="/ciqs/expFoodProd/jumpText?userId=${userId}&subId=${subId}">查看年度报告</a>
      </td>
    </tr>
    </table>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table-xqlb">
...
<!-- 				    <td width="250" class="table_xqlbbj2">整改报告照片&nbsp;<c:if test="${fn:length(V_XCCY_CZ_ZG) gt 1 }">${status.count }</c:if></td> -->
				    <c:if test="${status.count == 1}"><td width="200" class="table_xqlbbj2" rowspan="${fn:length(V_XCCY_CZ_ZG)}">整改报告照片&nbsp;</td></c:if>
				    <td align="center">
				    	操作时间:&nbsp;&nbsp;&nbsp;&nbsp;
				    	<span class='blue'>
				    		<fmt:formatDate value="${v.create_date }" type="both" pattern="yyyy-MM-dd HH:mm:ss"/>
		...
		<!-- q其他	 -->		
			...
			<tr class="table_xqlbnr">
		      ...
		      <td width="220" height="90" align="center" ><a href="/ciqs/expFoodProd/unPassXwjable2?apply_no=${apply_no}&compName=${model.orgname}&type=V_SP_A_C">不符合项及跟踪报告</a></td>
		    </tr>
		    </c:if>
	</table>
    </div>
...
<!-- 				    <td width="250" class="table_xqlbbj2">整改报告照片&nbsp;<c:if test="${fn:length(V_SP_RP_6) gt 1 }">${status.count }</c:if></td> -->
				    <c:if test="${status.count == 1}"><td width="200" class="table_xqlbbj2" rowspan="${fn:length(V_SP_RP_6)}">整改报告照片&nbsp;</td></c:if>
				    <td align="center">
				    	操作时间:&nbsp;&nbsp;&nbsp;&nbsp;
				    	<span class='blue'>
				    		<fmt:formatDate value="${v.create_date }" type="both" pattern="yyyy-MM-dd HH:mm:ss"/>
		...
		<tr class="table_xqlbnr">
	      <td width="220" height="90" align="center" colspan="2">不符合项及跟踪报告</td>
	      <td width="220" height="90" align="center" ><a href="/ciqs/expFoodProd/unPassXwjable2?apply_no=${apply_no}&compName=${model.orgname}&type=V_SP_A_D">不符合项及跟踪报告</a></td>
	    </tr>
	    </c:if>
		<!-- q其他	 -->		
