...
<div class="dpn-content">
    <div class="crumb">
	             当前位置：口岸卫生许可证核发 &gt;<span class="need">许可证申报修改  </span>
    </div>
     <div class="form">
		<div class="main">
    <form action="<%=request.getContextPath()%>/dc/licenseDecUpdate" method="post" id="licenseDecForm"  enctype="multipart/form-data">
	<%-- <form styleId="LicenseDecForm" action="/addLicenseDec.do?doType=addLicenseDec" enctype="multipart/form-data" method="post"> --%>
        ...
        <input type="hidden" id="approval_result" name="approval_result" value="${dto.approval_result }"/>
		...
		<table id="form_table">
			...
			<tr>
				<th style="width:30%;" id="left_noline"><span id="hygieneLicenseSpan" class="need"></span>原卫生许可证证号：</th>
				<td id="right_noline" colspan="3">
					<input type = "text" id = "hygiene_license_number" name = "hygiene_license_number" value="${dto.hygiene_license_number}" maxlength="50"/>
				</td>
			</tr>
			<tr>
				<th style="width:30%;" id="left_noline">审批结果</th>
				<td id="right_noline" colspan="3">
			...
			<c:if test="${not empty dto.filePath}">
				<tr>
					...
					<td id="right_noline" colspan="3">
						<input type = "button" value="下载附件" onclick="downloadFile(1)"/>
					</td>
				</tr>
			</c:if>
			<tr>
		...
		<!-- 	<tr>
				<th style="width:30%;" id="left_noline">上传附件说明：</th>
				<td id="right_noline" colspan="3">
					<input type = "button" value="点击查看说明" onclick="javascript:openWin('/pages/xk/uploadDisc.jsp','newwindow',700,500)"/>
				</td>
			</tr> -->
			<c:if test="${dto.approval_result == 3}">
				<tr>
					<th style="width:30%;" id="left_noline">准予受理决定书：</th>
					<td id="right_noline" colspan="3">
						<input type = "button" value="点击查看" onclick="slWinShow()"/>
					</td>
				</tr>
			</c:if>
			<c:if test="${dto.approval_result == 2}">
				<tr>
					<th style="width:30%;" id="left_noline">不予受理决定书：</th>
					<td id="right_noline" colspan="3">
						<input type = "button" value="点击查看" onclick="byWinShow()"/>
					</td>
				</tr>
			</c:if>
			<%-- <c:if test="${not empty dto.filePath2}">
				<tr>
					<th style = "width:30%;" id="left_noline">查看整改报告：</th>
					<td id="right_noline" colspan="3">
						<input type = "button" value="下载整改报告" onclick="downloadFile(2)"/>
					</td>
				</tr>
			</c:if> --%>
			<%-- <c:if test="${dto.approval_result == 1 || dto.approval_result == 2}">
				<tr>
					<th style="width:30%;" id="left_noline">上传整改报告：</th>
					<td id="right_noline" colspan="3">
						<input type = "file" id = "zgbgFileUpload" name = "file"  />
					</td>
				</tr>
			</c:if> --%>
		</table>
