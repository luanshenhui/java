...
<body>
	<form:form id="searchForm" modelAttribute="dcaTopicPhysics" action="${ctx}/dca/dcaTopicPhysics/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<shiro:hasPermission name="dca:dcaTopicPhysics:edit">
				<li class="btns pull-right"><a href="${ctx}/dca/dcaTopicPhysics/tableAdd" class="btn-s btn-add "><i>+</i>新建</a></li>
			</shiro:hasPermission>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th width="30%">物理表中文名</th>
				<shiro:hasPermission name="dca:dcaTopicPhysics:edit"><th width="30%">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${page.list}" var="dcaTopicPhysics" step="1" varStatus="topicPhysicsStatus">
				<tr>
					<td>
						<c:out value="${topicPhysicsStatus.index + (1 + (page.pageNo - 1) * page.pageSize)}"/>
					</td>
					<td>
					...
					<shiro:hasPermission name="dca:dcaTopicPhysics:edit"><td>
	    				<a href="${ctx}/dca/dcaTopicPhysics/showDetail?id=${dcaTopicPhysics.tableName}" class="btn-s btn-opear">详情</a>
	    				<a href="${ctx}/dca/dcaTopicPhysics/tableEdit?id=${dcaTopicPhysics.tableName}&tableComment=${dcaTopicPhysics.tableComment}" class="btn-s btn-opear">编辑</a>
