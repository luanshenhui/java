...
<body>
	...
	<div class="container-fluid breadcrumb">
		<div class="row-fluid span11 ">
			<span class="span4 font-normal">角色名称: <b>${role.name}</b></span>
			<span class="span4 font-normal">归属机构: ${role.office.name}</span>
			<span class="span4 font-normal">英文名称: ${role.enname}</span>
		</div>
		<div class="row-fluid span9">
			<span class="span4 font-normal">角色类型: ${role.roleType}</span>
			<c:set var="dictvalue" value="${role.dataScope}" scope="page" />
			<span class="span4 font-normal">数据范围: ${fns:getDictLabel(dictvalue, 'sys_data_scope', '')}</span>
		</div>
	</div>
	...
	<div class="span3">
		...
		<input id="assignButton" class="btn-s btn-opear" type="submit" value="分配角色"/>
		...
		<c:forEach items="${userList}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td>
					<a href="${ctx}/sys/role/outrole?userId=${user.id}&roleId=${role.id}" class="btn-s btn-opear"
