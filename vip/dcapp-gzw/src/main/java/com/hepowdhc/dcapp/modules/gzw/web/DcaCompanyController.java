...
import com.hepowdhc.dcapp.modules.gzw.service.DcaCompanyService;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/dca/dcaCompany")
public class DcaCompanyController extends BaseController {

	@Autowired
	private DcaCompanyService dcaCompanyService;
	...
	@RequiresPermissions("dca:dcaCompany:view")
	...
	public String list(DcaCompany dcaCompany, HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		return "modules/gzw/dcaCompanyList";
	}
	...
	public String form(DcaCompany dcaCompany, Model model) {
		...
		return "modules/gzw/dcaCompanyForm";
	}

	@RequiresPermissions("dca:dcaCompany:edit")
	...
	public String save(DcaCompany dcaCompany, Model model, RedirectAttributes redirectAttributes) {
		...
		return "redirect:"+Global.getAdminPath()+"/dca/dcaCompany/?repage";
	}
	...
	public String importFile(MultipartFile file, RedirectAttributes redirectAttributes, HttpServletRequest request) {
		try {
			...
			String companyYm = request.getParameter("companyYm");
			if ("".equals(company)) {
				addMessage(redirectAttributes, "企业名称，必须选择！");
			}

			if ("".equals(companyYm)){
				addMessage(redirectAttributes, "年月，必须输入！");
			}
			
			if ("".equals(company) || "".equals(companyYm)){
				return "redirect:" + adminPath + "/dca/dcaCompany/list?repage";
			}
			
			try {  
		        int year = Integer.parseInt(companyYm.substring(0, 4));  
		        if (year <= 0) {
					addMessage(redirectAttributes, "请输入正确年月！例如(201701)");
					return "redirect:" + adminPath + "/dca/dcaCompany/list?repage";
				}  
		        int month = Integer.parseInt(companyYm.substring(4, 6));  
		        if (month <= 0 || month > 12)  {
					addMessage(redirectAttributes, "请输入正确年月！例如(201701)");
					return "redirect:" + adminPath + "/dca/dcaCompany/list?repage";
				}
			...
			companyp.setCompanyYm(companyYm);
	...
	public String reportAll(HttpServletRequest request,HttpServletResponse response, RedirectAttributes redirectAttributes) {
		return "modules/gzw/dcaCompanyReportAll";
	 ...
	 */
	@RequiresPermissions("dca:dcaCompany:edit")
	@RequestMapping(value = "reportYm", method = RequestMethod.POST)
	public String reportYM(DcaCompany dcaCompany, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
		request.getSession().setAttribute("companyId", dcaCompany.getCompanyId());
		request.getSession().setAttribute("companyY", dcaCompany.getCompanyYm());
		String company = dcaCompany.getCompanyId();
		String companyYm = dcaCompany.getCompanyYm();
		if ("".equals(company)) {
			addMessage(redirectAttributes, "企业名称，必须选择！");
		}
		...
		if ("".equals(company) || "".equals(companyYm)){
			return "redirect:" + Global.getAdminPath() + "/dca/dcaCompany/list?repage";
		}
		...
		return "modules/gzw/dcaCompanyReportYM";
	 ...
	 */
	@RequiresPermissions("dca:dcaCompany:view")
	@RequestMapping(value = "reportTime")
	public String reportTime(DcaCompany dcaCompany, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
		request.getSession().setAttribute("companyYm", dcaCompany.getCompanyYm());
		...
		return "modules/gzw/dcaCompanyTime";
	...
	public String reportYmData(HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		String companyId = String.valueOf(request.getSession().getAttribute("companyId"));
		String companyY = String.valueOf(request.getSession().getAttribute("companyY"));
		
		DcaCompany dcompanyp = new DcaCompany();
		dcompanyp.setCompanyId(companyId);
		dcompanyp.setCompanyY(companyY.substring(0, 4).toString());

		List<DcaCompany> companylistAll = dcaCompanyService.findListY(dcompanyp);
		DcaCompanyReport companyReport = new DcaCompanyReport();
		TreeMap<String, Integer> mapReviewType = new TreeMap<String, Integer>();
		TreeMap<String, Integer> mapCompanyYM = new TreeMap<String, Integer>();
		mapCompanyYM.put(dcompanyp.getCompanyY()+"01", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"02", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"03", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"04", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"05", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"06", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"07", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"08", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"09", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"10", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"11", 1);
		mapCompanyYM.put(dcompanyp.getCompanyY()+"12", 1);
		...
		for (String typekey : mapReviewType.keySet()) {
			List<String> typeCountlist = Lists.newArrayList();
			for (String companykey : mapCompanyYM.keySet()) {
				DcaCompany companypCount = new DcaCompany();
				companypCount.setCompanyId(dcompanyp.getCompanyId());
				companypCount.setCompanyYm(companykey);
				companypCount.setReviewType(typekey+",");
	 ...
	 * @date 2016年12月5日
	 */
	@RequestMapping(value = "reportTimeData")
	@ResponseBody
	public String reportTimeData(Model model,HttpServletRequest request, RedirectAttributes redirectAttributes) {

		String companyYm = String.valueOf(request.getSession().getAttribute("companyYm"));
		
		if ("null".equals(companyYm)) {
			addMessage(redirectAttributes, "导入年月，必须输入！");
		}
		DcaCompany dcaCompany = new DcaCompany();
		
		dcaCompany.setCompanyYm(companyYm);
		DcaCompanyReport companyReport = new DcaCompanyReport();
		List<DcaCompany> companylistAll = dcaCompanyService.findListTime(dcaCompany);
		List<String> companylist = Lists.newArrayList();
		for(DcaCompany company:companylistAll){
			String companyName = DictUtils.getDictLabel(company.getCompanyId(), "company_name", "");
