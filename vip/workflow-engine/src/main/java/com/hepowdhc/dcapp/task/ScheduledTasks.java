package com.hepowdhc.dcapp.task;
 ...
 * <p>
 * “/”字符用于指定增量。如：“0/15”在秒域意思是每分钟的0，15，30和45秒。“5/15”在分钟域表示每小时的5，20，35和50。 符号“*”在“/”前面（如：*
 * /10）等价于0在“/”前面（如：0/10）。记住一条本质：表达式的每个数值域都是一个有最大值和最小值的集合，如： 秒域和分钟域的集合是0-59，日期域是1-31，月份域是1-12。字符“/”可以帮助你在每个字符域中取相应的数值。如：“7/6”在月份域的时候只 有当7月的时候才会触发，并不是表示每个6月。
 * <p>
...
//
//    private final String corn = engineConfig.getProperty("engine.corn");
...
//    @Scheduled(cron = corn)
    public void reportCurrentTime() {
        ...
        try {
            ...
            Optional<FlowEngine> engineOptional = Optional.ofNullable(flowEngine.initialization());

            if (engineOptional.isPresent()) {

                engineOptional.get().execute();
            ...
            e.printStackTrace();
            logger.error("引擎运行时错误:", e.fillInStackTrace());
        }
