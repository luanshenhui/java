...
$.fn.extend({
	jsplit:function(j){
		return this.each(function(){
			...
			var _self = this;
			var Close = false;
			//因去掉了内部元素中的滚动条，所以修改一下	
			//$(_self).css({ position:"relative",float:j.FloatD,overflow:"hidden",padding:"0px"});
			$(_self).css({ position:"relative",float:j.FloatD,padding:"0px"});
			...
			$(_self).wrapInner("<div class='jsplit-c' style='top:0px;z-index:9999;zoom:1;width:100%;position:relative;height:100%'></div>");
		    ...
		    $(_self).children(".jsplit-c").append("<div class='jsplit-e' unselectable='on' style='background:#fff;height:100%;width:6px;top:0px;-moz-user-select:none;"+antiD+":0px;position:absolute;cursor:e-resize;z-index:10000;'><div class='jsplit-e-handle'  unselectable='on'  style='height:40px;width:100%;top:50%;margin-top:-20px;left:0;position:absolute;cursor:pointer;-moz-user-select:none;'></div></div>");
			var dw = $(_self).width();
