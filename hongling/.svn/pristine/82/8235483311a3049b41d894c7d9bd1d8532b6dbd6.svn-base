...
<script type="text/javascript" src="<%=request.getContextPath()%>/pages/orden/addOrden.js"></script>
...
<body>
		...
		<form id="form" class="form_template">
				...
				<tr>
					<td align="left">
						...
						<div id="container_clothings">
							<ul>
								<s:iterator value="dicts" var="dict_temp">
									<li><s:if test="#dict_temp.ID==1">
											<label class="clothRadioChecked">
										</s:if> <s:else>
											<label>
										</s:else> <s:if test="#dict_temp.ID==1">
											<input type='radio' name='clothingID' checked="checked"
												value='${ID}' onclick='$.csOrdenPost.clothingChange(${ID});' />
										</s:if> <s:else>
											<input type='radio' name='clothingID' value='${ID}'
												onclick='$.csOrdenPost.clothingChange(${ID});' />
										</s:else> <s:property value="name" /> </label></li>
								</s:iterator>
							</ul>
						</div>
						<!-- 客户信息 -->
						...
						<div id="container_customer" style="padding-left: 40px">
							<table style="width: 70%;" align="left">
								<tr>
									...
									<td colspan="2"><input type="text" id="userNo"
										name="userNo" class="textbox" maxlength="32"/></td>
								</tr>
								<tr>
									<td class="label lblName star"><s:text name="lblCustomerName"></s:text>
									</td>
									<td colspan="2"><input type="hidden" name="ID" id="ID" />
										<s:if
											test="#session.company.iscompany == 10051 && #session.company.singOnBean.customerName != '' && #session.company.singOnBean.customerName != null">
											<input type="text" id="name" name="name" class="textbox" maxlength="50" 
												value="<s:property value="#session.company.singOnBean.customerName" />"
												disabled />
										</s:if> <s:else>
											<input type="text" id="name" name="name" class="textbox" maxlength="50"/>
										</s:else>
								...
								<tr>
									...
									<td><input type="text" id="height" name="height"
										class="textbox" oncontextmenu="return false;"
										onkeyup="$.csOrdenPost.clearNoNum(this)"
										style="width:120px;height:18px;" /></td>
									<td><select id="heightUnitID" name="heightUnitID"
										class="w80">
											<option value="10266">
								...
								<tr>
									...
									<td style="width:100px;"><input type="text" id="weight"
										oncontextmenu="return false;"
										onkeyup="$.csOrdenPost.clearNoNum(this)" name="weight"
										class="textbox" style="width:120px;height:18px;" /></td>
								...
								<tr>
									...
									<td colspan="2"><input type="text" id="address"
										name="address" class="textbox" maxlength="300"/></td>
								</tr>
								<tr>
									<td class="label lblLtName"><s:text name="lblLtName"></s:text>
									</td>
									<td colspan="2"><input type="text" id="ltName"
										name="ltName" class="textbox" maxlength="4" /></td>
								</tr>
								<tr>
									<td class="label lblMemo"><s:text name="lblMemo"></s:text>
									</td>
									<td colspan="2">
										<textarea id="memos" name="memos" rows="3" cols="25" 
										style="background-color: #FFFAFA;"></textarea>
									</td>
								</tr>
							</table>
						...
						<div style="width: 50%">
							<table class="fabricCode">
								<tr>
									<td class="star label" width="40%"><s:text
											name="lblFabric"></s:text>
									</td>
									<td><input type="text" id="fabricCode" name="fabricCode" /><span></span>
					...
					<td class="padding_left20">
						...
						<div id="container_component">
							<s:iterator value="cloth_double">
								<div class='list_search'>
									<s:property value="name" />
								</div>
								<table id='category_${ID}' class='list_result'>
									<tr>
										<td><input type='text' id='text_${ID}_1' style='width:80px' class='textbox'/><span/></td>
									</tr>
								</table>
							</s:iterator>
						</div></td>
			...
			<div id="container_embroid" style="padding-left: 50px">
				<s:iterator value="cloth_double">
					<div class='list_search'>
						<s:property value="name" />
						<a onclick='$.csOrdenPost.addEmbroidRow(${ID})'><s:text
								name="btnCreateFabric"></s:text> </a><input id='clothing${ID}'
							style=' display: none;' />
					</div>
					<table id='category_embroid_${ID}' class='list_result'></table>
				</s:iterator>
			</div>
				...
				<table id="size" style="clear:both;width: :100%">
					<tr>
						<td id="size_info">
							...
							<div id="size_spec_part" class="horizontal">
								${size_spec_part}
							</div>
							<div id="size_bodytype" class="horizontal">
								${size_bodytype}
							</div>
							<div id="style_title" class="horizontal" style="float:left;"><span id="styleContainer">
								${style_title}
							</span></div>
	...
	<form id="toOrdenListPage" action="" method="post"></form>
