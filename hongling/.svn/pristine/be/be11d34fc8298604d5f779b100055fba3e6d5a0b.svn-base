...
//			self.location=document.referrer;//后退一步
			window.location.href='/hongling/orden/dordenPage.do';
		}
	},
	validate: function() {
		if ($("#memos").val().length >100){
			$("#memos").val($("#memos").val().substring(0, 100));
...
//		$.csOrdenPost.generateEmbroid(clothingID,orden);
	...
	getComponentRowHTML: function(clothingID, index) {
		return "<tr index='" + index + "'>" + "<td><input type='text' id='text_" + clothingID + "_" + index + "' style='width:130px' class='textbox'/><span/></td>" + "<td onclick='$(this).parent().remove();' " +
//				"$.csOrdenPost.removeStyle('','"+clothingID+"','"+ index++ +"',$(this)) " +
						"style='width:30px'><a class='remove'></a></td>" + "</tr>";
	},
							   ...
							   while(hasEmpty){
								   var lineCount = $("#category_" + clothingID + " tr").length;
								    for(var u=0 ;u<lineCount ;u++){
								    	var trDom = $("#category_" + clothingID + " tr").eq(u);
							    ...
							    //此处默认当前带款式号行数为最后一行
							    if(trCount>1){
								    for(var y = 0;y<trCount-1;y++){
				//					    codesStr += $("#category_" + clothingID + " tr").eq(x).children().eq(0).eq(0).first().children().eq(0).attr('value')+",";
									    tempCodeStr = $("#category_" + clothingID + " tr").eq(y).find('input:first').attr('value');
...
//		alert('oldclothingID : '+oldclothingID +"\n" + 'clothingID : '+ clothingID);
		...
		$.ajax({
                ...
                dataType:"json",
                async:false,
                data:{changeClothID:clothingID,
            		changeOrdenID:$('#ordenID2').val(),
            		size_unit:size_unitID},
                success:function(data){
                	//无库存的 托管
//                	if(''==$('#fabricCode').val() || 0.0==data.fabricInventory){
//                		$('#autoContainer').html(data.fabricInventoryHtml);
//                	}else{
//                		$('#fabric_result').html(data.fabricInventoryHtml);
//                	}
                	
//                	if( '' != $('#fabricCode').val()){
//            			var fbs = "1,2,3,2000,4000,5000";
//            			//面料
//            			var oldclothingID = $.csCore.invoke($.csCore.buildServicePath('/service/orden/gettempclothingid'));
//            			alert("oldclothingID: "+oldclothingID +"\n" + "clothingID: "+clothingID);
////            			
//            			if(oldclothingID != clothingID && (fbs.indexOf(oldclothingID)<0 || fbs.indexOf(clothingID)<0)){
//            				$('#fabricCode').val('');
//            			}else if(oldclothingID == clothingID || ((fbs.indexOf(oldclothingID)>=0 && fbs.indexOf(clothingID)>=0))){
//            				$('#fabricCode').val(data.fabricCode);
//            				if(data.fabricInventory>0){
//            					$('#fabric_result').html(data.fabricInventoryHtml);
//            				}else{
//            					$('#autoContainer').html(data.fabricInventoryHtml);
//            				}
...
//		$.csOrdenPost.generateEmbroid(clothingID,'');
	...
	loadSize: function(clothingID, orden) {
		...
		$.csCore.invoke($.csCore.buildServicePath('/service/orden/settempclothingid'), $.csControl.appendKeyValue("", "clothingid", clothingID));
		$.csCore.loadPage("container_size", $.csOrdenPost.getPath()+"/pages/size/size9.jsp",
		function() {
...
//		}
		...
		var embroid = "<tr align='center' index='"
				+ index
				+ "'>"
				+ "<td><span/><select id='category_label_"
				+ clothingID
				+ "_Position_"
				+ index
				+ "' style='width: 120px' onchange=$.csOrdenPost.changePosition('"
				+ clothingID
				+ "','"
				+ index
				+ "')/></td>"
				...
				+ index
				+ "' style='width: 120px' /></td>"
				+ "<td><span/><input type='text' id='category_textbox_"
				+ clothingID
				+ "_Content_"
				+ index
				+ "' style='width:120px;background-color: #FFF; border: 1px solid #626061; color: #000;height: 20px;line-height: 20px;' class='category_textbox_"
				+ clothingID
				+ "_Content_"
				+ index
				+ "'/></td>"
				+ size
				+ "<td onclick='$(this).parent().remove()' style='width:30px'><a class='remove'></a></td></tr>";
		return embroid;
	},
...
//				$('#autoContainer').html("<label style='color:red;'>"+$.csCore.getValue("Orden_FabricArrivedControl") + "</label><br />"); 暂时隐藏
