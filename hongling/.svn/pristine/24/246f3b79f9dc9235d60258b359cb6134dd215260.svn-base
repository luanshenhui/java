...
public class GetStyleByCode extends BaseServlet {
	...
	protected void service(HttpServletRequest request,
		...
		super.service();
		try {
			String strCode = getParameter("code");//款式Code
			String strFabric = getParameter("fabric");//面料
//			Assemble assemble = new StyleUIManager().getAssembleByCode(strCode);
			Assemble assemble = new StyleUIManager().getStyleByCode(strCode,strFabric);
			...
			e.printStackTrace();
			LogPrinter.debug("GetStyleByID_err" + "----" + e.getMessage());
		}
	}
}
