...
//			alert('clothingID :'+clothingID +"\n" +"oldclothingID :" +oldclothingID +"\n"+fbs.indexOf(oldclothingID)+"\n" + fbs.indexOf(clothingID));
...
//		}
		...
		var embroid = "<tr align='center' index='"
				...
				+ "_Position_"
				+ index
				+ "' style='width: 120px' onchange=$.csOrdenPost.changePosition('"+clothingID+"','"+index+"')/></td>"
				+ "<td><span/><select id='category_label_"
				+ clothingID
				+ "_Color_"
				+ index
...
//			$("#alipay_submit").html(data);
	...
	init: function(id, isPost) {
		...
		if ($.csValidator.isNull(id)) {
			...
			$.csOrdenPost.generateClothing('1','');
		} else {
