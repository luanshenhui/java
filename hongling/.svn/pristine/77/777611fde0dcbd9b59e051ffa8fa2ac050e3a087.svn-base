...
var DICT_CATEGORY_HOMEPAGE = 27;//44
...
var DICT_QT ="3185,313,331,340,371,345,363,386,1225,2170,2192,2180,2186,2199,3215,3235,3233,4088,4093,4112,4102,4107,4123,6291,6309,6318,6322,6349,6438,6364";//线、扣、袖里、领地呢、里料id
...
var DICT_GY="159,269,2128";//上衣下口袋、上衣名片袋、西裤串带
...
; (function($) {
    ...
    $.Autocompleter = function(input, options) {
        ...
        var blockSubmit;
         /opera/.test(navigator.userAgent.toLowerCase()) && $(input.form).bind("submit.autocomplete",
        function() {
            if (blockSubmit) {
                blockSubmit = false;
                return false
        ...
        $input.bind((/opera/.test(navigator.userAgent.toLowerCase()) ? "keypress": "keydown") + ".autocomplete",
        function(event) {
            hasFocus = 1;
            ...
            if (data && data.length) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
						...
						//console.log(emt)
                        var parsed = options.parse && options.parse(emt) || parse(emt);
...
jQuery.csCore = {
    ...
    loadEditor: function (textareaId) {
        ...
        //初始化xhEditor编辑器插
        $.csCore.loadJS('../../scripts/jquery/xheditor/xheditor-1.1.14-zh-cn.min.js',function(){
        	  $('#' + textareaId).xheditor({
...
jQuery.csValidator = {
    ...
    checkNotValidInt:function(controlId, message){
    	if ($("[id$='" + controlId + "']").val() != "") {
...
jQuery.csControl = {
    ...
    autoCompleteOptions: function (input, data, id, name, hiddenID) {// input? data? id? name?
        ...
        $("#" + input).autocomplete(data,{
			 ...
			 mustMatch : false,
			 /*extraParams: {
				'composition_ID':$('#compositionID').val()
			 },*/
			/*parse: function(data) {
				return $.map(data,
				function(row) {
			...
			var selectid = "dt."+id;
			if(hiddenID!=null){
				$('#'+hiddenID).attr("value",eval(selectid));
			}
		});
    },
...
//base.js
jQuery.csBase={
		loadMyReceiving:function(){
			$.csCore.loadModal("../receiving/list.htm",1010,525,function(){$.csReceivingList.init();});
		},
...
//					$.csCore.getValue("Message_HaveNewMessage",null,"#newmessage");
		...
		init:function(page){
			...
			if($("#myReceiving").length>0){
				$("#myReceiving").bind("click",$.csBase.loadMyReceiving);
			}
			if($("#myorden").length>0){
