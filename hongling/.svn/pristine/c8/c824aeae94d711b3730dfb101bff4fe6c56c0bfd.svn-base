		...
		try {
			...
			for (int numSheet = 0; numSheet < workbook.getNumberOfSheets(); numSheet++) {
				...
				for(int i=3;i<row;i++){
						...
						deliveryId = cellDeliveyrId.getContents();
...
//							count =count+1;
							...
							memberName=cellMemberName.getContents();
...
//										}
							    ...
							    info[0]=memberName+"failed";
							    info[1]+="<br/>工作薄" + sheet.getName()+ "中运单号：("+deliveryId+")信息不完整<br/>";
                            }
						}else{
							workbook.close();
							is.close();
			...
			info[0]="failed";
			info[1]+="<br/>工作薄 " + sheet.getName()+ "中的运单号：("+deliveryId+")信息不完整<br/>";
			info[1]+="<br/>工作薄 " + sheet.getName()+ "中的运单号：("+deliveryId+")以及其以下的运单均未导入成功<br/>";
//			return info;
		} finally {
			if (is != null) {
				is.close();
