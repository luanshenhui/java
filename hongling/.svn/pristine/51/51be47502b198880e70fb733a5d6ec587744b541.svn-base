...
//			}
	...
	generateParts: function(singleClothingID) {
		...
		for (var i = 0; i < parts.length; i++) {
			...
			dom += " onblur=$.csSize.validatePartRange('" + this.buildPartID(parts[i].partID) + "','" + parts[i].ID + "','" + parts[i].sizeFrom + "','" + parts[i].sizeTo + "') value='" + defaultValue + "' id='" + this.buildPartID(parts[i].partID) + "' name='" +tun+ this.buildPartID(parts[i].partID) + "'/></li>";
		}
		dom += "</ul>";
...
//			$f("video_ipad", "../../scripts/jquery/flowplayer/flowplayer-3.2.15.swf").ipad();
//		} else {
//			$("#size_video").show();
