...
function loadSuccessFile(file,fsUploadProgress,ID,filePath,saveDataId){
	try {
		...
		//var _filePath = decodeURI(jsonObj.filenameindisk);
		//var ID = jsonObj.ID;
		...
		if(filePath==undefined){
			...
			//设置下载删除
			progress.setDownDeleteLink(ID,filePath,saveDataId);
...
function uploadSuccess(file, serverData) {
	try {
		...
		var attachment;
		if (IS_NET) {
		    attachment = JSON.parse(serverData);
        }else{
            attachment = JSON.parse(serverData);
        }
        var ID = attachment.ID;
		var _filePath = attachment.fileNameInDisk;
		if(_filePath==undefined){
			progress.setStatus(attachment.error);
			progress.setError();
			//progress.toggleCancel(true,this);
		}else{            var hiddenID = attachment.hiddenID;
			//设置下载删除
            progress.setDownDeleteLink(ID, _filePath, hiddenID);
            var oldData = $("#" + hiddenID).val();
			var newData;
			if(oldData!=""){
				newData = oldData+","+ID;	
			}else{
