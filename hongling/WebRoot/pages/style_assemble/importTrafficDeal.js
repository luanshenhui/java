// 导入EXL文件的JS
jQuery.csImportTrafficDealAssemble = {
	bindLabel : function() {
		$.csCore.getValue("Button_Submit", null, "#btnSave");
		$.csCore.getValue("Button_Cancel", null, "#btnCancel");
	},
	bindEvent : function() {
		$("#btnSave").click($.csImportTrafficDealAssemble.save);
		$("#btnCancel").click($.csCore.close);

		$('#uploadFile').change(function() {
			$.csImportTrafficDealAssemble.checkFile(this);
		});
	},
	...
	save : function() {
		...
		// .buildServicePath("/service/assemble/importAssemble"), "form").length
		// == 0) {
		// $.csAssembleList.list(0);
		// $.csCore.close();
		// }
		// var path = $.csImportAssemble.getRootPath();
		var requestUrl = $.csCore.buildServicePath('/service/assemble/importtrafficdealprocess');
		$.ajaxFileUpload({
