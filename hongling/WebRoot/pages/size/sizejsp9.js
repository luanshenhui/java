...
//		this.generateBodyType(orden);
	...
	changeSizeUnit : function(){
		...
		var sizeHeight = $.csControl.getRadioValue("sizeUnitID");
		var localObj = window.location;
		var contextPath = localObj.pathname.split("/")[1];
...
//         $('#size_video').html('');
		$.ajax({
			url:server_context+'/ordenajax/changeUnit_changeSizeInfoAction.do',
            dataType:"json",
            async:false,
            data:{changeOrdenID:$('#ordenID2').val(),
            		changeClothID:clothingID,
            		changeSize_category:size_category,
            		changeSize_unit:sizeUnitID},
            success:function(data){
				$('#size_area').html(data.size_areaHtml);
				$('#size_spec_part').html(data.size_spec_partHtml);
...
//		var param = $.csControl.appendKeyValue("", "sizecategoryid", $.csControl.getRadioValue("size_category"));
		var clothingID= $.csControl.getRadioValue("clothingID");
		var size_category = $.csControl.getRadioValue("size_category");
		var size_unit = $.csControl.getRadioValue("sizeUnitID");
		var localObj = window.location;
		var contextPath = localObj.pathname.split("/")[1];
		var basePath = localObj.protocol + "//" + localObj.host + "/"
				+ contextPath;
		var server_context = basePath;
         ...
         $('#size_video').html('');
		$.ajax({
			url : server_context + '/ordenajax/changeSizeCategoryAction.do',
			dataType : "json",
			async : false,
			data : {
				changeClothID : clothingID,
				changeOrdenID : $('#ordenID2').val(),
				changeSize : size_category,
				changeSize_unit : size_unit
			},
			success : function(data) {
				$('#size_area').html(data.size_areaHtml);
				$('#size_spec_part').html(data.size_spec_partHtml);
				$('#size_bodytype').html(data.size_bodytypeHtml);
				$('#styleContainer').html(data.style_titleHtml);
				$('#size_video').html(data.imgHtml);
			}
		});
		...
		var areas = $.csCore.invoke($.csCore.buildServicePath('/service/size/getarea'), param);
		/*
		 * if ($.csControl.getRadioValue("size_category") ==
		 * DICT_SIZE_CATEGORY_STANDARD) { $("#size_bodytype").hide(); } else {
		 * $("#size_bodytype").show(); // if
		 * ($.csControl.getRadioValue("size_category") ==
		 * DICT_SIZE_CATEGORY_NAKED) { // $("#clothing_style").hide(); // } else {
		 * $("#clothing_style").show(); // } }
		 */
		if (!$.csValidator.isNull(areas)) {
			var dom = "<ul><li>" + $.csCore.getValue("Size_Code") + "：</li>";
	...
	generateSpec: function() {
		...
		var size_category = $.csControl.getRadioValue("size_category");
		var sizeUnitID = $.csControl.getRadioValue("sizeUnitID");
		var sizeArea = $.csControl.getRadioValue("area");
		var localObj = window.location;
		var contextPath = localObj.pathname.split("/")[1];
		var basePath = localObj.protocol + "//" + localObj.host + "/"
				+ contextPath;
...
//         $('#size_video').html('');
		$.ajax({
			url:server_context+'/ordenajax/changeArea_changeSizeInfoAction.do',
            dataType:"json",
            async:false,
            data:{changeOrdenID:$('#ordenID2').val(),
            		changeClothID:clothingID,
...
//				$('#size_video').html(data.imgHtml);
		...
		function(i, singleClothing) {
			$("#3PCS").val("");
			if(singleClothings.length == 3){//3件套
				$("#3PCS").val("3");
			}
	...
	generateSpecChest: function(singleClothingID) {
		...
		var sizeUnitID = $.csControl.getRadioValue("sizeUnitID");
		var sizeArea = $.csControl.getRadioValue("area");
		var sizeHeight = $.csControl.getRadioValue("size_spec_height_"+singleClothingID);
		...
		var basePath = localObj.protocol + "//" + localObj.host + "/"
				+ contextPath;
		var server_context = basePath;
		$('#part_'+singleClothingID).html('');
		$.ajax({
			url:server_context+'/ordenajax/changeHeight_changeSizeInfoAction.do',
            dataType:"json",
            async:false,
            data:{changeOrdenID:$('#ordenID2').val(),
            		changeClothID:singleClothingID,
            		changeSize_category:size_category,
            		changeSize_unit:sizeUnitID,
            		changeSize_area:sizeArea,
            		changeSize_height:sizeHeight},
            success:function(data){
            	$('#part_'+singleClothingID).html(data.size_heightHtml);
            }
            });
...
//			$.csSize.generateArea();
		...
		if ($.csControl.getRadioValue("size_category") == DICT_SIZE_CATEGORY_NAKED) {
			...
			if(orden != null && orden.styleDY != null){
				...
				for(var i=0;i<arr.length;i++){
					if(orden.styleDY == arr[0]){
						checkedDY=" checked='true' ";
					}
					strDY+="<label style='display:inline;clear:none;'><input type='radio' name='styleDY' value='"+arr[0]+"' "+checkedDY+"/>"+$.csCore.getValue("DY_"+arr[i])+ "</label> ";;
				}
			}else{
				strDY="<label style='display:inline;clear:none;'><input type='radio' name='styleDY' value='0A02' checked='true'/>"+$.csCore.getValue("DY_0A02")+"</label><label style='display:inline;clear:none;'><input type='radio' name='styleDY' value='0A01'/>"+$.csCore.getValue("DY_0A01")+ "</label> ";;
			}
	...
	getSingleClothings: function() {
		...
		var clothingID = $('#container_clothings input:checked').val();
		if(undefined == clothingID){
			return;
		}
		var param = $.csControl.appendKeyValue("", "clothingID", clothingID);
		var singleClothings = $.csCore.invoke($.csCore.buildServicePath('/service/clothing/getsingleclothings'),param);
