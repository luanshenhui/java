...
var pageShadowLeftImgUrl  ="spongebob/black_gradient.png";
var pageShadowRightImgUrl ="spongebob/black_gradientr.png";
...
function ini(){
	...
	for(var i=0;i<pagesDiv.childNodes.length;i++){
		if(pagesDiv.childNodes[i].nodeType == 1){ //element nodes only
			...
			if(pageShadow){createShadow(newElement,j);}; //翻页阴影效果
			if(j>0){createList(newElement,j);}; //下拉列表翻页
...
//		myBook.style.height=(pHeight+pBorders)+'px';
		myBook.style.width='900px';
		myBook.style.height='650px';
...
//			myDiv.style.top="0px";
			myDiv.style.left=(45+pWidth+pBorders)+'px';
			myDiv.style.top="31px";
...
function clipmeR(){//前一页
	...
	goleft(-pWidth,pWidth);//左翻页
}
function goleft(currLeft,currWidth){//左翻页
	if(currLeft >=(pWidth-(numPixels*2))){
		window.clearTimeout(ID);
		p2.style.left=(45+pWidth+pBorders)+"px";
...
function clipmeL(){//后一页
	...
	goright(pWidth*2,0);//右翻页
}
function goright(currLeft,currWidth){//右翻页
